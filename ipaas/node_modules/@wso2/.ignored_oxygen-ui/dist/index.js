var __defProp = Object.defineProperty;
var __defProps = Object.defineProperties;
var __getOwnPropDescs = Object.getOwnPropertyDescriptors;
var __getOwnPropSymbols = Object.getOwnPropertySymbols;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __propIsEnum = Object.prototype.propertyIsEnumerable;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __spreadValues = (a, b) => {
  for (var prop in b || (b = {}))
    if (__hasOwnProp.call(b, prop))
      __defNormalProp(a, prop, b[prop]);
  if (__getOwnPropSymbols)
    for (var prop of __getOwnPropSymbols(b)) {
      if (__propIsEnum.call(b, prop))
        __defNormalProp(a, prop, b[prop]);
    }
  return a;
};
var __spreadProps = (a, b) => __defProps(a, __getOwnPropDescs(b));
var __objRest = (source, exclude) => {
  var target = {};
  for (var prop in source)
    if (__hasOwnProp.call(source, prop) && exclude.indexOf(prop) < 0)
      target[prop] = source[prop];
  if (source != null && __getOwnPropSymbols)
    for (var prop of __getOwnPropSymbols(source)) {
      if (exclude.indexOf(prop) < 0 && __propIsEnum.call(source, prop))
        target[prop] = source[prop];
    }
  return target;
};

// inline-css-fonts:/home/runner/work/oxygen-ui/oxygen-ui/packages/oxygen-ui/src/fonts.css
(function() {
  if (typeof document !== "undefined") {
    const style = document.createElement("style");
    style.setAttribute("data-oxygen-fonts", "true");
    style.textContent = `/**
 * Copyright (c) 2025, WSO2 LLC. (https://www.wso2.com).
 *
 * WSO2 LLC. licenses this file to you under the Apache License,
 * Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied. See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */

/* inter-cyrillic-ext-wght-normal */
@font-face {
  font-family: 'Inter Variable';
  font-style: normal;
  font-display: swap;
  font-weight: 100 900;
  src: url(data:font/woff2;base64,) format('woff2-variations');
  unicode-range: U+0460-052F,U+1C80-1C8A,U+20B4,U+2DE0-2DFF,U+A640-A69F,U+FE2E-FE2F;
}

/* inter-cyrillic-wght-normal */
@font-face {
  font-family: 'Inter Variable';
  font-style: normal;
  font-display: swap;
  font-weight: 100 900;
  src: url(data:font/woff2;base64,) format('woff2-variations');
  unicode-range: U+0301,U+0400-045F,U+0490-0491,U+04B0-04B1,U+2116;
}

/* inter-greek-ext-wght-normal */
@font-face {
  font-family: 'Inter Variable';
  font-style: normal;
  font-display: swap;
  font-weight: 100 900;
  src: url(data:font/woff2;base64,) format('woff2-variations');
  unicode-range: U+1F00-1FFF;
}

/* inter-greek-wght-normal */
@font-face {
  font-family: 'Inter Variable';
  font-style: normal;
  font-display: swap;
  font-weight: 100 900;
  src: url(data:font/woff2;base64,) format('woff2-variations');
  unicode-range: U+0370-0377,U+037A-037F,U+0384-038A,U+038C,U+038E-03A1,U+03A3-03FF;
}

/* inter-vietnamese-wght-normal */
@font-face {
  font-family: 'Inter Variable';
  font-style: normal;
  font-display: swap;
  font-weight: 100 900;
  src: url(data:font/woff2;base64,) format('woff2-variations');
  unicode-range: U+0102-0103,U+0110-0111,U+0128-0129,U+0168-0169,U+01A0-01A1,U+01AF-01B0,U+0300-0301,U+0303-0304,U+0308-0309,U+0323,U+0329,U+1EA0-1EF9,U+20AB;
}

/* inter-latin-ext-wght-normal */
@font-face {
  font-family: 'Inter Variable';
  font-style: normal;
  font-display: swap;
  font-weight: 100 900;
  src: url(data:font/woff2;base64,) format('woff2-variations');
  unicode-range: U+0100-02BA,U+02BD-02C5,U+02C7-02CC,U+02CE-02D7,U+02DD-02FF,U+0304,U+0308,U+0329,U+1D00-1DBF,U+1E00-1E9F,U+1EF2-1EFF,U+2020,U+20A0-20AB,U+20AD-20C0,U+2113,U+2C60-2C7F,U+A720-A7FF;
}

/* inter-latin-wght-normal */
@font-face {
  font-family: 'Inter Variable';
  font-style: normal;
  font-display: swap;
  font-weight: 100 900;
  src: url(data:font/woff2;base64,) format('woff2-variations');
  unicode-range: U+0000-00FF,U+0131,U+0152-0153,U+02BB-02BC,U+02C6,U+02DA,U+02DC,U+0304,U+0308,U+0329,U+2000-206F,U+20AC,U+2122,U+2191,U+2193,U+2212,U+2215,U+FEFF,U+FFFD;
};
`;
    document.head.appendChild(style);
  }
})();

// package.json
var package_default = {
  name: "@wso2/oxygen-ui",
  version: "0.5.0",
  description: "WSO2 Oxygen UI | Design System - Powered with Material-UI component library with TypeScript support",
  type: "module",
  main: "./dist/index.js",
  module: "./dist/index.js",
  types: "./dist/index.d.ts",
  exports: {
    ".": {
      types: "./dist/index.d.ts",
      import: "./dist/index.js",
      require: "./dist/index.cjs"
    },
    "./*": {
      types: "./dist/*.d.ts",
      import: "./dist/*.js",
      require: "./dist/*.cjs"
    }
  },
  bin: {
    "oxygen-ui": "./bin/cli.js"
  },
  files: [
    "dist",
    "package.json",
    "README.md",
    ".claude",
    ".ai",
    "bin"
  ],
  sideEffects: [
    "src/styles/Themes/theme.css",
    "dist/styles/Themes/theme.css"
  ],
  scripts: {
    build: "pnpm clean && tsc -p tsconfig.json && node esbuild.config.js",
    "build:watch": "node esbuild.config.js --watch",
    dev: "node esbuild.config.js --watch",
    typecheck: "tsc --noEmit",
    lint: "eslint src",
    "lint:fix": "eslint src --fix",
    clean: "rimraf dist",
    test: "vitest run --passWithNoTests",
    "test:watch": "vitest"
  },
  keywords: [
    "wso2",
    "oxygen-ui",
    "design-system",
    "material-ui",
    "mui",
    "react",
    "typescript",
    "components",
    "ui-library"
  ],
  author: "WSO2 LLC",
  license: "Apache-2.0",
  repository: {
    type: "git",
    url: "https://github.com/wso2/oxygen-ui.git"
  },
  bugs: {
    url: "https://github.com/wso2/oxygen-ui/issues"
  },
  homepage: "https://github.com/wso2/oxygen-ui#readme",
  dependencies: {
    "@fontsource-variable/inter": "5.2.8",
    "@emotion/react": "catalog:",
    "@emotion/styled": "catalog:",
    "@mui/material": "catalog:",
    "@mui/x-data-grid": "catalog:",
    "@mui/x-date-pickers": "catalog:",
    "@mui/x-tree-view": "catalog:",
    "date-fns": "4.1.0",
    prismjs: "^1.30.0"
  },
  peerDependencies: {
    "@wso2/oxygen-ui-icons-react": ">=0.1.0",
    react: "catalog:",
    "react-dom": "catalog:"
  },
  devDependencies: {
    "@eslint/js": "catalog:",
    "@types/prismjs": "^1.26.5",
    "@types/react": "catalog:",
    "@types/react-dom": "catalog:",
    "@typescript-eslint/eslint-plugin": "catalog:",
    "@typescript-eslint/parser": "catalog:",
    "@vitest/ui": "catalog:",
    "@wso2/esbuild-plugin-inline-css-fonts": "workspace:^",
    "@wso2/oxygen-ui-icons-react": "workspace:^",
    esbuild: "catalog:",
    eslint: "catalog:",
    "eslint-plugin-react": "catalog:",
    "eslint-plugin-react-hooks": "catalog:",
    rimraf: "catalog:",
    typescript: "catalog:",
    vite: "catalog:",
    vitest: "catalog:"
  },
  publishConfig: {
    access: "public"
  },
  nx: {
    targets: {
      build: {
        dependsOn: [
          "^build"
        ]
      }
    }
  }
};

// src/styles/Themes/AcrylicBaseTheme.ts
import { extendTheme as extendTheme2 } from "@mui/material/styles";

// src/styles/Themes/OxygenThemeBase.ts
import { extendTheme } from "@mui/material/styles";
import { ChevronDown, X } from "@wso2/oxygen-ui-icons-react";
import React from "react";

// src/utils/formatRelativeTime.ts
function formatRelativeTime(date) {
  const now = /* @__PURE__ */ new Date();
  const diffMs = now.getTime() - date.getTime();
  const diffSec = Math.floor(diffMs / 1e3);
  const diffMin = Math.floor(diffSec / 60);
  const diffHour = Math.floor(diffMin / 60);
  const diffDay = Math.floor(diffHour / 24);
  if (diffSec < 60) {
    return "Just now";
  } else if (diffMin < 60) {
    return `${diffMin} minute${diffMin > 1 ? "s" : ""} ago`;
  } else if (diffHour < 24) {
    return `${diffHour} hour${diffHour > 1 ? "s" : ""} ago`;
  } else if (diffDay < 7) {
    return `${diffDay} day${diffDay > 1 ? "s" : ""} ago`;
  } else {
    return date.toLocaleDateString();
  }
}

// src/utils/pxToRem.ts
var pxToRem = (size) => `${size / 14}rem`;

// src/styles/Themes/OxygenThemeBase.ts
var OxygenThemeBase = extendTheme({
  cssVarPrefix: "oxygen",
  colorSchemeSelector: "data-color-scheme",
  typography: {
    fontFamily: "'Inter Variable', sans-serif",
    fontWeightRegular: 400,
    fontSize: 14,
    h1: {
      fontSize: pxToRem(36),
      fontWeight: 400
    },
    h2: {
      fontSize: pxToRem(30),
      fontWeight: 400
    },
    h3: {
      fontSize: pxToRem(24),
      fontWeight: 400
    },
    h4: {
      fontSize: pxToRem(18),
      fontWeight: 400
    },
    h5: {
      fontSize: pxToRem(16),
      fontWeight: 400
    },
    h6: {
      fontSize: pxToRem(14),
      fontWeight: 500
    },
    subtitle1: {
      fontSize: pxToRem(18)
    },
    subtitle2: {
      fontSize: pxToRem(14),
      fontWeight: 400
    },
    body1: {
      fontSize: pxToRem(14)
    },
    body2: {
      fontSize: pxToRem(12),
      fontWeight: 400
    },
    caption: {
      fontSize: pxToRem(11),
      fontWeight: 400
    }
  },
  shadows: Array(25).fill("none"),
  colorSchemes: {
    light: {
      palette: {
        primary: {
          main: "#ff7300",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#757575",
          contrastText: "#ffffff"
        },
        background: {
          default: "#fafafa",
          paper: "#ffffff"
        }
      }
    },
    dark: {
      palette: {
        primary: {
          main: "#ff7300",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#757575",
          contrastText: "#ffffff"
        },
        background: {
          default: "#121212",
          paper: "#121212"
        }
      }
    }
  },
  components: {
    MuiAutocomplete: {
      defaultProps: {
        popupIcon: React.createElement(ChevronDown),
        clearIcon: React.createElement(X)
      }
    },
    MuiSelect: {
      defaultProps: {
        size: "small",
        IconComponent: ChevronDown
      },
      styleOverrides: {
        icon: {
          top: "50%",
          transform: "translateY(-50%)",
          marginTop: 0
        },
        select: {
          paddingTop: "10px",
          paddingBottom: "10px"
        }
      }
    },
    MuiTabs: {
      styleOverrides: {
        root: {
          "&.MuiTabs-vertical": {
            alignItems: "flex-start"
          }
        },
        flexContainer: {
          "&.MuiTabs-flexContainerVertical": {
            alignItems: "flex-start"
          }
        }
      }
    },
    MuiTab: {
      styleOverrides: {
        root: ({ theme }) => ({
          textTransform: "none",
          fontWeight: 400,
          fontSize: theme.typography.body1.fontSize,
          "&.MuiTab-root": {
            minHeight: 48,
            padding: "8px 16px"
          },
          ".MuiTabs-vertical &": {
            width: "100%",
            minHeight: 40,
            padding: "8px 16px",
            alignItems: "center",
            justifyContent: "flex-start",
            textAlign: "left"
          }
        })
      }
    }
  }
});
var customProperties = {
  blur: {
    light: "blur(3px)",
    medium: "blur(10px)",
    heavy: "blur(20px)"
  },
  border: {
    width: "1px",
    style: "solid"
  },
  gradient: {
    primary: "linear-gradient(90deg, #F47B20 0%, #EF4223 100%)"
  },
  syntax: {
    light: {
      background: "#f5f5f5",
      text: "#24292e",
      comment: "#6a737d",
      keyword: "#d73a49",
      string: "#032f62",
      function: "#6f42c1",
      number: "#005cc5",
      operator: "#d73a49"
    },
    dark: {
      background: "#1e1e1e",
      text: "#d4d4d4",
      comment: "#6a9955",
      keyword: "#569cd6",
      string: "#ce9178",
      function: "#dcdcaa",
      number: "#b5cea8",
      operator: "#d4d4d4"
    }
  }
};
Object.assign(OxygenThemeBase, customProperties);
if (OxygenThemeBase.vars) {
  Object.assign(OxygenThemeBase.vars, customProperties);
}
var OxygenThemeBase_default = OxygenThemeBase;

// src/styles/Themes/AcrylicBaseTheme.ts
var AcrylicBaseTheme = extendTheme2(OxygenThemeBase_default, {
  colorSchemes: {
    light: {
      palette: {
        primary: {
          main: "#fa7b3f",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#e8e8e8"
        },
        background: {
          default: "#f5f5f5",
          paper: "#ffffffe1",
          acrylic: "#ffffff78"
        },
        text: {
          primary: "#40404B",
          secondary: "#40404B"
        },
        divider: "#00000012"
      }
    },
    dark: {
      palette: {
        primary: {
          main: "#F87643",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#3c3c3c"
        },
        background: {
          default: "#000000",
          paper: "#000000b8",
          acrylic: "#00000045"
          // More transparent version for Paper and Card components
        },
        text: {
          primary: "#efefef",
          secondary: "#D0D3E2"
        },
        divider: "#ffffff16"
      }
    }
  },
  shape: {
    borderRadius: 12
  },
  components: {
    MuiAvatar: {
      styleOverrides: {
        root: ({ theme }) => ({
          backgroundColor: theme.vars.palette.secondary.main,
          color: theme.vars.palette.text.primary
        })
      }
    },
    MuiButton: {
      styleOverrides: {
        root: {
          borderRadius: 20,
          textTransform: "none",
          fontWeight: 400
        },
        containedPrimary: ({ theme }) => ({
          "&:not(:disabled)": {
            background: `${theme.gradient.primary} !important`,
            color: "#fff !important",
            "&:hover": {
              background: `${theme.gradient.primary} !important`,
              opacity: 0.9,
              color: "#fff !important"
            }
          }
        }),
        outlinedPrimary: ({ theme }) => ({
          borderColor: theme.palette.primary.main,
          color: theme.palette.primary.main,
          "&:hover": {
            borderColor: theme.palette.primary.dark,
            backgroundColor: theme.palette.action.hover
          }
        }),
        containedSecondary: ({ theme }) => ({
          backgroundColor: theme.vars.palette.secondary.main,
          color: theme.vars.palette.text.primary,
          "&:hover": {
            backgroundColor: theme.vars.palette.secondary.main,
            opacity: 0.8
          }
        }),
        outlinedSecondary: ({ theme }) => ({
          borderColor: theme.vars.palette.divider,
          color: theme.vars.palette.text.primary,
          "&:hover": {
            borderColor: theme.vars.palette.text.secondary,
            backgroundColor: theme.palette.action.hover
          }
        })
      }
    },
    MuiFab: {
      styleOverrides: {
        primary: ({ theme }) => ({
          background: theme.gradient.primary,
          color: "#fff",
          "&:hover": {
            background: theme.gradient.primary,
            opacity: 0.9
          }
        })
      }
    },
    MuiDialog: {
      styleOverrides: {
        paper: ({ theme }) => ({
          WebkitBackdropFilter: theme.blur.medium,
          backdropFilter: theme.blur.medium,
          background: theme.vars.palette.background.paper
        })
      }
    },
    MuiDrawer: {
      styleOverrides: {
        paper: ({ theme }) => ({
          background: theme.vars.palette.background.paper
        })
      }
    },
    MuiAppBar: {
      styleOverrides: {
        root: ({ theme }) => ({
          backgroundColor: `${theme.vars.palette.background.acrylic} !important`,
          WebkitBackdropFilter: theme.blur.medium,
          backdropFilter: theme.blur.medium,
          borderRadius: 0
        })
      },
      defaultProps: {
        color: "transparent"
      }
    },
    MuiFormLabel: {
      styleOverrides: {
        root: ({ theme }) => ({
          fontSize: theme.typography.body2.fontSize,
          marginBottom: "2px"
        })
      }
    },
    MuiPaper: {
      styleOverrides: {
        root: ({ theme }) => ({
          borderRadius: theme.shape.borderRadius,
          WebkitBackdropFilter: theme.blur.light,
          backdropFilter: theme.blur.light,
          border: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`,
          // More transparent for direct Paper component usage
          backgroundColor: theme.vars.palette.background.acrylic,
          boxShadow: theme.shadows[1]
        })
      }
    },
    MuiPopover: {
      styleOverrides: {
        paper: ({ theme }) => ({
          background: theme.vars.palette.background.paper,
          opacity: 0.95
        })
      }
    },
    MuiCard: {
      styleOverrides: {
        root: ({ theme }) => ({
          // More transparent for Card component
          backgroundColor: theme.vars.palette.background.acrylic
        })
      }
    },
    MuiTextField: {
      defaultProps: { size: "small" }
    },
    MuiOutlinedInput: {
      styleOverrides: {
        root: ({ theme }) => ({
          fontSize: theme.typography.body2.fontSize,
          borderRadius: 8,
          backgroundColor: theme.vars.palette.background.acrylic
        }),
        notchedOutline: ({ theme }) => ({
          borderWidth: 1,
          borderColor: theme.vars.palette.divider
        })
      }
    },
    MuiMenuItem: {
      styleOverrides: {
        root: ({ theme }) => ({
          fontSize: theme.typography.body2.fontSize
        })
      }
    },
    MuiLink: {
      styleOverrides: {
        root: ({ theme }) => ({
          color: theme.palette.primary.main,
          textDecoration: "none",
          "&:hover": {
            color: theme.palette.primary.dark,
            textDecoration: "underline"
          }
        })
      }
    },
    MuiListItemButton: {
      styleOverrides: {
        root: ({ theme }) => ({
          borderRadius: 8,
          "&.Mui-selected": {
            // Use CSS variable - automatically adapts to color scheme
            backgroundColor: theme.palette.action.selected,
            "&:hover": {
              backgroundColor: theme.palette.action.hover
            }
          }
        })
      }
    },
    MuiDataGrid: {
      styleOverrides: {
        root: ({ theme }) => ({
          border: "none",
          backgroundColor: "transparent",
          overflow: "hidden",
          "& .MuiDataGrid-columnHeaders": {
            backgroundColor: theme.vars.palette.action.hover,
            WebkitBackdropFilter: theme.blur.medium,
            backdropFilter: theme.blur.medium,
            borderBottom: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`,
            position: "sticky",
            top: 0,
            zIndex: 10
          },
          "& .MuiDataGrid-virtualScroller": {
            marginTop: "0 !important",
            overflow: "auto"
          }
        }),
        columnHeaders: ({ theme }) => ({
          backgroundColor: theme.vars.palette.action.hover,
          WebkitBackdropFilter: theme.blur.medium,
          backdropFilter: theme.blur.medium,
          borderBottom: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`
        }),
        columnHeader: {
          backgroundColor: "transparent"
        },
        cell: ({ theme }) => ({
          borderBottom: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`,
          backgroundColor: "transparent"
        })
      }
    }
  }
});
var AcrylicBaseTheme_default = AcrylicBaseTheme;

// src/styles/Themes/AcrylicOrangeTheme.ts
import { extendTheme as extendTheme3 } from "@mui/material/styles";
var AcrylicOrangeTheme = extendTheme3(AcrylicBaseTheme_default, {
  components: {
    MuiCssBaseline: {
      styleOverrides: {
        "html[data-color-scheme='dark'] body": {
          backgroundAttachment: "fixed",
          backgroundImage: `
            radial-gradient(circle at 65% 30%, rgb(255 116 0 / 13%) 10%, rgba(0, 0, 0, 0) 60% 40%),
            radial-gradient(circle at 15% 50%, rgb(132 40 0 / 18%) 1%, rgba(0, 0, 0, 0) 40% 70%),
            radial-gradient(circle at center, rgba(0, 0, 0, 0.6) 0%, var(--oxygen-palette-background-default) 100%)
          `,
          backgroundBlendMode: "screen"
        },
        "html[data-color-scheme='light'] body": {
          backgroundAttachment: "fixed",
          backgroundImage: `
            radial-gradient(circle at 65% 30%, rgba(255, 116, 0, 0.1) 10%, rgba(255, 255, 255, 0) 40% 40%),
            radial-gradient(circle at 15% 50%, rgba(74, 41, 165, 0.1) 1%, rgba(255, 255, 255, 0) 40% 70%),
            radial-gradient(circle at center, rgba(255, 255, 255, 0.6) 0%, var(--oxygen-palette-background-default) 100%)
          `,
          backgroundBlendMode: "normal"
        }
      }
    }
  }
});
var AcrylicOrangeTheme_default = AcrylicOrangeTheme;

// src/styles/Themes/AcrylicPurpleTheme.ts
import { extendTheme as extendTheme4 } from "@mui/material/styles";
var AcrylicPurpleThemeBase = extendTheme4(AcrylicBaseTheme_default, {
  colorSchemes: {
    light: {
      palette: {
        primary: {
          main: "#646cff",
          mainChannel: "100 108 255",
          light: "#8187ff",
          dark: "#4651cc",
          contrastText: "#ffffff"
        },
        action: {
          selected: "rgba(100, 108, 255, 0.16)",
          selectedOpacity: 0.16
        }
      }
    },
    dark: {
      palette: {
        primary: {
          main: "#646cff",
          mainChannel: "100 108 255",
          light: "#8187ff",
          dark: "#4651cc",
          contrastText: "#ffffff"
        },
        action: {
          selected: "rgba(100, 108, 255, 0.24)",
          selectedOpacity: 0.24
        }
      }
    }
  },
  components: {
    MuiButton: {
      styleOverrides: {
        containedPrimary: () => ({
          "&:not(:disabled)": {
            background: "linear-gradient(77.74deg, #5567d5 11.16%, #7155d5ff 99.55%) !important",
            color: "#fff !important",
            "&:hover": {
              background: "linear-gradient(77.74deg, #5567d5 11.16%, #7155d5ff 99.55%) !important",
              opacity: 0.9,
              color: "#fff !important"
            }
          }
        })
      }
    },
    MuiTableRow: {
      styleOverrides: {
        root: {
          "&.Mui-selected": {
            backgroundColor: "rgba(100, 108, 255, 0.16)",
            "&:hover": {
              backgroundColor: "rgba(100, 108, 255, 0.24)"
            }
          }
        }
      }
    },
    MuiCssBaseline: {
      styleOverrides: {
        "html[data-color-scheme='dark'] body": {
          backgroundAttachment: "fixed",
          backgroundImage: `
            radial-gradient(circle at 65% 30%, rgb(85 103 213 / 20%) 10%, rgba(0, 0, 0, 0) 60% 40%),
            radial-gradient(circle at 15% 50%, rgb(113 85 213 / 25%) 1%, rgba(0, 0, 0, 0) 40% 70%),
            radial-gradient(circle at center, rgba(0, 0, 0, 0.6) 0%, var(--oxygen-palette-background-default) 100%)
          `,
          backgroundBlendMode: "screen"
        },
        "html[data-color-scheme='light'] body": {
          backgroundAttachment: "fixed",
          backgroundImage: `
            radial-gradient(circle at 65% 30%, rgba(100, 108, 255, 0.12) 10%, rgba(255, 255, 255, 0) 40% 40%),
            radial-gradient(circle at 15% 50%, rgba(113, 85, 213, 0.15) 1%, rgba(255, 255, 255, 0) 40% 70%),
            radial-gradient(circle at center, rgba(255, 255, 255, 0.6) 0%, var(--oxygen-palette-background-default) 100%)
          `,
          backgroundBlendMode: "normal"
        }
      }
    }
  }
});
var acrylicPurpleGradient = {
  gradient: {
    primary: "linear-gradient(77.74deg, #5567d5 11.16%, #7155d5ff 99.55%)"
  }
};
Object.assign(AcrylicPurpleThemeBase, acrylicPurpleGradient);
if (AcrylicPurpleThemeBase.vars) {
  Object.assign(AcrylicPurpleThemeBase.vars, acrylicPurpleGradient);
}
var AcrylicPurpleTheme = AcrylicPurpleThemeBase;
var AcrylicPurpleTheme_default = AcrylicPurpleTheme;

// src/styles/Themes/ChoreoTheme.ts
import { extendTheme as extendTheme5 } from "@mui/material/styles";
var ChoreoThemeBase = extendTheme5(OxygenThemeBase_default, {
  shape: {
    borderRadius: 8
  },
  colorSchemes: {
    light: {
      palette: {
        primary: {
          main: "#5567d5",
          mainChannel: "85 103 213",
          light: "#7a8aef",
          dark: "#3d4db0",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#8d91a3",
          light: "#f7f8fb",
          dark: "#40404b"
        },
        error: {
          main: "#ef4444",
          light: "#f87171",
          dark: "#dc2626"
        },
        warning: {
          main: "#f59e0b",
          light: "#fbbf24",
          dark: "#d97706"
        },
        success: {
          main: "#22c55e",
          light: "#4ade80",
          dark: "#16a34a"
        },
        info: {
          main: "#5567d5",
          light: "#7a8aef",
          dark: "#3d4db0"
        },
        background: {
          default: "#f7f8fb",
          paper: "#ffffff"
        },
        text: {
          primary: "#1d2028",
          secondary: "#4a4a5a",
          disabled: "#9ca3af"
        },
        divider: "#e6e7ec",
        action: {
          active: "rgba(85, 103, 213, 0.56)",
          hover: "rgba(85, 103, 213, 0.08)",
          selected: "rgba(85, 103, 213, 0.16)",
          selectedOpacity: 0.16,
          focus: "rgba(85, 103, 213, 0.24)"
        }
      }
    },
    dark: {
      palette: {
        primary: {
          main: "#6b7de0",
          mainChannel: "107 125 224",
          light: "#8b9aef",
          dark: "#5567d5",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#4a4e5e",
          light: "#6b7280",
          dark: "#2d2d35"
        },
        error: {
          main: "#f87171",
          light: "#fca5a5",
          dark: "#ef4444"
        },
        warning: {
          main: "#fbbf24",
          light: "#fcd34d",
          dark: "#f59e0b"
        },
        success: {
          main: "#4ade80",
          light: "#86efac",
          dark: "#22c55e"
        },
        info: {
          main: "#6b7de0",
          light: "#8b9aef",
          dark: "#5567d5"
        },
        background: {
          default: "#0d0d14",
          paper: "#1a1a24"
        },
        text: {
          primary: "#efefef",
          secondary: "#d0d3e2",
          disabled: "#6b7280"
        },
        divider: "#ffffff16",
        action: {
          active: "rgba(107, 125, 224, 0.56)",
          hover: "rgba(107, 125, 224, 0.12)",
          selected: "rgba(107, 125, 224, 0.24)",
          selectedOpacity: 0.24,
          focus: "rgba(107, 125, 224, 0.32)"
        }
      }
    }
  },
  components: {
    MuiButton: {
      styleOverrides: {
        root: {
          textTransform: "none",
          fontWeight: 400
        },
        containedPrimary: () => ({
          "&:not(:disabled)": {
            background: "linear-gradient(77.74deg, #4a5bc8 11.16%, #745bcc 99.55%) !important",
            color: "#fff !important",
            "&:hover": {
              background: "linear-gradient(77.74deg, #4a5bc8 11.16%, #745bcc 99.55%) !important",
              opacity: 0.9,
              color: "#fff !important"
            }
          }
        })
      }
    },
    MuiButtonBase: {
      styleOverrides: {
        root: {
          "&.Mui-focusVisible": {
            outline: "2px solid currentColor",
            outlineOffset: "2px"
          }
        }
      }
    },
    MuiPaper: {
      styleOverrides: {
        root: ({ theme }) => ({
          borderRadius: theme.shape.borderRadius,
          border: `1px solid ${theme.vars.palette.divider}`,
          boxShadow: "0 0 1px rgba(141,145,163,0.4), 0 1px 2px rgba(203,206,219,0.3)"
        })
      }
    },
    MuiAppBar: {
      styleOverrides: {
        root: ({ theme }) => ({
          backgroundColor: theme.vars.palette.background.paper,
          borderBottom: `1px solid ${theme.vars.palette.divider}`,
          boxShadow: "none"
        })
      }
    },
    MuiTextField: {
      defaultProps: { size: "small" }
    },
    MuiOutlinedInput: {
      styleOverrides: {
        root: ({ theme }) => ({
          fontSize: theme.typography.body2.fontSize
        }),
        notchedOutline: ({ theme }) => ({
          borderColor: theme.vars.palette.divider
        })
      }
    },
    MuiTableRow: {
      styleOverrides: {
        root: {
          "&.Mui-selected": {
            backgroundColor: "rgba(85, 103, 213, 0.16)",
            "&:hover": {
              backgroundColor: "rgba(85, 103, 213, 0.24)"
            }
          },
          "[data-color-scheme='dark'] &.Mui-selected": {
            backgroundColor: "rgba(107, 125, 224, 0.24)",
            "&:hover": {
              backgroundColor: "rgba(107, 125, 224, 0.32)"
            }
          }
        }
      }
    },
    MuiTooltip: {
      styleOverrides: {
        tooltip: ({ theme }) => ({
          backgroundColor: theme.vars.palette.background.paper,
          color: theme.vars.palette.text.primary,
          border: `1px solid ${theme.vars.palette.divider}`,
          fontSize: theme.typography.caption.fontSize
        })
      }
    }
  }
});
var choreoGradient = {
  gradient: {
    primary: "linear-gradient(77.74deg, #4a5bc8 11.16%, #745bcc 99.55%)"
  }
};
var choreoSyntax = {
  syntax: {
    light: {
      background: "#f8f9fc",
      text: "#1d2028",
      comment: "#8d91a3",
      keyword: "#5567d5",
      string: "#16a34a",
      function: "#745bcc",
      number: "#ea580c",
      operator: "#40404b"
    },
    dark: {
      background: "#0d0d14",
      text: "#e4e4ef",
      comment: "#6b7280",
      keyword: "#6b7de0",
      string: "#4ade80",
      function: "#a78bfa",
      number: "#fb923c",
      operator: "#d0d3e2"
    }
  }
};
Object.assign(ChoreoThemeBase, choreoGradient, choreoSyntax);
if (ChoreoThemeBase.vars) {
  Object.assign(ChoreoThemeBase.vars, choreoGradient, choreoSyntax);
}
var ChoreoTheme = ChoreoThemeBase;
var ChoreoTheme_default = ChoreoTheme;

// src/styles/Themes/ClassicTheme.ts
import { extendTheme as extendTheme6 } from "@mui/material/styles";
var ClassicTheme = extendTheme6(OxygenThemeBase_default, {
  shadows: Array(25).fill("none")
});
var ClassicTheme_default = ClassicTheme;

// src/styles/Themes/HighContrastTheme.ts
import { extendTheme as extendTheme7 } from "@mui/material/styles";
var HighContrastThemeBase = extendTheme7(OxygenThemeBase_default, {
  colorSchemes: {
    light: {
      palette: {
        primary: {
          main: "#0000FF",
          light: "#4040FF",
          dark: "#0000CC",
          contrastText: "#FFFFFF"
        },
        secondary: {
          main: "#FFD700",
          light: "#FFED4E",
          dark: "#CCB000",
          contrastText: "#000000"
        },
        background: {
          default: "#FFFFFF",
          paper: "#FFFFFF"
        },
        text: {
          primary: "#000000",
          secondary: "#000000"
        },
        divider: "#000000",
        error: {
          main: "#CC0000",
          contrastText: "#FFFFFF"
        },
        warning: {
          main: "#FF6600",
          contrastText: "#000000"
        },
        info: {
          main: "#0066CC",
          contrastText: "#FFFFFF"
        },
        success: {
          main: "#008000",
          contrastText: "#FFFFFF"
        }
      }
    },
    dark: {
      palette: {
        primary: {
          main: "#00FFFF",
          light: "#66FFFF",
          dark: "#00CCCC",
          contrastText: "#000000"
        },
        secondary: {
          main: "#FFFF00",
          light: "#FFFF66",
          dark: "#CCCC00",
          contrastText: "#000000"
        },
        background: {
          default: "#000000",
          paper: "#000000"
        },
        text: {
          primary: "#FFFFFF",
          secondary: "#FFFFFF"
        },
        divider: "#FFFFFF",
        error: {
          main: "#FF0000",
          contrastText: "#FFFFFF"
        },
        warning: {
          main: "#FF9900",
          contrastText: "#000000"
        },
        info: {
          main: "#00BFFF",
          contrastText: "#000000"
        },
        success: {
          main: "#00FF00",
          contrastText: "#000000"
        }
      }
    }
  },
  components: {
    MuiButton: {
      styleOverrides: {
        root: {
          borderRadius: 0,
          textTransform: "none",
          fontWeight: 700,
          border: "3px solid",
          "&:hover": {
            borderWidth: "3px"
          }
        },
        containedPrimary: ({ theme }) => ({
          backgroundColor: theme.vars.palette.primary.main,
          color: theme.vars.palette.primary.contrastText,
          borderColor: theme.vars.palette.primary.main,
          "&:hover": {
            backgroundColor: theme.vars.palette.primary.dark,
            borderColor: theme.vars.palette.primary.dark
          }
        }),
        contained: ({ theme }) => ({
          borderColor: theme.vars.palette.primary.main
        }),
        outlined: ({ theme }) => ({
          borderWidth: "3px",
          borderColor: theme.vars.palette.primary.main,
          "&:hover": {
            borderWidth: "3px",
            backgroundColor: theme.vars.palette.action.hover
          }
        })
      }
    },
    MuiCard: {
      styleOverrides: {
        root: {
          border: "3px solid",
          borderRadius: 0
        }
      }
    },
    MuiPaper: {
      styleOverrides: {
        root: ({ theme }) => ({
          border: `3px solid ${theme.vars.palette.divider}`,
          borderRadius: 0
        }),
        outlined: {
          borderWidth: "3px"
        }
      }
    },
    MuiTextField: {
      styleOverrides: {
        root: {
          "& .MuiOutlinedInput-root": {
            "& fieldset": {
              borderWidth: "3px"
            },
            "&:hover fieldset": {
              borderWidth: "3px"
            },
            "&.Mui-focused fieldset": {
              borderWidth: "3px"
            }
          }
        }
      }
    },
    MuiLink: {
      styleOverrides: {
        root: {
          textDecoration: "underline",
          textDecorationThickness: "2px",
          textUnderlineOffset: "4px",
          fontWeight: 700
        }
      }
    }
  }
});
var HighContrastTheme = HighContrastThemeBase;
Object.assign(HighContrastTheme, {
  gradient: {
    primary: "none"
  }
});
if (HighContrastTheme.vars) {
  Object.assign(HighContrastTheme.vars, {
    gradient: {
      primary: "none"
    }
  });
}
var HighContrastTheme_default = HighContrastTheme;

// src/styles/Themes/PaleBaseTheme.ts
import { extendTheme as extendTheme8 } from "@mui/material/styles";
var PaleBaseTheme = extendTheme8(OxygenThemeBase_default, {
  shape: {
    borderRadius: 8
  },
  components: {
    MuiAvatar: {
      styleOverrides: {
        root: ({ theme }) => ({
          backgroundColor: theme.vars.palette.secondary.main,
          color: theme.vars.palette.text.primary
        })
      }
    },
    MuiButton: {
      styleOverrides: {
        root: {
          borderRadius: 4,
          textTransform: "none",
          fontWeight: 400
        },
        containedPrimary: ({ theme }) => ({
          "&:not(:disabled)": {
            // Flat design with subtle gradient
            background: `${theme.gradient.primary} !important`,
            color: `${theme.vars.palette.primary.contrastText} !important`,
            "&:hover": {
              background: `${theme.gradient.primary} !important`,
              opacity: 0.9,
              color: `${theme.vars.palette.primary.contrastText} !important`
            }
          }
        }),
        outlinedPrimary: ({ theme }) => ({
          // Flat design - solid borders and colors
          borderColor: theme.vars.palette.primary.main,
          color: theme.vars.palette.primary.main,
          "&:hover": {
            borderColor: theme.vars.palette.primary.dark,
            backgroundColor: theme.vars.palette.action.hover,
            color: theme.vars.palette.primary.dark
          }
        }),
        containedSecondary: ({ theme }) => ({
          // Flat design - solid colors
          backgroundColor: theme.vars.palette.secondary.main,
          color: theme.vars.palette.secondary.contrastText,
          "&:hover": {
            backgroundColor: theme.vars.palette.secondary.dark,
            color: theme.vars.palette.secondary.contrastText
          }
        }),
        outlinedSecondary: ({ theme }) => ({
          borderColor: theme.vars.palette.divider,
          color: theme.vars.palette.text.primary,
          "&:hover": {
            borderColor: theme.vars.palette.text.secondary,
            backgroundColor: theme.palette.action.hover
          }
        })
      }
    },
    MuiFab: {
      styleOverrides: {
        primary: ({ theme }) => ({
          // Flat design with subtle gradient
          background: theme.gradient.primary,
          color: theme.vars.palette.primary.contrastText,
          "&:hover": {
            background: theme.gradient.primary,
            opacity: 0.9,
            color: theme.vars.palette.primary.contrastText
          }
        })
      }
    },
    MuiDialog: {
      styleOverrides: {
        paper: ({ theme }) => ({
          // Flat design - no blur, solid background
          background: theme.vars.palette.background.paper
        })
      }
    },
    MuiDrawer: {
      styleOverrides: {
        paper: ({ theme }) => ({
          // Paper background is already set by colorSchemes
          background: theme.vars.palette.background.paper
        })
      }
    },
    MuiAppBar: {
      styleOverrides: {
        root: ({ theme }) => ({
          // Flat design - solid background, no blur
          backgroundColor: `${theme.vars.palette.background.paper} !important`,
          borderRadius: 0
        })
      },
      defaultProps: {
        color: "transparent"
      }
    },
    MuiFormLabel: {
      styleOverrides: {
        root: ({ theme }) => ({
          fontSize: theme.typography.body2.fontSize,
          marginBottom: "2px"
        })
      }
    },
    MuiPaper: {
      styleOverrides: {
        root: ({ theme }) => ({
          borderRadius: theme.shape.borderRadius,
          // Flat design - solid background, no blur, minimal shadow
          border: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`,
          backgroundColor: theme.vars.palette.background.paper,
          boxShadow: "none"
        })
      }
    },
    MuiPopover: {
      styleOverrides: {
        paper: ({ theme }) => ({
          // Flat design - solid background
          background: theme.vars.palette.background.paper
        })
      }
    },
    MuiCard: {
      styleOverrides: {
        root: ({ theme }) => ({
          // Flat design - solid background
          backgroundColor: theme.vars.palette.background.paper
        })
      }
    },
    MuiTextField: {
      defaultProps: { size: "small" }
    },
    MuiOutlinedInput: {
      styleOverrides: {
        root: ({ theme }) => ({
          fontSize: theme.typography.body2.fontSize,
          borderRadius: 8,
          // Flat design - solid background
          backgroundColor: theme.vars.palette.background.paper
        }),
        notchedOutline: ({ theme }) => ({
          borderWidth: 1,
          borderColor: theme.vars.palette.divider
        })
      }
    },
    MuiMenuItem: {
      styleOverrides: {
        root: ({ theme }) => ({
          fontSize: theme.typography.body2.fontSize
        })
      }
    },
    MuiLink: {
      styleOverrides: {
        root: ({ theme }) => ({
          // Flat design - solid link colors
          color: theme.vars.palette.primary.main,
          textDecoration: "none",
          "&:hover": {
            color: theme.vars.palette.primary.dark,
            textDecoration: "underline"
          }
        })
      }
    },
    MuiListItemButton: {
      styleOverrides: {
        root: ({ theme }) => ({
          borderRadius: 8,
          "&.Mui-selected": {
            // Flat design - solid selected background
            backgroundColor: theme.vars.palette.action.selected,
            "&:hover": {
              backgroundColor: theme.vars.palette.action.hover
            }
          }
        })
      }
    },
    MuiTableRow: {
      styleOverrides: {
        root: ({ theme }) => ({
          "&.Mui-selected": {
            backgroundColor: theme.vars.palette.action.selected,
            "&:hover": {
              backgroundColor: theme.vars.palette.action.hover
            }
          }
        })
      }
    },
    MuiDataGrid: {
      styleOverrides: {
        root: ({ theme }) => ({
          border: "none",
          backgroundColor: "transparent",
          overflow: "hidden",
          "& .MuiDataGrid-columnHeaders": {
            // Flat design - solid background, no blur
            backgroundColor: theme.vars.palette.action.hover,
            borderBottom: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`,
            position: "sticky",
            top: 0,
            zIndex: 10
          },
          "& .MuiDataGrid-virtualScroller": {
            marginTop: "0 !important",
            overflow: "auto"
          }
        }),
        columnHeaders: ({ theme }) => ({
          // Flat design - solid background, no blur
          backgroundColor: theme.vars.palette.action.hover,
          borderBottom: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`
        }),
        columnHeader: {
          backgroundColor: "transparent"
        },
        cell: ({ theme }) => ({
          borderBottom: `${theme.border.width} ${theme.border.style} ${theme.vars.palette.divider}`,
          backgroundColor: "transparent"
        })
      }
    }
  }
});
var lowColorBaseGradient = {
  gradient: {
    primary: "linear-gradient(90deg, var(--oxygen-palette-primary-main) 0%, var(--oxygen-palette-primary-dark) 100%)"
  }
};
Object.assign(PaleBaseTheme, lowColorBaseGradient);
if (PaleBaseTheme.vars) {
  Object.assign(PaleBaseTheme.vars, lowColorBaseGradient);
}
var PaleBaseTheme_default = PaleBaseTheme;

// src/styles/Themes/PaleGray.ts
import { extendTheme as extendTheme9 } from "@mui/material/styles";
var PaleGrayTheme = extendTheme9(PaleBaseTheme_default, {
  colorSchemes: {
    light: {
      palette: {
        primary: {
          main: "#212121",
          mainChannel: "33 33 33",
          light: "#424242",
          dark: "#000000",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#757575",
          mainChannel: "117 117 117",
          light: "#9E9E9E",
          dark: "#616161",
          contrastText: "#ffffff"
        },
        background: {
          default: "#F5F5F5",
          paper: "#FFFFFF",
          acrylic: "#FFFFFF"
        },
        text: {
          primary: "#212121",
          secondary: "#757575"
        },
        divider: "#E0E0E0",
        action: {
          selected: "rgba(33, 33, 33, 0.12)",
          selectedOpacity: 0.12,
          hover: "rgba(33, 33, 33, 0.08)",
          hoverOpacity: 0.08
        },
        error: {
          main: "#D32F2F",
          light: "#EF5350",
          dark: "#C62828",
          contrastText: "#ffffff"
        },
        warning: {
          main: "#ED6C02",
          light: "#FF9800",
          dark: "#E65100",
          contrastText: "#ffffff"
        },
        info: {
          main: "#616161",
          light: "#757575",
          dark: "#424242",
          contrastText: "#ffffff"
        },
        success: {
          main: "#2E7D32",
          light: "#4CAF50",
          dark: "#1B5E20",
          contrastText: "#ffffff"
        }
      }
    },
    dark: {
      palette: {
        primary: {
          main: "#E0E0E0",
          mainChannel: "224 224 224",
          light: "#F5F5F5",
          dark: "#BDBDBD",
          contrastText: "#000000"
        },
        secondary: {
          main: "#9E9E9E",
          mainChannel: "158 158 158",
          light: "#BDBDBD",
          dark: "#757575",
          contrastText: "#000000"
        },
        background: {
          default: "#121212",
          paper: "#1E1E1E",
          acrylic: "#1E1E1E"
        },
        text: {
          primary: "#FFFFFF",
          secondary: "#B0B0B0"
        },
        divider: "#424242",
        action: {
          selected: "rgba(224, 224, 224, 0.16)",
          selectedOpacity: 0.16,
          hover: "rgba(224, 224, 224, 0.12)",
          hoverOpacity: 0.12
        },
        error: {
          main: "#EF5350",
          light: "#E57373",
          dark: "#E53935",
          contrastText: "#ffffff"
        },
        warning: {
          main: "#FFA726",
          light: "#FFB74D",
          dark: "#FB8C00",
          contrastText: "#000000"
        },
        info: {
          main: "#9E9E9E",
          light: "#BDBDBD",
          dark: "#757575",
          contrastText: "#000000"
        },
        success: {
          main: "#66BB6A",
          light: "#81C784",
          dark: "#43A047",
          contrastText: "#000000"
        }
      }
    }
  }
});
var PaleGray_default = PaleGrayTheme;

// src/styles/Themes/PaleIndigo.ts
import { extendTheme as extendTheme10 } from "@mui/material/styles";
var PaleIndigoTheme = extendTheme10(PaleBaseTheme_default, {
  colorSchemes: {
    light: {
      palette: {
        primary: {
          main: "#5567d5",
          mainChannel: "85 103 213",
          light: "#7785dd",
          dark: "#3f4db5",
          contrastText: "#ffffff"
        },
        secondary: {
          main: "#757575",
          mainChannel: "117 117 117",
          light: "#9E9E9E",
          dark: "#616161",
          contrastText: "#ffffff"
        },
        background: {
          default: "#F5F5F5",
          paper: "#FFFFFF",
          acrylic: "#FFFFFF"
        },
        text: {
          primary: "#212121",
          secondary: "#757575"
        },
        divider: "#E0E0E0",
        action: {
          selected: "rgba(85, 103, 213, 0.12)",
          selectedOpacity: 0.12,
          hover: "rgba(85, 103, 213, 0.08)",
          hoverOpacity: 0.08
        },
        error: {
          main: "#D32F2F",
          light: "#EF5350",
          dark: "#C62828",
          contrastText: "#ffffff"
        },
        warning: {
          main: "#ED6C02",
          light: "#FF9800",
          dark: "#E65100",
          contrastText: "#ffffff"
        },
        info: {
          main: "#5567d5",
          light: "#7785dd",
          dark: "#3f4db5",
          contrastText: "#ffffff"
        },
        success: {
          main: "#2E7D32",
          light: "#4CAF50",
          dark: "#1B5E20",
          contrastText: "#ffffff"
        }
      }
    },
    dark: {
      palette: {
        primary: {
          main: "#7785dd",
          mainChannel: "119 133 221",
          light: "#99a5e5",
          dark: "#5567d5",
          contrastText: "#000000"
        },
        secondary: {
          main: "#9E9E9E",
          mainChannel: "158 158 158",
          light: "#BDBDBD",
          dark: "#757575",
          contrastText: "#000000"
        },
        background: {
          default: "#121212",
          paper: "#1E1E1E",
          acrylic: "#1E1E1E"
        },
        text: {
          primary: "#FFFFFF",
          secondary: "#B0B0B0"
        },
        divider: "#424242",
        action: {
          selected: "rgba(119, 133, 221, 0.16)",
          selectedOpacity: 0.16,
          hover: "rgba(119, 133, 221, 0.12)",
          hoverOpacity: 0.12
        },
        error: {
          main: "#EF5350",
          light: "#E57373",
          dark: "#E53935",
          contrastText: "#ffffff"
        },
        warning: {
          main: "#FFA726",
          light: "#FFB74D",
          dark: "#FB8C00",
          contrastText: "#000000"
        },
        info: {
          main: "#7785dd",
          light: "#99a5e5",
          dark: "#5567d5",
          contrastText: "#000000"
        },
        success: {
          main: "#66BB6A",
          light: "#81C784",
          dark: "#43A047",
          contrastText: "#000000"
        }
      }
    }
  }
});
var PaleIndigo_default = PaleIndigoTheme;

// src/contexts/OxygenUIThemeProvider/OxygenUIThemeProvider.tsx
import { ThemeProvider, StyledEngineProvider } from "@mui/material/styles";
import { CssBaseline } from "@mui/material";
import { createContext, useContext, useState, useEffect } from "react";
import { jsx, jsxs } from "react/jsx-runtime";
var THEME_STORAGE_KEY = "oxygen-theme";
var defaultThemes = [
  {
    key: "default",
    label: "Default",
    theme: AcrylicBaseTheme_default
  },
  {
    key: "acrylicOrange",
    label: "Acrylic Orange",
    theme: AcrylicOrangeTheme_default
  },
  {
    key: "acrylicPurple",
    label: "Acrylic Purple",
    theme: AcrylicPurpleTheme_default
  }
];
var ThemeSwitcherContext = createContext(null);
var useThemeSwitcher = () => {
  const context = useContext(ThemeSwitcherContext);
  if (!context) {
    throw new Error("useThemeSwitcher must be used within OxygenUIThemeProvider with themes prop");
  }
  return context;
};
function OxygenUIThemeProvider({
  children,
  theme,
  themes,
  initialTheme
}) {
  var _a, _b;
  const useThemeSwitching = !!themes;
  const themeOptions = themes || defaultThemes;
  const [currentThemeKey, setCurrentThemeKey] = useState(() => {
    var _a2;
    if (typeof window !== "undefined" && useThemeSwitching) {
      try {
        const storedTheme = localStorage.getItem(THEME_STORAGE_KEY);
        if (storedTheme && themeOptions.some((t) => t.key === storedTheme)) {
          return storedTheme;
        }
      } catch (error) {
        console.warn("Failed to read theme from localStorage:", error);
      }
    }
    return initialTheme || ((_a2 = themeOptions[0]) == null ? void 0 : _a2.key) || "default";
  });
  useEffect(() => {
    if (typeof window !== "undefined" && useThemeSwitching) {
      try {
        localStorage.setItem(THEME_STORAGE_KEY, currentThemeKey);
      } catch (error) {
        console.warn("Failed to save theme to localStorage:", error);
      }
    }
  }, [currentThemeKey, useThemeSwitching]);
  let resolvedTheme;
  if (useThemeSwitching) {
    resolvedTheme = ((_a = themeOptions.find((t) => t.key === currentThemeKey)) == null ? void 0 : _a.theme) || ((_b = themeOptions[0]) == null ? void 0 : _b.theme);
  } else {
    resolvedTheme = theme || AcrylicBaseTheme_default;
  }
  const setTheme = (themeKey) => {
    if (themeOptions.some((t) => t.key === themeKey)) {
      setCurrentThemeKey(themeKey);
    }
  };
  const isActive = (themeKey) => currentThemeKey === themeKey;
  const contextValue = {
    theme: resolvedTheme,
    currentTheme: currentThemeKey,
    themes: themeOptions,
    setTheme,
    isActive
  };
  const content = /* @__PURE__ */ jsx(StyledEngineProvider, { injectFirst: true, children: /* @__PURE__ */ jsxs(ThemeProvider, { theme: resolvedTheme, children: [
    /* @__PURE__ */ jsx(CssBaseline, { enableColorScheme: true }),
    children
  ] }) });
  if (useThemeSwitching) {
    return /* @__PURE__ */ jsx(ThemeSwitcherContext.Provider, { value: contextValue, children: content });
  }
  return content;
}

// src/components/CodeBlock/CodeBlock.tsx
import React2, { useEffect as useEffect2 } from "react";
import { Box, useTheme } from "@mui/material";
import Prism from "prismjs";
import "prismjs/components/prism-javascript";
import "prismjs/components/prism-typescript";
import "prismjs/components/prism-jsx";
import "prismjs/components/prism-tsx";
import "prismjs/components/prism-css";
import "prismjs/components/prism-bash";
import "prismjs/components/prism-json";
import "prismjs/components/prism-markup";
import { jsx as jsx2 } from "react/jsx-runtime";
var CodeBlock = ({
  code,
  language = "tsx",
  showLineNumbers = false
}) => {
  var _a, _b, _c, _d, _e, _f;
  const theme = useTheme();
  const [isDark, setIsDark] = React2.useState(false);
  React2.useEffect(() => {
    const checkColorScheme = () => {
      const htmlElement = document.documentElement;
      const colorScheme = htmlElement.getAttribute("data-color-scheme");
      setIsDark(colorScheme === "dark" || theme.palette.mode === "dark");
    };
    checkColorScheme();
    const observer = new MutationObserver(checkColorScheme);
    observer.observe(document.documentElement, {
      attributes: true,
      attributeFilter: ["data-color-scheme"]
    });
    return () => observer.disconnect();
  }, [theme.palette.mode]);
  useEffect2(() => {
    Prism.highlightAll();
  }, [code, language]);
  const defaultColors = {
    background: isDark ? "#1e1e1e" : "#f5f5f5",
    text: isDark ? "#d4d4d4" : "#24292e",
    comment: isDark ? "#6a9955" : "#6a737d",
    keyword: isDark ? "#569cd6" : "#d73a49",
    string: isDark ? "#ce9178" : "#032f62",
    function: isDark ? "#dcdcaa" : "#6f42c1",
    number: isDark ? "#b5cea8" : "#005cc5",
    operator: isDark ? "#d4d4d4" : "#d73a49"
  };
  const colors2 = (isDark ? (_b = (_a = theme.vars) == null ? void 0 : _a.syntax) == null ? void 0 : _b.dark : (_d = (_c = theme.vars) == null ? void 0 : _c.syntax) == null ? void 0 : _d.light) || defaultColors;
  return /* @__PURE__ */ jsx2(
    Box,
    {
      sx: __spreadValues({
        position: "relative",
        borderRadius: 1,
        overflow: "hidden",
        border: `1px solid ${((_e = theme.vars) == null ? void 0 : _e.palette.divider) || theme.palette.divider}`,
        mb: 2,
        "& pre": {
          margin: 0,
          padding: 2,
          overflow: "auto",
          backgroundColor: colors2.background,
          color: colors2.text,
          fontFamily: "'Consolas', 'Monaco', 'Courier New', monospace",
          fontSize: "0.875rem",
          lineHeight: 1.6
        },
        "& code": {
          fontFamily: "inherit",
          fontSize: "inherit",
          color: colors2.text
        },
        // Prism.js syntax highlighting colors
        "& .token.comment, & .token.prolog, & .token.doctype, & .token.cdata": {
          color: colors2.comment
        },
        "& .token.punctuation": {
          color: colors2.text
        },
        "& .token.property, & .token.tag, & .token.boolean, & .token.constant, & .token.symbol, & .token.deleted": {
          color: colors2.keyword
        },
        "& .token.selector, & .token.attr-name, & .token.string, & .token.char, & .token.builtin, & .token.inserted": {
          color: colors2.string
        },
        "& .token.operator, & .token.entity, & .token.url": {
          color: colors2.operator
        },
        "& .token.atrule, & .token.attr-value, & .token.keyword": {
          color: colors2.keyword
        },
        "& .token.function, & .token.class-name": {
          color: colors2.function
        },
        "& .token.regex, & .token.important, & .token.variable": {
          color: colors2.keyword
        },
        "& .token.number": {
          color: colors2.number
        }
      }, showLineNumbers && {
        "& .line-numbers-rows": {
          borderRight: `1px solid ${((_f = theme.vars) == null ? void 0 : _f.palette.divider) || theme.palette.divider}`
        }
      }),
      children: /* @__PURE__ */ jsx2(
        Box,
        {
          component: "pre",
          className: showLineNumbers ? "line-numbers" : "",
          children: /* @__PURE__ */ jsx2("code", { className: `language-${language}`, children: code })
        }
      )
    }
  );
};
CodeBlock.displayName = "CodeBlock";
var CodeBlock_default = CodeBlock;

// src/components/ColorSchemeImage/ColorSchemeImage.tsx
import Box2 from "@mui/material/Box";
import { useColorScheme } from "@mui/material/styles";
import useMediaQuery from "@mui/material/useMediaQuery";
import { jsx as jsx3 } from "react/jsx-runtime";
function ColorSchemeImage(_a) {
  var _b = _a, { src, alt, width = 16, height = 16 } = _b, rest = __objRest(_b, ["src", "alt", "width", "height"]);
  const { mode } = useColorScheme();
  const prefersDark = useMediaQuery("(prefers-color-scheme: dark)");
  let actualMode;
  if (mode === "system") {
    actualMode = prefersDark ? "dark" : "light";
  } else {
    actualMode = mode;
  }
  const imageSrc = actualMode === "dark" ? src.dark : src.light;
  let imageAlt;
  if (typeof alt === "string") {
    imageAlt = alt;
  } else {
    imageAlt = actualMode === "dark" ? alt.dark : alt.light;
  }
  return /* @__PURE__ */ jsx3(Box2, __spreadValues({ component: "img", src: imageSrc, alt: imageAlt, width, height }, rest));
}

// src/components/ColorSchemeToggle/ColorSchemeToggle.tsx
import { useColorScheme as useColorScheme2 } from "@mui/material/styles";
import IconButton from "@mui/material/IconButton";
import { Moon, Monitor, Sun } from "@wso2/oxygen-ui-icons-react";
import { Tooltip } from "@mui/material";
import { jsx as jsx4 } from "react/jsx-runtime";
var ColorSchemeToggle = (_a) => {
  var _b = _a, {
    darkModeIcon,
    lightModeIcon,
    systemModeIcon
  } = _b, buttonProps = __objRest(_b, [
    "darkModeIcon",
    "lightModeIcon",
    "systemModeIcon"
  ]);
  const { mode, setMode } = useColorScheme2();
  if (!mode) {
    return null;
  }
  const nextMode = (currentMode2) => {
    if (currentMode2 === "light") return "dark";
    if (currentMode2 === "dark") return "system";
    return "light";
  };
  const currentMode = mode != null ? mode : "system";
  const ColorSchemeIcon = () => {
    switch (currentMode) {
      case "light":
        return lightModeIcon != null ? lightModeIcon : /* @__PURE__ */ jsx4(Sun, {});
      case "dark":
        return darkModeIcon != null ? darkModeIcon : /* @__PURE__ */ jsx4(Moon, {});
      default:
        return systemModeIcon != null ? systemModeIcon : /* @__PURE__ */ jsx4(Monitor, {});
    }
  };
  return /* @__PURE__ */ jsx4(Tooltip, { title: `${currentMode.charAt(0).toUpperCase()}${currentMode.slice(1)} Mode`, children: /* @__PURE__ */ jsx4(IconButton, __spreadProps(__spreadValues({}, buttonProps), { onClick: () => setMode(nextMode(currentMode)), children: /* @__PURE__ */ jsx4(ColorSchemeIcon, {}) })) });
};
var ColorSchemeToggle_default = ColorSchemeToggle;

// src/components/ComplexSelect/ComplexSelect.tsx
import { Children, isValidElement } from "react";
import MuiSelect, { selectClasses } from "@mui/material/Select";
import MuiFormControl from "@mui/material/FormControl";
import MuiInputLabel from "@mui/material/InputLabel";
import MuiTypography from "@mui/material/Typography";
import { styled as styled6 } from "@mui/material/styles";

// src/components/ComplexSelect/ComplexSelectListHeader.tsx
import MuiListSubheader from "@mui/material/ListSubheader";
import { styled } from "@mui/material/styles";
import { jsx as jsx5 } from "react/jsx-runtime";
var StyledListSubheader = styled(MuiListSubheader, {
  name: "MuiComplexSelectListHeader",
  slot: "Root"
})(() => ({
  paddingTop: 0
}));
function ComplexSelectListHeader(_a) {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx5(StyledListSubheader, __spreadProps(__spreadValues({}, props), { children }));
}
ComplexSelectListHeader.displayName = "ComplexSelect.ListHeader";
var ComplexSelectListHeader_default = ComplexSelectListHeader;

// src/components/ComplexSelect/ComplexSelectMenuItem.tsx
import MuiMenuItem from "@mui/material/MenuItem";
import { styled as styled4 } from "@mui/material/styles";

// src/components/ComplexSelect/ComplexSelectMenuItemAvatar.tsx
import { styled as styled2 } from "@mui/material/styles";
import MuiListItemAvatar from "@mui/material/ListItemAvatar";
import MuiAvatar from "@mui/material/Avatar";
import { jsx as jsx6 } from "react/jsx-runtime";
var StyledAvatar = styled2(MuiAvatar, {
  name: "MuiComplexSelectMenuItemAvatar",
  slot: "Avatar"
})(({ theme }) => ({
  width: 28,
  height: 28,
  fontSize: "1rem",
  backgroundColor: (theme.vars || theme).palette.background.paper,
  color: (theme.vars || theme).palette.text.secondary,
  border: `1px solid ${(theme.vars || theme).palette.divider}`
}));
var StyledListItemAvatar = styled2(MuiListItemAvatar, {
  name: "MuiComplexSelectMenuItemAvatar",
  slot: "Root"
})({
  minWidth: 0,
  marginRight: 12
});
function ComplexSelectMenuItemAvatar(_a) {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx6(StyledListItemAvatar, { children: /* @__PURE__ */ jsx6(StyledAvatar, __spreadProps(__spreadValues({}, props), { children })) });
}
ComplexSelectMenuItemAvatar.displayName = "ComplexSelect.MenuItemAvatar";
var ComplexSelectMenuItemAvatar_default = ComplexSelectMenuItemAvatar;

// src/components/ComplexSelect/ComplexSelectMenuItemText.tsx
import MuiListItemText from "@mui/material/ListItemText";
import { jsx as jsx7 } from "react/jsx-runtime";
function ComplexSelectMenuItemText(_a) {
  var _b = _a, { primary, secondary } = _b, props = __objRest(_b, ["primary", "secondary"]);
  return /* @__PURE__ */ jsx7(
    MuiListItemText,
    __spreadValues({
      primary,
      secondary,
      slotProps: { secondary: { variant: "caption" } }
    }, props)
  );
}
ComplexSelectMenuItemText.displayName = "ComplexSelect.MenuItemText";
var ComplexSelectMenuItemText_default = ComplexSelectMenuItemText;

// src/components/ComplexSelect/ComplexSelectMenuItemIcon.tsx
import MuiListItemIcon from "@mui/material/ListItemIcon";
import { styled as styled3 } from "@mui/material/styles";
import { jsx as jsx8 } from "react/jsx-runtime";
var StyledListItemIcon = styled3(MuiListItemIcon, {
  name: "MuiComplexSelectMenuItemIcon",
  slot: "Root"
})(() => ({
  minWidth: "36px"
}));
function ComplexSelectMenuItemIcon(_a) {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx8(StyledListItemIcon, __spreadProps(__spreadValues({}, props), { children }));
}
ComplexSelectMenuItemIcon.displayName = "ComplexSelect.MenuItemIcon";
var ComplexSelectMenuItemIcon_default = ComplexSelectMenuItemIcon;

// src/components/ComplexSelect/ComplexSelectMenuItem.tsx
import { jsx as jsx9 } from "react/jsx-runtime";
var StyledMenuItem = styled4(MuiMenuItem, {
  name: "MuiComplexSelectMenuItem",
  slot: "Root"
})(({ theme }) => ({
  "&:hover": {
    backgroundColor: theme.vars ? `rgba(${theme.vars.palette.primary.mainChannel} / 0.08)` : theme.palette.action.hover
  },
  "&.Mui-selected": {
    backgroundColor: theme.vars ? `rgba(${theme.vars.palette.primary.mainChannel} / 0.12)` : theme.palette.action.selected,
    "&:hover": {
      backgroundColor: theme.vars ? `rgba(${theme.vars.palette.primary.mainChannel} / 0.16)` : theme.palette.action.selected
    }
  }
}));
var ComplexSelectMenuItem = Object.assign(
  (_a) => {
    var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
    return /* @__PURE__ */ jsx9(StyledMenuItem, __spreadProps(__spreadValues({}, props), { children }));
  },
  {
    Avatar: ComplexSelectMenuItemAvatar_default,
    Text: ComplexSelectMenuItemText_default,
    Icon: ComplexSelectMenuItemIcon_default
  }
);
StyledMenuItem.displayName = "ComplexSelect.MenuItem";
var ComplexSelectMenuItem_default = ComplexSelectMenuItem;

// src/components/ComplexSelect/ComplexSelectDivider.tsx
import MuiDivider from "@mui/material/Divider";
import { styled as styled5 } from "@mui/material/styles";
import { jsx as jsx10 } from "react/jsx-runtime";
var StyledDivider = styled5(MuiDivider, {
  name: "MuiComplexSelectDivider",
  slot: "Root"
})(({ theme }) => ({
  marginLeft: theme.spacing(-1),
  marginRight: theme.spacing(-1)
}));
function ComplexSelectDivider(_a) {
  var props = __objRest(_a, []);
  return /* @__PURE__ */ jsx10(StyledDivider, __spreadValues({}, props));
}
ComplexSelectDivider.displayName = "ComplexSelect.Divider";
var ComplexSelectDivider_default = ComplexSelectDivider;

// src/components/ComplexSelect/ComplexSelect.tsx
import { Fragment, jsx as jsx11, jsxs as jsxs2 } from "react/jsx-runtime";
var StyledSelect = styled6(MuiSelect, {
  name: "MuiComplexSelect",
  slot: "Root",
  shouldForwardProp: (prop) => prop !== "labelAnchor"
})(({ theme }) => ({
  "&.MuiList-root": {
    padding: theme.spacing(1)
  },
  [`&.MuiInputBase-root .${selectClasses.select}`]: {
    display: "flex",
    alignItems: "center",
    paddingTop: "5px",
    paddingBottom: "5px",
    paddingRight: "42px"
  },
  [`& .${selectClasses.icon}`]: {
    marginLeft: theme.spacing(1)
  }
}));
var InsideLabelText = styled6(MuiTypography, {
  name: "MuiComplexSelect",
  slot: "InsideLabel"
})(({ theme }) => {
  var _a;
  return {
    fontSize: theme.typography.caption.fontSize,
    color: ((_a = theme.vars) == null ? void 0 : _a.palette.text.secondary) || theme.palette.text.secondary,
    opacity: 0.7,
    display: "block",
    lineHeight: 1.2,
    marginBottom: theme.spacing(0.125)
  };
});
var StyledFormControl = styled6(MuiFormControl, {
  name: "MuiComplexSelect",
  slot: "FormControl"
})(() => ({
  minWidth: 120
}));
var StyledInputLabel = styled6(MuiInputLabel, {
  name: "MuiComplexSelect",
  slot: "InputLabel",
  shouldForwardProp: (prop) => prop !== "labelAnchor"
})(({ theme, labelAnchor = "inside" }) => {
  var _a, _b;
  return __spreadValues({}, labelAnchor === "outside" && {
    position: "relative",
    transform: "none",
    marginBottom: theme.spacing(1),
    color: ((_a = theme.vars) == null ? void 0 : _a.palette.text.primary) || theme.palette.text.primary,
    fontSize: theme.typography.body2.fontSize,
    fontWeight: theme.typography.fontWeightMedium,
    "&.Mui-focused": {
      color: ((_b = theme.vars) == null ? void 0 : _b.palette.text.primary) || theme.palette.text.primary
    }
  });
});
function ComplexSelect(_a) {
  var _b = _a, { children, sx, label, labelId, labelAnchor = "inside" } = _b, props = __objRest(_b, ["children", "sx", "label", "labelId", "labelAnchor"]);
  const injectLabelIntoContent = (content) => {
    if (!label || labelAnchor !== "inside") return content;
    const processChildren = (node) => {
      if (!isValidElement(node)) return node;
      if (node.type && typeof node.type === "function") {
        const nodeProps2 = node.props;
        if (nodeProps2.primary) {
          return __spreadProps(__spreadValues({}, node), {
            props: __spreadProps(__spreadValues({}, nodeProps2), {
              primary: /* @__PURE__ */ jsxs2(Fragment, { children: [
                /* @__PURE__ */ jsx11(InsideLabelText, { children: label }),
                nodeProps2.primary
              ] }),
              // Remove secondary text when label is inside
              secondary: void 0
            })
          });
        }
      }
      const nodeProps = node.props;
      if (nodeProps.children) {
        return __spreadProps(__spreadValues({}, node), {
          props: __spreadProps(__spreadValues({}, nodeProps), {
            children: Children.map(nodeProps.children, processChildren)
          })
        });
      }
      return node;
    };
    return Children.map(content, processChildren);
  };
  const findMenuItemContent = (selected) => {
    let content = selected;
    const findInChildren = (childNodes) => {
      Children.forEach(childNodes, (child) => {
        if (isValidElement(child)) {
          const childProps = child.props;
          if (childProps.value === selected) {
            content = childProps.children;
          } else if (childProps.children) {
            findInChildren(childProps.children);
          }
        }
      });
    };
    findInChildren(children);
    return injectLabelIntoContent(content);
  };
  const renderInsideValue = labelAnchor === "inside" && label ? (selected) => findMenuItemContent(selected) : props.renderValue;
  const defaultMenuProps = __spreadValues({
    anchorOrigin: {
      vertical: "bottom",
      horizontal: "left"
    },
    transformOrigin: {
      vertical: "top",
      horizontal: "left"
    }
  }, props.MenuProps);
  const selectContent = /* @__PURE__ */ jsx11(
    StyledSelect,
    __spreadProps(__spreadValues({}, props), {
      label,
      labelId,
      sx,
      renderValue: renderInsideValue,
      labelAnchor,
      MenuProps: defaultMenuProps,
      children
    })
  );
  if (label) {
    const generatedLabelId = labelId || `complex-select-label-${Math.random().toString(36).substr(2, 9)}`;
    return /* @__PURE__ */ jsxs2(StyledFormControl, { fullWidth: props.fullWidth, children: [
      labelAnchor !== "inside" && /* @__PURE__ */ jsx11(
        StyledInputLabel,
        {
          id: generatedLabelId,
          labelAnchor,
          children: label
        }
      ),
      /* @__PURE__ */ jsx11(
        StyledSelect,
        __spreadProps(__spreadValues({}, props), {
          label: labelAnchor === "border" ? label : void 0,
          labelId: generatedLabelId,
          labelAnchor,
          renderValue: labelAnchor === "inside" ? renderInsideValue : props.renderValue,
          MenuProps: defaultMenuProps,
          sx,
          children
        })
      )
    ] });
  }
  return selectContent;
}
var ComplexSelectCompound = Object.assign(ComplexSelect, {
  ListHeader: ComplexSelectListHeader_default,
  MenuItem: ComplexSelectMenuItem_default,
  Divider: ComplexSelectDivider_default
});
var ComplexSelect_default = ComplexSelectCompound;

// src/components/PageContent/PageContent.tsx
import { styled as styled7 } from "@mui/material/styles";
import { Box as Box3 } from "@mui/material";
import { jsx as jsx12 } from "react/jsx-runtime";
var PageContentRoot = styled7(Box3, {
  name: "MuiPageContent",
  slot: "Root"
})(() => ({
  width: "100%",
  overflow: "auto",
  height: "100%",
  display: "flex",
  flexDirection: "column"
}));
var PageContentInner = styled7(Box3, {
  name: "MuiPageContent",
  slot: "Inner",
  shouldForwardProp: (prop) => prop !== "maxWidth" && prop !== "centered" && prop !== "fullWidth" && prop !== "noPadding"
})(({ theme, maxWidth = "1400px", centered = true, fullWidth = false, noPadding = false }) => __spreadValues(__spreadValues(__spreadValues({
  width: "100%"
}, !fullWidth && { maxWidth }), centered && { marginLeft: "auto", marginRight: "auto" }), !noPadding && { padding: theme.spacing(8) }));
var PageContent = (_a) => {
  var _b = _a, {
    children,
    maxWidth,
    centered,
    fullWidth,
    noPadding
  } = _b, props = __objRest(_b, [
    "children",
    "maxWidth",
    "centered",
    "fullWidth",
    "noPadding"
  ]);
  return /* @__PURE__ */ jsx12(PageContentRoot, { children: /* @__PURE__ */ jsx12(
    PageContentInner,
    __spreadProps(__spreadValues({
      maxWidth,
      centered,
      fullWidth,
      noPadding
    }, props), {
      children
    })
  ) });
};
PageContent.displayName = "PageContent";
var PageContent_default = PageContent;

// src/components/PageTitle/PageTitle.tsx
import React4 from "react";
import { styled as styled14 } from "@mui/material/styles";

// src/components/PageTitle/PageTitleHeader.tsx
import { styled as styled8 } from "@mui/material/styles";
import Typography from "@mui/material/Typography";
import { jsx as jsx13 } from "react/jsx-runtime";
var PageTitleHeaderStyled = styled8(Typography, {
  name: "MuiPageTitle",
  slot: "Header"
})(({ theme }) => {
  var _a;
  return {
    fontWeight: 500,
    fontSize: "1.5rem",
    lineHeight: 1.334,
    letterSpacing: "0em",
    color: ((_a = theme.vars) == null ? void 0 : _a.palette.text.primary) || theme.palette.text.primary
  };
});
var PageTitleHeader = (_a) => {
  var _b = _a, { children, variant = "h4" } = _b, props = __objRest(_b, ["children", "variant"]);
  return /* @__PURE__ */ jsx13(PageTitleHeaderStyled, __spreadProps(__spreadValues({ variant }, props), { children }));
};
PageTitleHeader.displayName = "PageTitle.Header";
var PageTitleHeader_default = PageTitleHeader;

// src/components/PageTitle/PageTitleSubHeader.tsx
import { styled as styled9 } from "@mui/material/styles";
import Typography2 from "@mui/material/Typography";
import { jsx as jsx14 } from "react/jsx-runtime";
var PageTitleSubHeaderStyled = styled9(Typography2, {
  name: "MuiPageTitle",
  slot: "SubHeader"
})(({ theme }) => {
  var _a;
  return {
    fontSize: "0.875rem",
    lineHeight: 1.43,
    letterSpacing: "0.01071em",
    color: ((_a = theme.vars) == null ? void 0 : _a.palette.text.secondary) || theme.palette.text.secondary
  };
});
var PageTitleSubHeader = (_a) => {
  var _b = _a, { children, variant = "body2" } = _b, props = __objRest(_b, ["children", "variant"]);
  return /* @__PURE__ */ jsx14(PageTitleSubHeaderStyled, __spreadProps(__spreadValues({ variant }, props), { children }));
};
PageTitleSubHeader.displayName = "PageTitle.SubHeader";
var PageTitleSubHeader_default = PageTitleSubHeader;

// src/components/PageTitle/PageTitleAvatar.tsx
import { styled as styled10 } from "@mui/material/styles";
import Avatar from "@mui/material/Avatar";
import { jsx as jsx15 } from "react/jsx-runtime";
var PageTitleAvatarStyled = styled10(Avatar, {
  name: "MuiPageTitle",
  slot: "Avatar"
})(({ theme }) => ({
  width: 55,
  height: 55,
  marginRight: theme.spacing(2)
}));
var PageTitleAvatar = (_a) => {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx15(PageTitleAvatarStyled, __spreadProps(__spreadValues({}, props), { children }));
};
PageTitleAvatar.displayName = "PageTitle.Avatar";
var PageTitleAvatar_default = PageTitleAvatar;

// src/components/PageTitle/PageTitleLink.tsx
import { styled as styled11 } from "@mui/material/styles";
import Link from "@mui/material/Link";
import Box4 from "@mui/material/Box";
import { jsx as jsx16, jsxs as jsxs3 } from "react/jsx-runtime";
var PageTitleLinkStyled = styled11(Link, {
  name: "MuiPageTitle",
  slot: "Link"
})(({ theme }) => ({
  fontSize: "0.875rem",
  lineHeight: 1.43,
  display: "inline-flex",
  alignItems: "center",
  gap: theme.spacing(0.5),
  textDecoration: "none",
  color: theme.palette.primary.main,
  "&:hover": {
    textDecoration: "underline"
  }
}));
var PageTitleLink = (_a) => {
  var _b = _a, { children, icon } = _b, props = __objRest(_b, ["children", "icon"]);
  return /* @__PURE__ */ jsxs3(PageTitleLinkStyled, __spreadProps(__spreadValues({}, props), { children: [
    icon && /* @__PURE__ */ jsx16(Box4, { component: "span", sx: { display: "inline-flex", alignItems: "center", fontSize: "1rem" }, children: icon }),
    children
  ] }));
};
PageTitleLink.displayName = "PageTitle.Link";
var PageTitleLink_default = PageTitleLink;

// src/components/PageTitle/PageTitleActions.tsx
import { styled as styled12 } from "@mui/material/styles";
import { jsx as jsx17 } from "react/jsx-runtime";
var PageTitleActionsRoot = styled12("div", {
  name: "MuiPageTitle",
  slot: "Actions"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(2),
  marginLeft: "auto"
}));
var PageTitleActions = (_a) => {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx17(PageTitleActionsRoot, __spreadProps(__spreadValues({}, props), { children }));
};
PageTitleActions.displayName = "PageTitle.Actions";
var PageTitleActions_default = PageTitleActions;

// src/components/PageTitle/PageTitleBackButton.tsx
import React3 from "react";
import { styled as styled13 } from "@mui/material/styles";
import Button from "@mui/material/Button";
import Typography3 from "@mui/material/Typography";
import { ArrowLeft } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx18, jsxs as jsxs4 } from "react/jsx-runtime";
var PageTitleBackButtonRoot = styled13("div", {
  name: "MuiPageTitle",
  slot: "BackButton"
})();
var PageTitleBackButton = (_a) => {
  var _b = _a, {
    children = "Back",
    component: Component
  } = _b, props = __objRest(_b, [
    "children",
    "component"
  ]);
  const button = /* @__PURE__ */ jsxs4(
    Button,
    __spreadProps(__spreadValues({
      variant: "text",
      sx: {
        display: "flex",
        alignItems: "center",
        marginLeft: "-10px",
        textDecoration: "none"
      }
    }, props), {
      children: [
        /* @__PURE__ */ jsx18(ArrowLeft, { size: 20 }),
        /* @__PURE__ */ jsx18(Typography3, { variant: "body2", children })
      ]
    })
  );
  if (React3.isValidElement(Component)) {
    return /* @__PURE__ */ jsx18(PageTitleBackButtonRoot, { children: React3.cloneElement(
      Component,
      { style: { textDecoration: "none" } },
      button
    ) });
  }
  if (Component) {
    const WrapperComponent = Component;
    return /* @__PURE__ */ jsx18(PageTitleBackButtonRoot, { children: /* @__PURE__ */ jsx18(WrapperComponent, { style: { textDecoration: "none" }, children: button }) });
  }
  return /* @__PURE__ */ jsx18(PageTitleBackButtonRoot, { children: button });
};
PageTitleBackButton.displayName = "PageTitle.BackButton";
var PageTitleBackButton_default = PageTitleBackButton;

// src/components/PageTitle/PageTitle.tsx
import { jsx as jsx19, jsxs as jsxs5 } from "react/jsx-runtime";
var PageTitleRoot = styled14("div", {
  name: "MuiPageTitle",
  slot: "Root"
})(({ theme }) => ({
  marginBottom: theme.spacing(5),
  display: "flex",
  flexDirection: "column",
  gap: theme.spacing(2),
  width: "100%"
}));
var PageTitleRow1 = styled14("div", {
  name: "MuiPageTitle",
  slot: "Row1"
})();
var PageTitleRow2 = styled14("div", {
  name: "MuiPageTitle",
  slot: "Row2"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(2),
  width: "100%"
}));
var PageTitleColumnLeft = styled14("div", {
  name: "MuiPageTitle",
  slot: "ColumnLeft"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  flex: 1,
  '&:has(> [class*="Column1"] > *)': {
    gap: theme.spacing(2)
  }
}));
var PageTitleColumn1 = styled14("div", {
  name: "MuiPageTitle",
  slot: "Column1"
})();
var PageTitleColumn2 = styled14("div", {
  name: "MuiPageTitle",
  slot: "Column2"
})(({ theme }) => ({
  display: "flex",
  flexDirection: "column",
  gap: theme.spacing(0.5),
  flex: 1
}));
var PageTitleColumnRight = styled14("div", {
  name: "MuiPageTitle",
  slot: "ColumnRight"
})();
var PageTitle = (_a) => {
  var _b = _a, { children, sx } = _b, props = __objRest(_b, ["children", "sx"]);
  const childrenArray = React4.Children.toArray(children);
  const avatar = childrenArray.find(
    (child) => React4.isValidElement(child) && child.type === PageTitleAvatar_default
  );
  const actions = childrenArray.find(
    (child) => React4.isValidElement(child) && child.type === PageTitleActions_default
  );
  let backButtonElement = null;
  childrenArray.forEach((child) => {
    if (React4.isValidElement(child)) {
      if (child.type === PageTitleBackButton_default) {
        backButtonElement = child;
      }
      if (child.props && typeof child.props === "object" && "children" in child.props && React4.isValidElement(child.props.children)) {
        if (child.props.children.type === PageTitleBackButton_default) {
          backButtonElement = child;
        }
      }
    }
  });
  const otherChildren = childrenArray.filter(
    (child) => {
      if (!React4.isValidElement(child)) {
        return true;
      }
      if (child.type === PageTitleAvatar_default || child.type === PageTitleActions_default || child.type === PageTitleBackButton_default) {
        return false;
      }
      if (child.props && typeof child.props === "object" && "children" in child.props && React4.isValidElement(child.props.children)) {
        const childrenProp = child.props.children;
        if (React4.isValidElement(childrenProp) && childrenProp.type === PageTitleBackButton_default) {
          return false;
        }
      }
      return true;
    }
  );
  return /* @__PURE__ */ jsxs5(PageTitleRoot, __spreadProps(__spreadValues({ sx }, props), { children: [
    backButtonElement && /* @__PURE__ */ jsx19(PageTitleRow1, { children: backButtonElement }),
    /* @__PURE__ */ jsxs5(PageTitleRow2, { children: [
      /* @__PURE__ */ jsxs5(PageTitleColumnLeft, { children: [
        /* @__PURE__ */ jsx19(PageTitleColumn1, { children: avatar }),
        /* @__PURE__ */ jsx19(PageTitleColumn2, { children: otherChildren })
      ] }),
      /* @__PURE__ */ jsx19(PageTitleColumnRight, { children: actions })
    ] })
  ] }));
};
PageTitle.Avatar = PageTitleAvatar_default;
PageTitle.Header = PageTitleHeader_default;
PageTitle.SubHeader = PageTitleSubHeader_default;
PageTitle.Link = PageTitleLink_default;
PageTitle.Actions = PageTitleActions_default;
PageTitle.BackButton = PageTitleBackButton_default;
PageTitle.Link = PageTitleLink_default;
PageTitle.displayName = "PageTitle";
var PageTitle_default = PageTitle;

// src/components/StatCard/StatCard.tsx
import { Card, CardContent, Box as Box5, Typography as Typography4 } from "@mui/material";
import { jsx as jsx20, jsxs as jsxs6 } from "react/jsx-runtime";
var StatCard = (_a) => {
  var _b = _a, {
    value,
    label,
    icon,
    iconColor = "primary"
  } = _b, props = __objRest(_b, [
    "value",
    "label",
    "icon",
    "iconColor"
  ]);
  return /* @__PURE__ */ jsx20(Card, __spreadProps(__spreadValues({ variant: "outlined" }, props), { children: /* @__PURE__ */ jsx20(CardContent, { children: /* @__PURE__ */ jsxs6(Box5, { sx: { display: "flex", alignItems: "center", gap: 2 }, children: [
    icon && /* @__PURE__ */ jsx20(Box5, { sx: { color: (theme) => theme.palette[iconColor].main, display: "flex", alignItems: "center" }, children: icon }),
    /* @__PURE__ */ jsxs6(Box5, { children: [
      /* @__PURE__ */ jsx20(Typography4, { variant: "h5", children: value }),
      /* @__PURE__ */ jsx20(Typography4, { variant: "body2", color: "text.secondary", children: label })
    ] })
  ] }) }) }));
};
StatCard.displayName = "StatCard";
var StatCard_default = StatCard;

// src/components/ThemeSwitcher/ThemeSelect.tsx
import { Select, MenuItem, FormControl, InputLabel } from "@mui/material";
import { jsx as jsx21, jsxs as jsxs7 } from "react/jsx-runtime";
function ThemeSelect({
  size = "small",
  showLabel = false,
  label = "Theme",
  variant = "outlined",
  sx
}) {
  const { currentTheme, themes, setTheme } = useThemeSwitcher();
  const handleChange = (event) => {
    setTheme(event.target.value);
  };
  return /* @__PURE__ */ jsxs7(FormControl, { size, variant, sx: __spreadValues({ minWidth: 120 }, sx), children: [
    showLabel && /* @__PURE__ */ jsx21(InputLabel, { id: "theme-select-label", children: label }),
    /* @__PURE__ */ jsx21(
      Select,
      {
        labelId: showLabel ? "theme-select-label" : void 0,
        value: currentTheme,
        onChange: handleChange,
        label: showLabel ? label : void 0,
        children: themes.map((themeOption) => /* @__PURE__ */ jsx21(MenuItem, { value: themeOption.key, children: themeOption.label }, themeOption.key))
      }
    )
  ] });
}

// src/components/ThemeSwitcher/ThemeSwitcher.tsx
import { Fragment as Fragment2, jsx as jsx22 } from "react/jsx-runtime";
function ThemeSwitcher({
  children,
  size,
  showLabel,
  label,
  variant,
  sx
}) {
  const themeSwitcherContext = useThemeSwitcher();
  if (children) {
    return /* @__PURE__ */ jsx22(Fragment2, { children: children(themeSwitcherContext) });
  }
  return /* @__PURE__ */ jsx22(
    ThemeSelect,
    {
      size,
      showLabel,
      label,
      variant,
      sx
    }
  );
}

// src/components/SearchBar/SearchBarWithAdvancedFilter.tsx
import * as React5 from "react";
import TextField from "@mui/material/TextField";
import InputAdornment from "@mui/material/InputAdornment";
import Box6 from "@mui/material/Box";
import IconButton2 from "@mui/material/IconButton";
import Popover from "@mui/material/Popover";
import Typography5 from "@mui/material/Typography";
import Divider from "@mui/material/Divider";
import Button2 from "@mui/material/Button";
import Stack from "@mui/material/Stack";
import FormControl2 from "@mui/material/FormControl";
import InputLabel2 from "@mui/material/InputLabel";
import Select2 from "@mui/material/Select";
import MenuItem2 from "@mui/material/MenuItem";
import { Search } from "@wso2/oxygen-ui-icons-react";
import { ListFilter } from "@wso2/oxygen-ui-icons-react";
import { Fragment as Fragment3, jsx as jsx23, jsxs as jsxs8 } from "react/jsx-runtime";
function SearchBarWithAdvancedFilter(_a) {
  var _b = _a, {
    value,
    onChange,
    advancedFilter,
    onAdvancedFilterChange,
    attributeOptions,
    conditionOptions,
    onAdvancedSearch,
    maxHeight = 56,
    placeholder = "Search by name",
    popoverWidth = 460,
    sx
  } = _b, props = __objRest(_b, [
    "value",
    "onChange",
    "advancedFilter",
    "onAdvancedFilterChange",
    "attributeOptions",
    "conditionOptions",
    "onAdvancedSearch",
    "maxHeight",
    "placeholder",
    "popoverWidth",
    "sx"
  ]);
  const [anchorEl, setAnchorEl] = React5.useState(null);
  const open = Boolean(anchorEl);
  const handleInputChange = (e) => {
    onChange(e.target.value);
  };
  const handleOpen = (e) => setAnchorEl(e.currentTarget);
  const handleClose = () => setAnchorEl(null);
  const handleAttrChange = (e) => {
    onAdvancedFilterChange(__spreadProps(__spreadValues({}, advancedFilter), { attribute: e.target.value }));
  };
  const handleCondChange = (e, child) => {
    onAdvancedFilterChange(__spreadProps(__spreadValues({}, advancedFilter), { condition: e.target.value }));
  };
  const handleFilterValueChange = (e) => {
    onAdvancedFilterChange(__spreadProps(__spreadValues({}, advancedFilter), { value: e.target.value }));
  };
  const handleAdvancedSearch = () => {
    onAdvancedSearch == null ? void 0 : onAdvancedSearch({
      searchText: value,
      attribute: advancedFilter.attribute,
      condition: advancedFilter.condition,
      value: advancedFilter.value
    });
    handleClose();
  };
  const popoverId = open ? "advanced-search-popover" : void 0;
  return /* @__PURE__ */ jsxs8(Fragment3, { children: [
    /* @__PURE__ */ jsx23(
      TextField,
      __spreadProps(__spreadValues({}, props), {
        value,
        onChange: handleInputChange,
        placeholder,
        variant: "outlined",
        sx: __spreadValues({
          maxHeight,
          "& .MuiInputBase-root": {
            maxHeight,
            pr: 0
          }
        }, sx),
        InputProps: {
          startAdornment: /* @__PURE__ */ jsx23(InputAdornment, { position: "start", children: /* @__PURE__ */ jsx23(Search, { fontSize: "small" }) }),
          endAdornment: /* @__PURE__ */ jsx23(InputAdornment, { position: "end", sx: { m: 0 }, children: /* @__PURE__ */ jsxs8(Box6, { sx: { display: "flex", alignItems: "center", height: "100%" }, children: [
            /* @__PURE__ */ jsx23(Divider, { orientation: "vertical", flexItem: true, sx: { mx: 0.5 } }),
            /* @__PURE__ */ jsx23(
              IconButton2,
              {
                "aria-describedby": popoverId,
                "aria-label": "Open advanced search",
                onClick: handleOpen,
                size: "small",
                sx: { mx: 0.5 },
                children: /* @__PURE__ */ jsx23(ListFilter, { fontSize: "tiny" })
              }
            )
          ] }) })
        }
      })
    ),
    /* @__PURE__ */ jsxs8(
      Popover,
      {
        id: popoverId,
        open,
        anchorEl,
        onClose: handleClose,
        anchorOrigin: { vertical: "bottom", horizontal: "right" },
        transformOrigin: { vertical: "top", horizontal: "right" },
        PaperProps: {
          sx: {
            width: popoverWidth,
            borderRadius: 1,
            p: 2
          }
        },
        children: [
          /* @__PURE__ */ jsx23(Typography5, { variant: "subtitle2", sx: { fontWeight: 600 }, children: "Advanced search" }),
          /* @__PURE__ */ jsx23(Divider, { sx: { my: 1 } }),
          /* @__PURE__ */ jsxs8(Stack, { spacing: 2, marginTop: 2, children: [
            /* @__PURE__ */ jsxs8(FormControl2, { fullWidth: true, size: "small", children: [
              /* @__PURE__ */ jsxs8(InputLabel2, { id: "adv-filter-attribute-label", children: [
                "Filter attribute",
                " ",
                /* @__PURE__ */ jsx23(Box6, { component: "span", sx: { color: "error.main" }, children: "*" })
              ] }),
              /* @__PURE__ */ jsx23(
                Select2,
                {
                  labelId: "adv-filter-attribute-label",
                  value: advancedFilter.attribute,
                  label: "Filter attribute *",
                  onChange: handleAttrChange,
                  children: attributeOptions.map((opt) => /* @__PURE__ */ jsx23(MenuItem2, { value: opt.value, children: opt.label }, opt.value))
                }
              )
            ] }),
            /* @__PURE__ */ jsxs8(Box6, { sx: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }, children: [
              /* @__PURE__ */ jsxs8(FormControl2, { fullWidth: true, size: "small", children: [
                /* @__PURE__ */ jsxs8(InputLabel2, { id: "adv-filter-condition-label", children: [
                  "Filter condition",
                  " ",
                  /* @__PURE__ */ jsx23(Box6, { component: "span", sx: { color: "error.main" }, children: "*" })
                ] }),
                /* @__PURE__ */ jsx23(
                  Select2,
                  {
                    labelId: "adv-filter-condition-label",
                    value: advancedFilter.condition,
                    label: "Filter condition *",
                    onChange: handleCondChange,
                    children: conditionOptions.map((opt) => /* @__PURE__ */ jsx23(MenuItem2, { value: opt.value, children: opt.label }, opt.value))
                  }
                )
              ] }),
              /* @__PURE__ */ jsx23(
                TextField,
                {
                  size: "small",
                  required: true,
                  label: "Filter value",
                  placeholder: "Enter value to search",
                  value: advancedFilter.value,
                  onChange: handleFilterValueChange
                }
              )
            ] }),
            /* @__PURE__ */ jsx23(Box6, { sx: { display: "flex", justifyContent: "flex-start" }, children: /* @__PURE__ */ jsx23(
              Button2,
              {
                variant: "contained",
                onClick: handleAdvancedSearch,
                disabled: !advancedFilter.attribute || !advancedFilter.condition || !advancedFilter.value,
                sx: { borderRadius: 999, px: 3 },
                children: "Search"
              }
            ) })
          ] })
        ]
      }
    )
  ] });
}
var SearchBarWithAdvancedFilter_default = SearchBarWithAdvancedFilter;

// src/components/SearchBar/SearchBar.tsx
import TextField2 from "@mui/material/TextField";
import InputAdornment2 from "@mui/material/InputAdornment";
import { Search as Search2 } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx24 } from "react/jsx-runtime";
function SearchBar(_a) {
  var _b = _a, {
    placeholder = "Search"
  } = _b, props = __objRest(_b, [
    "placeholder"
  ]);
  return /* @__PURE__ */ jsx24(
    TextField2,
    __spreadProps(__spreadValues({}, props), {
      placeholder,
      variant: "outlined",
      slotProps: {
        input: {
          startAdornment: /* @__PURE__ */ jsx24(InputAdornment2, { position: "start", children: /* @__PURE__ */ jsx24(Search2, { fontSize: "small" }) })
        }
      }
    })
  );
}
var SearchBar_default = SearchBar;

// src/components/Header/Header.tsx
import * as React7 from "react";
import AppBar from "@mui/material/AppBar";
import Toolbar from "@mui/material/Toolbar";
import Box11 from "@mui/material/Box";
import { styled as styled21 } from "@mui/material/styles";

// src/components/Header/context.ts
import * as React6 from "react";
var HeaderContext = React6.createContext(null);
var useHeader = () => {
  const context = React6.useContext(HeaderContext);
  if (!context) {
    throw new Error("Header compound components must be used within Header");
  }
  return context;
};

// src/components/Header/HeaderToggle.tsx
import IconButton3 from "@mui/material/IconButton";
import Tooltip2 from "@mui/material/Tooltip";
import { styled as styled15 } from "@mui/material/styles";
import { Menu } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx25 } from "react/jsx-runtime";
var HeaderToggleRoot = styled15(IconButton3, {
  name: "MuiHeader",
  slot: "Toggle"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.secondary
}));
var HeaderToggle = ({
  collapsed,
  onToggle,
  expandLabel = "Expand sidebar",
  collapseLabel = "Collapse sidebar",
  expandIcon,
  collapseIcon,
  sx
}) => {
  return /* @__PURE__ */ jsx25(Tooltip2, { title: collapsed ? expandLabel : collapseLabel, children: /* @__PURE__ */ jsx25(HeaderToggleRoot, { onClick: onToggle, size: "small", sx, children: collapsed ? expandIcon || /* @__PURE__ */ jsx25(Menu, { size: 20 }) : collapseIcon || /* @__PURE__ */ jsx25(Menu, { size: 20 }) }) });
};

// src/components/Header/HeaderBrand.tsx
import Box8 from "@mui/material/Box";
import { styled as styled18 } from "@mui/material/styles";

// src/components/Header/HeaderBrandLogo.tsx
import Box7 from "@mui/material/Box";
import { styled as styled16 } from "@mui/material/styles";
import { jsx as jsx26 } from "react/jsx-runtime";
var HeaderBrandLogoRoot = styled16(Box7, {
  name: "MuiHeader",
  slot: "BrandLogo"
})({
  display: "flex",
  alignItems: "center"
});
var HeaderBrandLogo = ({
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx26(HeaderBrandLogoRoot, { sx, children });
};
HeaderBrandLogo.displayName = "HeaderBrandLogo";

// src/components/Header/HeaderBrandTitle.tsx
import Typography6 from "@mui/material/Typography";
import { styled as styled17 } from "@mui/material/styles";
import { jsx as jsx27 } from "react/jsx-runtime";
var HeaderBrandTitleRoot = styled17(Typography6, {
  name: "MuiHeader",
  slot: "BrandTitle"
})(({ theme }) => ({
  fontWeight: 600,
  fontSize: 16,
  marginLeft: theme.spacing(1),
  display: "none",
  [theme.breakpoints.up("sm")]: {
    display: "block",
    fontSize: 18
  }
}));
var HeaderBrandTitle = ({
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx27(HeaderBrandTitleRoot, { variant: "h6", component: "div", sx, children });
};
HeaderBrandTitle.displayName = "HeaderBrandTitle";

// src/components/Header/HeaderBrand.tsx
import { jsx as jsx28 } from "react/jsx-runtime";
var HeaderBrandRoot = styled18(Box8, {
  name: "MuiHeader",
  slot: "Brand",
  shouldForwardProp: (prop) => prop !== "clickable"
})(({ theme, clickable }) => ({
  display: "flex",
  alignItems: "center",
  marginLeft: theme.spacing(1),
  cursor: clickable ? "pointer" : "default"
}));
var HeaderBrand = ({
  onClick,
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx28(HeaderBrandRoot, { onClick, clickable: !!onClick, sx, children });
};
HeaderBrand.displayName = "HeaderBrand";

// src/components/Header/HeaderSwitchers.tsx
import Box9 from "@mui/material/Box";
import Divider2 from "@mui/material/Divider";
import { styled as styled19 } from "@mui/material/styles";
import { jsx as jsx29, jsxs as jsxs9 } from "react/jsx-runtime";
var HeaderSwitchersRoot = styled19(Box9, {
  name: "MuiHeader",
  slot: "Switchers"
})(({ theme }) => ({
  display: "none",
  alignItems: "center",
  gap: theme.spacing(1),
  marginLeft: theme.spacing(2),
  [theme.breakpoints.up("md")]: {
    display: "flex"
  }
}));
var HeaderSwitchersDivider = styled19(Divider2, {
  name: "MuiHeader",
  slot: "SwitchersDivider"
})(({ theme }) => ({
  marginLeft: theme.spacing(1),
  marginRight: theme.spacing(1)
}));
var HeaderSwitchers = ({
  children,
  showDivider = true,
  sx
}) => {
  const { minimal } = useHeader();
  if (minimal) {
    return null;
  }
  return /* @__PURE__ */ jsxs9(HeaderSwitchersRoot, { sx, children: [
    showDivider && /* @__PURE__ */ jsx29(HeaderSwitchersDivider, { orientation: "vertical", flexItem: true }),
    children
  ] });
};

// src/components/Header/HeaderActions.tsx
import Box10 from "@mui/material/Box";
import { styled as styled20 } from "@mui/material/styles";
import { jsx as jsx30 } from "react/jsx-runtime";
var HeaderActionsRoot = styled20(Box10, {
  name: "MuiHeader",
  slot: "Actions"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(0.5)
}));
var HeaderActions = ({
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx30(HeaderActionsRoot, { sx, children });
};

// src/components/Header/Header.tsx
import { jsx as jsx31 } from "react/jsx-runtime";
var HeaderRootStyled = styled21(AppBar, {
  name: "MuiHeader",
  slot: "Root"
})(({ theme }) => ({
  backgroundColor: (theme.vars || theme).palette.background.paper,
  borderBottom: `1px solid ${(theme.vars || theme).palette.divider}`
}));
var HeaderToolbar = styled21(Toolbar, {
  name: "MuiHeader",
  slot: "Toolbar"
})(({ theme }) => ({
  gap: theme.spacing(1),
  minHeight: 56,
  paddingLeft: theme.spacing(1),
  paddingRight: theme.spacing(1),
  [theme.breakpoints.up("sm")]: {
    minHeight: 64,
    paddingLeft: theme.spacing(2),
    paddingRight: theme.spacing(2)
  }
}));
var HeaderSpacerStyled = styled21(Box11, {
  name: "MuiHeader",
  slot: "Spacer"
})({
  flexGrow: 1
});
var Header = ({
  children,
  minimal = false,
  sx
}) => {
  const contextValue = React7.useMemo(() => ({ minimal }), [minimal]);
  return /* @__PURE__ */ jsx31(HeaderContext.Provider, { value: contextValue, children: /* @__PURE__ */ jsx31(HeaderRootStyled, { position: "static", color: "default", elevation: 0, sx, children: /* @__PURE__ */ jsx31(HeaderToolbar, { children }) }) });
};
var HeaderSpacer = ({ sx }) => /* @__PURE__ */ jsx31(HeaderSpacerStyled, { sx });
Header.Toggle = HeaderToggle;
Header.Brand = HeaderBrand;
Header.BrandLogo = HeaderBrandLogo;
Header.BrandTitle = HeaderBrandTitle;
Header.Switchers = HeaderSwitchers;
Header.Actions = HeaderActions;
Header.Spacer = HeaderSpacer;
Header.displayName = "Header";

// src/components/Sidebar/Sidebar.tsx
import * as React14 from "react";
import Box17 from "@mui/material/Box";
import { styled as styled34 } from "@mui/material/styles";

// src/components/Sidebar/context.ts
import * as React8 from "react";
var SidebarContext = React8.createContext(null);
var useSidebar = () => {
  const context = React8.useContext(SidebarContext);
  if (!context) {
    throw new Error("Sidebar compound components must be used within Sidebar");
  }
  return context;
};

// src/components/Sidebar/SidebarNav.tsx
import * as React9 from "react";
import Box12 from "@mui/material/Box";
import Divider3 from "@mui/material/Divider";
import { styled as styled22 } from "@mui/material/styles";
import { jsx as jsx32, jsxs as jsxs10 } from "react/jsx-runtime";
var SidebarNavRoot = styled22(Box12, {
  name: "MuiSidebar",
  slot: "Nav"
})(({ theme }) => ({
  flex: 1,
  overflowY: "auto",
  overflowX: "hidden",
  paddingTop: theme.spacing(1),
  paddingBottom: theme.spacing(1),
  "&::-webkit-scrollbar": {
    width: 6
  },
  "&::-webkit-scrollbar-thumb": {
    backgroundColor: (theme.vars || theme).palette.action.disabled,
    borderRadius: 3
  },
  "&::-webkit-scrollbar-thumb:hover": {
    backgroundColor: (theme.vars || theme).palette.action.active
  }
}));
var SidebarNavDivider = styled22(Divider3, {
  name: "MuiSidebar",
  slot: "NavDivider"
})(({ theme }) => ({
  marginTop: theme.spacing(1),
  marginBottom: theme.spacing(1),
  marginLeft: theme.spacing(2),
  marginRight: theme.spacing(2)
}));
var SidebarNav = ({
  children,
  showDividers = true,
  sx
}) => {
  const { collapsed } = useSidebar();
  const childArray = React9.Children.toArray(children);
  return /* @__PURE__ */ jsx32(SidebarNavRoot, { component: "nav", sx, children: childArray.map((child, index) => /* @__PURE__ */ jsxs10(React9.Fragment, { children: [
    index > 0 && showDividers && !collapsed && /* @__PURE__ */ jsx32(SidebarNavDivider, {}),
    child
  ] }, index)) });
};

// src/components/Sidebar/SidebarCategory.tsx
import * as React10 from "react";
import Box13 from "@mui/material/Box";
import List from "@mui/material/List";
import { styled as styled24 } from "@mui/material/styles";

// src/components/Sidebar/SidebarCategoryLabel.tsx
import Typography7 from "@mui/material/Typography";
import { styled as styled23 } from "@mui/material/styles";
import { jsx as jsx33 } from "react/jsx-runtime";
var SidebarCategoryLabelRoot = styled23(Typography7, {
  name: "MuiSidebar",
  slot: "CategoryLabel",
  shouldForwardProp: (prop) => prop !== "collapsed"
})(({ theme, collapsed }) => ({
  paddingLeft: theme.spacing(3),
  paddingRight: theme.spacing(3),
  paddingTop: collapsed ? 0 : theme.spacing(1),
  paddingBottom: collapsed ? 0 : theme.spacing(1),
  height: collapsed ? 0 : "auto",
  opacity: collapsed ? 0 : 1,
  overflow: "hidden",
  transition: theme.transitions.create(["padding", "height", "opacity"], {
    easing: theme.transitions.easing.sharp,
    duration: collapsed ? theme.transitions.duration.leavingScreen : theme.transitions.duration.enteringScreen
  }),
  display: "block",
  color: (theme.vars || theme).palette.text.secondary,
  fontWeight: 600,
  textTransform: "uppercase",
  letterSpacing: "0.5px",
  fontSize: 11
}));
var SidebarCategoryLabel = ({ children, sx }) => {
  const { collapsed } = useSidebar();
  return /* @__PURE__ */ jsx33(SidebarCategoryLabelRoot, { variant: "caption", collapsed, sx, children });
};
SidebarCategoryLabel.displayName = "SidebarCategoryLabel";

// src/components/Sidebar/SidebarCategory.tsx
import { jsx as jsx34, jsxs as jsxs11 } from "react/jsx-runtime";
var SidebarCategoryRoot = styled24(Box13, {
  name: "MuiSidebar",
  slot: "Category"
})(({ theme }) => ({
  marginBottom: theme.spacing(1)
}));
var separateChildren = (children) => {
  let labelChild = null;
  const otherChildren = [];
  React10.Children.forEach(children, (child) => {
    var _a;
    if (React10.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      if (displayName === "SidebarCategoryLabel") {
        labelChild = child;
      } else {
        otherChildren.push(child);
      }
    } else {
      otherChildren.push(child);
    }
  });
  return { labelChild, otherChildren };
};
var SidebarCategory = ({
  children,
  sx
}) => {
  const { labelChild, otherChildren } = separateChildren(children);
  return /* @__PURE__ */ jsxs11(SidebarCategoryRoot, { sx, children: [
    labelChild,
    /* @__PURE__ */ jsx34(List, { disablePadding: true, children: otherChildren })
  ] });
};
SidebarCategory.displayName = "SidebarCategory";

// src/components/Sidebar/SidebarItem.tsx
import * as React12 from "react";
import Box14 from "@mui/material/Box";
import List2 from "@mui/material/List";
import ListItem from "@mui/material/ListItem";
import ListItemButton from "@mui/material/ListItemButton";
import ListItemIcon2 from "@mui/material/ListItemIcon";
import ListItemText2 from "@mui/material/ListItemText";
import Collapse from "@mui/material/Collapse";
import Tooltip3 from "@mui/material/Tooltip";
import Popover2 from "@mui/material/Popover";
import { styled as styled28 } from "@mui/material/styles";
import { ChevronDown as ChevronDown2, ChevronUp } from "@wso2/oxygen-ui-icons-react";

// src/components/Sidebar/SidebarItemContext.tsx
import * as React11 from "react";
import { jsx as jsx35 } from "react/jsx-runtime";
var SidebarItemContext = React11.createContext(null);
var useSidebarItemContext = () => {
  const context = React11.useContext(SidebarItemContext);
  if (!context) {
    throw new Error(
      "useSidebarItemContext must be used within a SidebarItem component"
    );
  }
  return context;
};
var SidebarItemProvider = ({ value, children }) => {
  return /* @__PURE__ */ jsx35(SidebarItemContext.Provider, { value, children });
};

// src/components/Sidebar/SidebarItemIcon.tsx
import ListItemIcon from "@mui/material/ListItemIcon";
import { styled as styled25 } from "@mui/material/styles";
import { jsx as jsx36 } from "react/jsx-runtime";
var SidebarItemIconRoot = styled25(ListItemIcon, {
  name: "MuiSidebar",
  slot: "ItemIcon",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ theme, ownerState }) => ({
  minWidth: 0,
  marginRight: ownerState.collapsed ? 0 : theme.spacing(2),
  justifyContent: "center",
  color: ownerState.isActive ? (theme.vars || theme).palette.primary.main : "inherit"
}));
var SidebarItemIcon = ({
  children,
  sx
}) => {
  const { collapsed } = useSidebar();
  const { isActive } = useSidebarItemContext();
  return /* @__PURE__ */ jsx36(
    SidebarItemIconRoot,
    {
      ownerState: { collapsed, isActive },
      sx,
      children
    }
  );
};
SidebarItemIcon.displayName = "SidebarItemIcon";

// src/components/Sidebar/SidebarItemLabel.tsx
import ListItemText from "@mui/material/ListItemText";
import { styled as styled26 } from "@mui/material/styles";
import { jsx as jsx37 } from "react/jsx-runtime";
var SidebarItemLabelRoot = styled26(ListItemText, {
  name: "MuiSidebar",
  slot: "ItemLabel",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ theme, ownerState }) => ({
  margin: 0,
  opacity: ownerState.collapsed ? 0 : 1,
  width: ownerState.collapsed ? 0 : "auto",
  overflow: "hidden",
  whiteSpace: "nowrap",
  transition: theme.transitions.create(["opacity", "width", "margin"], {
    easing: theme.transitions.easing.sharp,
    duration: ownerState.collapsed ? theme.transitions.duration.leavingScreen : theme.transitions.duration.enteringScreen
  }),
  "& .MuiListItemText-primary": {
    fontSize: 14,
    fontWeight: ownerState.isActive ? 600 : 400
  }
}));
var SidebarItemLabel = ({
  children,
  sx
}) => {
  const { collapsed } = useSidebar();
  const { isActive } = useSidebarItemContext();
  return /* @__PURE__ */ jsx37(SidebarItemLabelRoot, { primary: children, ownerState: { isActive, collapsed }, sx });
};
SidebarItemLabel.displayName = "SidebarItemLabel";

// src/components/Sidebar/SidebarItemBadge.tsx
import Chip from "@mui/material/Chip";
import { styled as styled27 } from "@mui/material/styles";
import { jsx as jsx38 } from "react/jsx-runtime";
var SidebarItemBadgeRoot = styled27(Chip, {
  name: "MuiSidebar",
  slot: "ItemBadge"
})({
  height: 20,
  fontSize: 11,
  marginLeft: "auto"
});
var SidebarItemBadge = ({
  children,
  color = "error",
  sx
}) => {
  const { collapsed } = useSidebar();
  if (collapsed) {
    return null;
  }
  return /* @__PURE__ */ jsx38(SidebarItemBadgeRoot, { label: children, size: "small", color, sx });
};
SidebarItemBadge.displayName = "SidebarItemBadge";

// src/components/Sidebar/SidebarItem.tsx
import { Fragment as Fragment5, jsx as jsx39, jsxs as jsxs12 } from "react/jsx-runtime";
var SidebarItemRoot = styled28(ListItem, {
  name: "MuiSidebar",
  slot: "Item"
})({
  display: "block",
  padding: 0
});
var SidebarItemButton = styled28(ListItemButton, {
  name: "MuiSidebar",
  slot: "ItemButton",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ theme, ownerState }) => ({
  minHeight: 44,
  paddingRight: theme.spacing(2),
  paddingLeft: ownerState.collapsed ? theme.spacing(2) : theme.spacing(2 + ownerState.depth * 2),
  justifyContent: ownerState.collapsed ? "center" : "initial",
  borderRadius: theme.shape.borderRadius,
  marginLeft: theme.spacing(1),
  marginRight: theme.spacing(1),
  transition: theme.transitions.create(["padding-left", "padding-right"], {
    easing: theme.transitions.easing.sharp,
    duration: ownerState.collapsed ? theme.transitions.duration.leavingScreen : theme.transitions.duration.enteringScreen
  }),
  "&.Mui-selected": {
    backgroundColor: (theme.vars || theme).palette.action.selected,
    "&:hover": {
      backgroundColor: (theme.vars || theme).palette.action.selected
    }
  }
}));
var SidebarItemChevron = styled28(Box14, {
  name: "MuiSidebar",
  slot: "ItemChevron"
})({
  marginLeft: "auto",
  paddingLeft: 1,
  display: "flex",
  alignItems: "center"
});
var SidebarItemPopover = styled28(Popover2, {
  name: "MuiSidebar",
  slot: "ItemPopover"
})({
  pointerEvents: "none"
});
var SidebarItemPopoverList = styled28(List2, {
  name: "MuiSidebar",
  slot: "ItemPopoverList"
})(({ theme }) => ({
  padding: theme.spacing(0.5),
  pointerEvents: "auto"
}));
var SidebarItemPopoverButton = styled28(ListItemButton, {
  name: "MuiSidebar",
  slot: "ItemPopoverButton"
})(({ theme }) => ({
  minHeight: 40,
  paddingRight: theme.spacing(2),
  paddingLeft: theme.spacing(2),
  borderRadius: theme.shape.borderRadius,
  gap: theme.spacing(1.5),
  "&.Mui-selected": {
    backgroundColor: (theme.vars || theme).palette.action.selected,
    "&:hover": {
      backgroundColor: (theme.vars || theme).palette.action.selected
    }
  }
}));
var CHILD_DISPLAY_NAMES = [
  "SidebarItemIcon",
  "SidebarItemLabel",
  "SidebarItemBadge"
];
var separateChildren2 = (children) => {
  const composableChildren = [];
  const nestedItems = [];
  React12.Children.toArray(children).forEach((child) => {
    var _a;
    if (React12.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      if (displayName && CHILD_DISPLAY_NAMES.includes(displayName)) {
        composableChildren.push(child);
      } else {
        nestedItems.push(child);
      }
    } else {
      nestedItems.push(child);
    }
  });
  return { composableChildren, nestedItems };
};
var getTooltipLabel = (children) => {
  let tooltipLabel = "";
  React12.Children.forEach(children, (child) => {
    var _a;
    if (React12.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      const props = child.props;
      if (displayName === "SidebarItemLabel" && props.children) {
        tooltipLabel = String(props.children);
      }
    }
  });
  return tooltipLabel;
};
var extractItemContent = (itemChildren) => {
  let icon = null;
  let label = "";
  React12.Children.forEach(itemChildren, (child) => {
    var _a;
    if (React12.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      const props = child.props;
      if (displayName === "SidebarItemIcon") {
        icon = props.children;
      } else if (displayName === "SidebarItemLabel" && props.children) {
        label = String(props.children);
      }
    }
  });
  return { icon, label };
};
var SidebarItem = ({
  id,
  children,
  depth = 0,
  sx
}) => {
  const { collapsed, activeItem, expandedMenus, onSelect, onToggleExpand } = useSidebar();
  const { composableChildren, nestedItems } = separateChildren2(children);
  const hasNestedItems = nestedItems.length > 0;
  const isActive = activeItem === id;
  const isExpanded = expandedMenus[id] || false;
  const hasActiveChild = React12.useMemo(() => {
    if (!hasNestedItems || !activeItem) return false;
    return nestedItems.some((child) => {
      if (React12.isValidElement(child)) {
        return child.props.id === activeItem;
      }
      return false;
    });
  }, [hasNestedItems, nestedItems, activeItem]);
  const [popoverAnchor, setPopoverAnchor] = React12.useState(null);
  const popoverOpen = Boolean(popoverAnchor);
  const contextValue = {
    id,
    isActive,
    isExpanded,
    hasChildren: hasNestedItems,
    depth
  };
  const handleClick = () => {
    if (hasNestedItems) {
      onToggleExpand == null ? void 0 : onToggleExpand(id);
    } else {
      onSelect == null ? void 0 : onSelect(id);
    }
  };
  const handleMouseEnter = (event) => {
    if (collapsed && hasNestedItems) {
      setPopoverAnchor(event.currentTarget);
    }
  };
  const handleMouseLeave = () => {
    setPopoverAnchor(null);
  };
  const tooltipLabel = getTooltipLabel(children);
  const ownerState = { collapsed, depth };
  const shouldShowSelected = isActive && !hasNestedItems || collapsed && hasActiveChild;
  const buttonContent = /* @__PURE__ */ jsx39(
    SidebarItemButton,
    {
      selected: shouldShowSelected,
      onClick: handleClick,
      ownerState,
      sx,
      children: /* @__PURE__ */ jsxs12(SidebarItemProvider, { value: contextValue, children: [
        composableChildren,
        !collapsed && hasNestedItems && /* @__PURE__ */ jsx39(SidebarItemChevron, { children: isExpanded ? /* @__PURE__ */ jsx39(ChevronUp, { size: 16 }) : /* @__PURE__ */ jsx39(ChevronDown2, { size: 16 }) }),
        !collapsed && !hasNestedItems && /* @__PURE__ */ jsx39(Box14, { sx: { ml: "auto", width: 16, pl: 1 } })
      ] })
    }
  );
  const collapsedWithNested = collapsed && hasNestedItems;
  const handlePopoverItemClick = (itemId) => {
    onSelect == null ? void 0 : onSelect(itemId);
    setPopoverAnchor(null);
  };
  return /* @__PURE__ */ jsxs12(Fragment5, { children: [
    /* @__PURE__ */ jsx39(
      SidebarItemRoot,
      {
        disablePadding: true,
        onMouseEnter: handleMouseEnter,
        onMouseLeave: handleMouseLeave,
        children: collapsed ? (
          // When collapsed: show tooltip only for items without nested children
          collapsedWithNested ? buttonContent : /* @__PURE__ */ jsx39(Tooltip3, { title: tooltipLabel, placement: "right", arrow: true, children: buttonContent })
        ) : buttonContent
      }
    ),
    collapsedWithNested && /* @__PURE__ */ jsx39(
      SidebarItemPopover,
      {
        open: popoverOpen,
        anchorEl: popoverAnchor,
        anchorOrigin: { vertical: "center", horizontal: "right" },
        transformOrigin: { vertical: "center", horizontal: "left" },
        onClose: handleMouseLeave,
        disableRestoreFocus: true,
        slotProps: {
          paper: {
            onMouseEnter: () => setPopoverAnchor(popoverAnchor),
            onMouseLeave: handleMouseLeave
          }
        },
        children: /* @__PURE__ */ jsx39(SidebarItemPopoverList, { disablePadding: true, children: nestedItems.map((child, index) => {
          if (React12.isValidElement(child)) {
            const childProps = child.props;
            const { icon, label } = extractItemContent(childProps.children);
            const isItemActive = activeItem === childProps.id;
            return /* @__PURE__ */ jsxs12(
              SidebarItemPopoverButton,
              {
                selected: isItemActive,
                onClick: () => handlePopoverItemClick(childProps.id),
                children: [
                  icon && /* @__PURE__ */ jsx39(ListItemIcon2, { sx: { minWidth: "auto" }, children: icon }),
                  /* @__PURE__ */ jsx39(ListItemText2, { primary: label })
                ]
              },
              childProps.id || index
            );
          }
          return child;
        }) })
      }
    ),
    hasNestedItems && !collapsed && /* @__PURE__ */ jsx39(Collapse, { in: isExpanded, timeout: "auto", unmountOnExit: true, children: /* @__PURE__ */ jsx39(List2, { disablePadding: true, children: nestedItems.map((child, index) => {
      if (React12.isValidElement(child)) {
        return React12.cloneElement(child, {
          key: child.key || index,
          depth: depth + 1
        });
      }
      return child;
    }) }) })
  ] });
};
SidebarItem.displayName = "SidebarItem";

// src/components/Sidebar/SidebarFooter.tsx
import Box15 from "@mui/material/Box";
import Divider4 from "@mui/material/Divider";
import { styled as styled29 } from "@mui/material/styles";
import { Fragment as Fragment6, jsx as jsx40, jsxs as jsxs13 } from "react/jsx-runtime";
var SidebarFooterRoot = styled29(Box15, {
  name: "MuiSidebar",
  slot: "Footer"
})(({ theme }) => ({
  paddingTop: theme.spacing(1),
  paddingBottom: theme.spacing(1)
}));
var SidebarFooter = ({
  children,
  showDivider = true,
  sx
}) => {
  return /* @__PURE__ */ jsxs13(Fragment6, { children: [
    showDivider && /* @__PURE__ */ jsx40(Divider4, {}),
    /* @__PURE__ */ jsx40(SidebarFooterRoot, { sx, children })
  ] });
};

// src/components/Sidebar/SidebarUser.tsx
import * as React13 from "react";
import Box16 from "@mui/material/Box";
import ListItemButton2 from "@mui/material/ListItemButton";
import Tooltip4 from "@mui/material/Tooltip";
import Divider5 from "@mui/material/Divider";
import { styled as styled33 } from "@mui/material/styles";

// src/components/Sidebar/SidebarUserAvatar.tsx
import Avatar2 from "@mui/material/Avatar";
import { styled as styled30 } from "@mui/material/styles";
import { jsx as jsx41 } from "react/jsx-runtime";
var SidebarUserAvatarRoot = styled30(Avatar2, {
  name: "MuiSidebar",
  slot: "UserAvatar"
})(({ theme }) => ({
  width: 32,
  height: 32,
  backgroundColor: (theme.vars || theme).palette.primary.main,
  color: (theme.vars || theme).palette.primary.contrastText,
  fontSize: 14
}));
var SidebarUserAvatar = ({
  children,
  src,
  alt,
  sx
}) => {
  return /* @__PURE__ */ jsx41(SidebarUserAvatarRoot, { src, alt, sx, children });
};
SidebarUserAvatar.displayName = "SidebarUserAvatar";

// src/components/Sidebar/SidebarUserName.tsx
import Typography8 from "@mui/material/Typography";
import { styled as styled31 } from "@mui/material/styles";
import { jsx as jsx42 } from "react/jsx-runtime";
var SidebarUserNameRoot = styled31(Typography8, {
  name: "MuiSidebar",
  slot: "UserName"
})({
  fontWeight: 600,
  whiteSpace: "nowrap",
  overflow: "hidden",
  textOverflow: "ellipsis"
});
var SidebarUserName = ({
  children,
  sx
}) => {
  const { collapsed } = useSidebar();
  if (collapsed) {
    return null;
  }
  return /* @__PURE__ */ jsx42(SidebarUserNameRoot, { variant: "body2", sx, children });
};
SidebarUserName.displayName = "SidebarUserName";

// src/components/Sidebar/SidebarUserEmail.tsx
import Typography9 from "@mui/material/Typography";
import { styled as styled32 } from "@mui/material/styles";
import { jsx as jsx43 } from "react/jsx-runtime";
var SidebarUserEmailRoot = styled32(Typography9, {
  name: "MuiSidebar",
  slot: "UserEmail"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.secondary,
  whiteSpace: "nowrap",
  overflow: "hidden",
  textOverflow: "ellipsis",
  display: "block"
}));
var SidebarUserEmail = ({
  children,
  sx
}) => {
  const { collapsed } = useSidebar();
  if (collapsed) {
    return null;
  }
  return /* @__PURE__ */ jsx43(SidebarUserEmailRoot, { variant: "caption", sx, children });
};
SidebarUserEmail.displayName = "SidebarUserEmail";

// src/components/Sidebar/SidebarUser.tsx
import { Fragment as Fragment7, jsx as jsx44, jsxs as jsxs14 } from "react/jsx-runtime";
var SidebarUserRoot = styled33(Box16, {
  name: "MuiSidebar",
  slot: "User"
})(({ theme }) => ({
  padding: theme.spacing(2)
}));
var SidebarUserButton = styled33(ListItemButton2, {
  name: "MuiSidebar",
  slot: "UserButton",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ theme, ownerState }) => ({
  minHeight: 56,
  paddingLeft: theme.spacing(2),
  paddingRight: theme.spacing(2),
  justifyContent: ownerState.collapsed ? "center" : "initial",
  borderRadius: theme.shape.borderRadius,
  marginLeft: theme.spacing(1),
  marginRight: theme.spacing(1),
  transition: theme.transitions.create(["padding-left", "padding-right"], {
    easing: theme.transitions.easing.sharp,
    duration: ownerState.collapsed ? theme.transitions.duration.leavingScreen : theme.transitions.duration.enteringScreen
  })
}));
var SidebarUserTextContainer = styled33(Box16, {
  name: "MuiSidebar",
  slot: "UserTextContainer",
  shouldForwardProp: (prop) => prop !== "collapsed"
})(({ theme, collapsed }) => ({
  marginLeft: theme.spacing(2),
  overflow: "hidden",
  opacity: collapsed ? 0 : 1,
  width: collapsed ? 0 : "auto",
  whiteSpace: "nowrap",
  transition: theme.transitions.create(["opacity", "width", "margin"], {
    easing: theme.transitions.easing.sharp,
    duration: collapsed ? theme.transitions.duration.leavingScreen : theme.transitions.duration.enteringScreen
  })
}));
var separateChildren3 = (children) => {
  let avatarChild = null;
  const textChildren = [];
  React13.Children.forEach(children, (child) => {
    var _a;
    if (React13.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      if (displayName === "SidebarUserAvatar") {
        avatarChild = child;
      } else if (displayName === "SidebarUserName" || displayName === "SidebarUserEmail") {
        textChildren.push(child);
      }
    }
  });
  return { avatarChild, textChildren };
};
var getTooltipText = (children) => {
  let tooltipText = "";
  React13.Children.forEach(children, (child) => {
    var _a;
    if (React13.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      const props = child.props;
      if (displayName === "SidebarUserName" && props.children) {
        tooltipText = String(props.children);
      }
    }
  });
  return tooltipText;
};
var SidebarUser = ({
  children,
  onClick,
  showDivider = true,
  sx
}) => {
  const { collapsed } = useSidebar();
  const { avatarChild, textChildren } = separateChildren3(children);
  const tooltipText = getTooltipText(children);
  const ownerState = { collapsed };
  const content = /* @__PURE__ */ jsxs14(SidebarUserButton, { onClick, ownerState, children: [
    avatarChild,
    /* @__PURE__ */ jsx44(SidebarUserTextContainer, { collapsed, children: textChildren })
  ] });
  return /* @__PURE__ */ jsxs14(Fragment7, { children: [
    showDivider && /* @__PURE__ */ jsx44(Divider5, {}),
    /* @__PURE__ */ jsx44(SidebarUserRoot, { sx, children: collapsed ? /* @__PURE__ */ jsx44(Tooltip4, { title: tooltipText, placement: "right", arrow: true, children: content }) : content })
  ] });
};
SidebarUser.displayName = "SidebarUser";

// src/components/Sidebar/Sidebar.tsx
import { jsx as jsx45 } from "react/jsx-runtime";
var SIDEBAR_WIDTH = 250;
var COLLAPSED_SIDEBAR_WIDTH = 64;
var SidebarRootStyled = styled34(Box17, {
  name: "MuiSidebar",
  slot: "Root",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ theme, ownerState }) => ({
  width: ownerState.collapsed ? ownerState.collapsedWidth : ownerState.width,
  transition: theme.transitions.create("width", {
    easing: theme.transitions.easing.sharp,
    duration: ownerState.collapsed ? theme.transitions.duration.leavingScreen : theme.transitions.duration.enteringScreen
  }),
  display: "flex",
  flexDirection: "column",
  height: "100%",
  backgroundColor: (theme.vars || theme).palette.background.paper,
  borderRight: `1px solid ${(theme.vars || theme).palette.divider}`,
  overflow: "hidden",
  "& a": {
    textDecoration: "none",
    "&:hover": {
      textDecoration: "none"
    }
  }
}));
var Sidebar = ({
  collapsed = false,
  activeItem,
  expandedMenus = {},
  onSelect,
  onToggleExpand,
  children,
  width = SIDEBAR_WIDTH,
  collapsedWidth = COLLAPSED_SIDEBAR_WIDTH,
  sx
}) => {
  const contextValue = React14.useMemo(
    () => ({
      collapsed,
      activeItem,
      expandedMenus,
      onSelect,
      onToggleExpand
    }),
    [collapsed, activeItem, expandedMenus, onSelect, onToggleExpand]
  );
  const ownerState = { collapsed, width, collapsedWidth };
  return /* @__PURE__ */ jsx45(SidebarContext.Provider, { value: contextValue, children: /* @__PURE__ */ jsx45(SidebarRootStyled, { component: "aside", ownerState, sx, children }) });
};
Sidebar.Nav = SidebarNav;
Sidebar.Category = SidebarCategory;
Sidebar.CategoryLabel = SidebarCategoryLabel;
Sidebar.Item = SidebarItem;
Sidebar.ItemIcon = SidebarItemIcon;
Sidebar.ItemLabel = SidebarItemLabel;
Sidebar.ItemBadge = SidebarItemBadge;
Sidebar.Footer = SidebarFooter;
Sidebar.User = SidebarUser;
Sidebar.UserAvatar = SidebarUserAvatar;
Sidebar.UserName = SidebarUserName;
Sidebar.UserEmail = SidebarUserEmail;
Sidebar.displayName = "Sidebar";

// src/components/NotificationPanel/NotificationPanel.tsx
import * as React20 from "react";
import Drawer from "@mui/material/Drawer";
import { styled as styled50 } from "@mui/material/styles";

// src/components/NotificationPanel/context.ts
import * as React15 from "react";
var NotificationPanelContext = React15.createContext(null);
var useNotificationPanel = () => {
  const context = React15.useContext(NotificationPanelContext);
  if (!context) {
    throw new Error(
      "NotificationPanel compound components must be used within NotificationPanel"
    );
  }
  return context;
};

// src/components/NotificationPanel/NotificationHeader.tsx
import * as React16 from "react";
import Box18 from "@mui/material/Box";
import { styled as styled35 } from "@mui/material/styles";
import { jsx as jsx46, jsxs as jsxs15 } from "react/jsx-runtime";
var NotificationHeaderRoot = styled35(Box18, {
  name: "MuiNotificationPanel",
  slot: "Header"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  justifyContent: "space-between",
  padding: theme.spacing(2),
  borderBottom: `1px solid ${(theme.vars || theme).palette.divider}`
}));
var NotificationHeaderContent = styled35(Box18, {
  name: "MuiNotificationPanel",
  slot: "HeaderContent"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(1)
}));
var separateChildren4 = (children) => {
  const leftChildren = [];
  let closeChild = null;
  React16.Children.forEach(children, (child) => {
    var _a;
    if (React16.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      if (displayName === "NotificationHeaderClose") {
        closeChild = child;
      } else {
        leftChildren.push(child);
      }
    } else {
      leftChildren.push(child);
    }
  });
  return { leftChildren, closeChild };
};
var NotificationHeader = ({
  children,
  sx
}) => {
  const { leftChildren, closeChild } = separateChildren4(children);
  return /* @__PURE__ */ jsxs15(NotificationHeaderRoot, { sx, children: [
    /* @__PURE__ */ jsx46(NotificationHeaderContent, { children: leftChildren }),
    closeChild
  ] });
};
NotificationHeader.displayName = "NotificationHeader";

// src/components/NotificationPanel/NotificationHeaderIcon.tsx
import Box19 from "@mui/material/Box";
import { styled as styled36 } from "@mui/material/styles";
import { jsx as jsx47 } from "react/jsx-runtime";
var NotificationHeaderIconRoot = styled36(Box19, {
  name: "MuiNotificationPanel",
  slot: "HeaderIcon"
})({
  display: "flex",
  alignItems: "center"
});
var NotificationHeaderIcon = ({
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx47(NotificationHeaderIconRoot, { sx, children });
};
NotificationHeaderIcon.displayName = "NotificationHeaderIcon";

// src/components/NotificationPanel/NotificationHeaderTitle.tsx
import Typography10 from "@mui/material/Typography";
import { styled as styled37 } from "@mui/material/styles";
import { jsx as jsx48 } from "react/jsx-runtime";
var NotificationHeaderTitleRoot = styled37(Typography10, {
  name: "MuiNotificationPanel",
  slot: "HeaderTitle"
})({
  fontWeight: 600
});
var NotificationHeaderTitle = ({
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx48(NotificationHeaderTitleRoot, { variant: "h6", sx, children });
};
NotificationHeaderTitle.displayName = "NotificationHeaderTitle";

// src/components/NotificationPanel/NotificationHeaderBadge.tsx
import Chip2 from "@mui/material/Chip";
import { styled as styled38 } from "@mui/material/styles";
import { jsx as jsx49 } from "react/jsx-runtime";
var NotificationHeaderBadgeRoot = styled38(Chip2, {
  name: "MuiNotificationPanel",
  slot: "HeaderBadge"
})({
  height: 20,
  fontSize: 11
});
var NotificationHeaderBadge = ({
  children,
  color = "primary",
  sx
}) => {
  if (!children || children === 0 || children === "0") {
    return null;
  }
  return /* @__PURE__ */ jsx49(NotificationHeaderBadgeRoot, { label: children, size: "small", color, sx });
};
NotificationHeaderBadge.displayName = "NotificationHeaderBadge";

// src/components/NotificationPanel/NotificationHeaderClose.tsx
import IconButton4 from "@mui/material/IconButton";
import { styled as styled39 } from "@mui/material/styles";
import { X as X2 } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx50 } from "react/jsx-runtime";
var NotificationHeaderCloseRoot = styled39(IconButton4, {
  name: "MuiNotificationPanel",
  slot: "HeaderClose"
})({});
var NotificationHeaderClose = ({
  children,
  onClick,
  sx
}) => {
  const { onClose } = useNotificationPanel();
  const handleClick = () => {
    if (onClick) {
      onClick();
    } else {
      onClose == null ? void 0 : onClose();
    }
  };
  return /* @__PURE__ */ jsx50(NotificationHeaderCloseRoot, { onClick: handleClick, size: "small", sx, children: children || /* @__PURE__ */ jsx50(X2, { size: 20 }) });
};
NotificationHeaderClose.displayName = "NotificationHeaderClose";

// src/components/NotificationPanel/NotificationTabs.tsx
import Box20 from "@mui/material/Box";
import Tabs from "@mui/material/Tabs";
import Tab from "@mui/material/Tab";
import Chip3 from "@mui/material/Chip";
import { styled as styled40 } from "@mui/material/styles";
import { jsx as jsx51, jsxs as jsxs16 } from "react/jsx-runtime";
var NotificationTabsRoot = styled40(Box20, {
  name: "MuiNotificationPanel",
  slot: "Tabs"
})(({ theme }) => ({
  borderBottom: `1px solid ${(theme.vars || theme).palette.divider}`
}));
var NotificationTabsInner = styled40(Tabs, {
  name: "MuiNotificationPanel",
  slot: "TabsInner"
})({
  minHeight: 44,
  "& .MuiTab-root": {
    minHeight: 44,
    textTransform: "none",
    fontWeight: 500
  }
});
var NotificationTabLabel = styled40(Box20, {
  name: "MuiNotificationPanel",
  slot: "TabLabel"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(1)
}));
var NotificationTabCount = styled40(Chip3, {
  name: "MuiNotificationPanel",
  slot: "TabCount"
})({
  height: 18,
  fontSize: 10
});
var NotificationTabs = ({
  tabs,
  value,
  onChange,
  sx
}) => {
  return /* @__PURE__ */ jsx51(NotificationTabsRoot, { sx, children: /* @__PURE__ */ jsx51(
    NotificationTabsInner,
    {
      value,
      onChange: (_, newValue) => onChange(newValue),
      variant: "fullWidth",
      children: tabs.map((tab, index) => /* @__PURE__ */ jsx51(
        Tab,
        {
          label: /* @__PURE__ */ jsxs16(NotificationTabLabel, { children: [
            tab.label,
            tab.count !== void 0 && tab.count > 0 && /* @__PURE__ */ jsx51(
              NotificationTabCount,
              {
                label: tab.count,
                size: "small",
                color: tab.color || "default",
                variant: tab.color ? "filled" : "outlined"
              }
            )
          ] })
        },
        index
      ))
    }
  ) });
};

// src/components/NotificationPanel/NotificationActions.tsx
import Box21 from "@mui/material/Box";
import Button3 from "@mui/material/Button";
import { styled as styled41 } from "@mui/material/styles";
import { CheckCheck, Trash2 } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx52, jsxs as jsxs17 } from "react/jsx-runtime";
var NotificationActionsRoot = styled41(Box21, {
  name: "MuiNotificationPanel",
  slot: "Actions"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  justifyContent: "flex-end",
  gap: theme.spacing(1),
  padding: theme.spacing(1),
  borderBottom: `1px solid ${(theme.vars || theme).palette.divider}`,
  backgroundColor: (theme.vars || theme).palette.action.hover
}));
var NotificationActionButton = styled41(Button3, {
  name: "MuiNotificationPanel",
  slot: "ActionButton"
})({
  textTransform: "none",
  fontSize: 12
});
var NotificationActions = ({
  hasUnread = false,
  onMarkAllRead,
  onClearAll,
  sx
}) => {
  return /* @__PURE__ */ jsxs17(NotificationActionsRoot, { sx, children: [
    hasUnread && onMarkAllRead && /* @__PURE__ */ jsx52(
      NotificationActionButton,
      {
        size: "small",
        startIcon: /* @__PURE__ */ jsx52(CheckCheck, { size: 14 }),
        onClick: onMarkAllRead,
        children: "Mark all read"
      }
    ),
    onClearAll && /* @__PURE__ */ jsx52(
      NotificationActionButton,
      {
        size: "small",
        startIcon: /* @__PURE__ */ jsx52(Trash2, { size: 14 }),
        onClick: onClearAll,
        color: "error",
        children: "Clear all"
      }
    )
  ] });
};

// src/components/NotificationPanel/NotificationList.tsx
import * as React17 from "react";
import Box22 from "@mui/material/Box";
import List3 from "@mui/material/List";
import Divider6 from "@mui/material/Divider";
import { styled as styled42 } from "@mui/material/styles";
import { jsx as jsx53, jsxs as jsxs18 } from "react/jsx-runtime";
var NotificationListRoot = styled42(Box22, {
  name: "MuiNotificationPanel",
  slot: "List"
})({
  flex: 1,
  overflow: "auto"
});
var NotificationList = ({
  children,
  showDividers = true,
  sx
}) => {
  const childArray = React17.Children.toArray(children);
  return /* @__PURE__ */ jsx53(NotificationListRoot, { sx, children: /* @__PURE__ */ jsx53(List3, { disablePadding: true, children: childArray.map((child, index) => /* @__PURE__ */ jsxs18(React17.Fragment, { children: [
    child,
    showDividers && index < childArray.length - 1 && /* @__PURE__ */ jsx53(Divider6, {})
  ] }, index)) }) });
};

// src/components/NotificationPanel/NotificationItem.tsx
import * as React19 from "react";
import Box23 from "@mui/material/Box";
import ListItem2 from "@mui/material/ListItem";
import ListItemButton3 from "@mui/material/ListItemButton";
import ListItemText3 from "@mui/material/ListItemText";
import IconButton5 from "@mui/material/IconButton";
import { styled as styled43 } from "@mui/material/styles";
import { X as X3 } from "@wso2/oxygen-ui-icons-react";

// src/components/NotificationPanel/utils.ts
import {
  Info,
  AlertTriangle,
  AlertCircle,
  CheckCircle
} from "@wso2/oxygen-ui-icons-react";
var getNotificationTypeProps = (type) => {
  switch (type) {
    case "success":
      return {
        icon: CheckCircle,
        color: "success.main",
        bgcolor: "success.light"
      };
    case "warning":
      return {
        icon: AlertTriangle,
        color: "warning.main",
        bgcolor: "warning.light"
      };
    case "error":
      return {
        icon: AlertCircle,
        color: "error.main",
        bgcolor: "error.light"
      };
    default:
      return {
        icon: Info,
        color: "info.main",
        bgcolor: "info.light"
      };
  }
};

// src/components/NotificationPanel/NotificationItemContext.tsx
import * as React18 from "react";
import { jsx as jsx54 } from "react/jsx-runtime";
var NotificationItemContext = React18.createContext(null);
var useNotificationItemContext = () => {
  const context = React18.useContext(NotificationItemContext);
  if (!context) {
    throw new Error(
      "useNotificationItemContext must be used within a NotificationItem component"
    );
  }
  return context;
};
var NotificationItemProvider = ({ value, children }) => {
  return /* @__PURE__ */ jsx54(NotificationItemContext.Provider, { value, children });
};

// src/components/NotificationPanel/NotificationItem.tsx
import { jsx as jsx55, jsxs as jsxs19 } from "react/jsx-runtime";
var NotificationItemRoot = styled43(ListItem2, {
  name: "MuiNotificationPanel",
  slot: "Item",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ theme, ownerState }) => ({
  backgroundColor: ownerState.read ? "transparent" : (theme.vars || theme).palette.action.hover,
  "&:hover": {
    backgroundColor: (theme.vars || theme).palette.action.selected
  }
}));
var NotificationItemButton = styled43(ListItemButton3, {
  name: "MuiNotificationPanel",
  slot: "ItemButton"
})(({ theme }) => ({
  paddingRight: theme.spacing(6)
}));
var NotificationItemDismiss = styled43(IconButton5, {
  name: "MuiNotificationPanel",
  slot: "ItemDismiss"
})({
  opacity: 0.5,
  "&:hover": {
    opacity: 1
  }
});
var NotificationItemFooter = styled43(Box23, {
  name: "MuiNotificationPanel",
  slot: "ItemFooter"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(1),
  marginTop: theme.spacing(0.5)
}));
var separateChildren5 = (children) => {
  let avatarChild = null;
  let titleChild = null;
  let messageChild = null;
  let timestampChild = null;
  let actionChild = null;
  React19.Children.forEach(children, (child) => {
    var _a;
    if (React19.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      switch (displayName) {
        case "NotificationItemAvatar":
          avatarChild = child;
          break;
        case "NotificationItemTitle":
          titleChild = child;
          break;
        case "NotificationItemMessage":
          messageChild = child;
          break;
        case "NotificationItemTimestamp":
          timestampChild = child;
          break;
        case "NotificationItemAction":
          actionChild = child;
          break;
      }
    }
  });
  return { avatarChild, titleChild, messageChild, timestampChild, actionChild };
};
var NotificationItem = ({
  id,
  type,
  read = false,
  children,
  onAction,
  onMarkRead,
  onDismiss,
  sx
}) => {
  const typeProps = getNotificationTypeProps(type);
  const { avatarChild, titleChild, messageChild, timestampChild, actionChild } = separateChildren5(children);
  const contextValue = {
    id,
    type,
    typeProps,
    read,
    onMarkRead: () => onMarkRead == null ? void 0 : onMarkRead(id),
    onAction
  };
  const handleClick = () => {
    if (!read) {
      onMarkRead == null ? void 0 : onMarkRead(id);
    }
    onAction == null ? void 0 : onAction();
  };
  const ownerState = { read };
  return /* @__PURE__ */ jsx55(NotificationItemProvider, { value: contextValue, children: /* @__PURE__ */ jsx55(
    NotificationItemRoot,
    {
      disablePadding: true,
      ownerState,
      secondaryAction: onDismiss && /* @__PURE__ */ jsx55(
        NotificationItemDismiss,
        {
          size: "small",
          onClick: (e) => {
            e.stopPropagation();
            onDismiss(id);
          },
          children: /* @__PURE__ */ jsx55(X3, { size: 16 })
        }
      ),
      sx,
      children: /* @__PURE__ */ jsxs19(NotificationItemButton, { onClick: handleClick, children: [
        avatarChild,
        /* @__PURE__ */ jsx55(
          ListItemText3,
          {
            primary: titleChild,
            secondary: /* @__PURE__ */ jsxs19(Box23, { component: "span", children: [
              messageChild,
              /* @__PURE__ */ jsxs19(NotificationItemFooter, { children: [
                timestampChild,
                actionChild
              ] })
            ] })
          }
        )
      ] })
    }
  ) });
};
NotificationItem.displayName = "NotificationItem";

// src/components/NotificationPanel/NotificationItemAvatar.tsx
import ListItemAvatar from "@mui/material/ListItemAvatar";
import Avatar3 from "@mui/material/Avatar";
import { styled as styled44 } from "@mui/material/styles";
import { jsx as jsx56 } from "react/jsx-runtime";
var NotificationItemAvatarStyled = styled44(Avatar3, {
  name: "MuiNotificationPanel",
  slot: "ItemAvatar",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ ownerState }) => ({
  width: 40,
  height: 40,
  backgroundColor: ownerState.bgcolor,
  color: ownerState.color,
  fontSize: ownerState.hasChildren ? 14 : void 0,
  fontWeight: ownerState.hasChildren ? 600 : void 0
}));
var NotificationItemAvatar = ({
  children,
  sx
}) => {
  const { typeProps } = useNotificationItemContext();
  const Icon = typeProps.icon;
  const ownerState = {
    bgcolor: typeProps.bgcolor,
    color: typeProps.color,
    hasChildren: !!children
  };
  return /* @__PURE__ */ jsx56(ListItemAvatar, { children: /* @__PURE__ */ jsx56(NotificationItemAvatarStyled, { ownerState, sx, children: children || /* @__PURE__ */ jsx56(Icon, { size: 20 }) }) });
};
NotificationItemAvatar.displayName = "NotificationItemAvatar";

// src/components/NotificationPanel/NotificationItemTitle.tsx
import Box24 from "@mui/material/Box";
import Typography11 from "@mui/material/Typography";
import { styled as styled45 } from "@mui/material/styles";
import { jsx as jsx57, jsxs as jsxs20 } from "react/jsx-runtime";
var NotificationItemTitleRoot = styled45(Box24, {
  name: "MuiNotificationPanel",
  slot: "ItemTitle"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(1)
}));
var NotificationItemTitleText = styled45(Typography11, {
  name: "MuiNotificationPanel",
  slot: "ItemTitleText",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ ownerState }) => ({
  fontWeight: ownerState.read ? 400 : 600,
  flex: 1
}));
var NotificationItemUnreadDot = styled45(Box24, {
  name: "MuiNotificationPanel",
  slot: "ItemUnreadDot"
})(({ theme }) => ({
  width: 8,
  height: 8,
  borderRadius: "50%",
  backgroundColor: (theme.vars || theme).palette.primary.main,
  flexShrink: 0
}));
var NotificationItemTitle = ({
  children,
  showUnreadIndicator = true,
  sx
}) => {
  const { read } = useNotificationItemContext();
  return /* @__PURE__ */ jsxs20(NotificationItemTitleRoot, { sx, children: [
    /* @__PURE__ */ jsx57(NotificationItemTitleText, { variant: "body2", ownerState: { read }, children }),
    showUnreadIndicator && !read && /* @__PURE__ */ jsx57(NotificationItemUnreadDot, {})
  ] });
};
NotificationItemTitle.displayName = "NotificationItemTitle";

// src/components/NotificationPanel/NotificationItemMessage.tsx
import Typography12 from "@mui/material/Typography";
import { styled as styled46 } from "@mui/material/styles";
import { jsx as jsx58 } from "react/jsx-runtime";
var NotificationItemMessageRoot = styled46(Typography12, {
  name: "MuiNotificationPanel",
  slot: "ItemMessage"
})(({ theme }) => ({
  display: "block",
  color: (theme.vars || theme).palette.text.secondary,
  marginBottom: theme.spacing(0.5)
}));
var NotificationItemMessage = ({
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx58(NotificationItemMessageRoot, { component: "span", variant: "caption", sx, children });
};
NotificationItemMessage.displayName = "NotificationItemMessage";

// src/components/NotificationPanel/NotificationItemTimestamp.tsx
import Typography13 from "@mui/material/Typography";
import { styled as styled47 } from "@mui/material/styles";
import { jsx as jsx59 } from "react/jsx-runtime";
var NotificationItemTimestampRoot = styled47(Typography13, {
  name: "MuiNotificationPanel",
  slot: "ItemTimestamp"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.disabled,
  fontSize: 11
}));
var NotificationItemTimestamp = ({
  children,
  sx
}) => {
  return /* @__PURE__ */ jsx59(NotificationItemTimestampRoot, { component: "span", variant: "caption", sx, children });
};
NotificationItemTimestamp.displayName = "NotificationItemTimestamp";

// src/components/NotificationPanel/NotificationItemAction.tsx
import Chip4 from "@mui/material/Chip";
import { styled as styled48 } from "@mui/material/styles";
import { jsx as jsx60 } from "react/jsx-runtime";
var NotificationItemActionRoot = styled48(Chip4, {
  name: "MuiNotificationPanel",
  slot: "ItemAction"
})({
  height: 20,
  fontSize: 10,
  cursor: "pointer"
});
var NotificationItemAction = ({
  children,
  onClick,
  sx
}) => {
  const { onAction } = useNotificationItemContext();
  const handleClick = (e) => {
    e.stopPropagation();
    if (onClick) {
      onClick(e);
    } else {
      onAction == null ? void 0 : onAction();
    }
  };
  return /* @__PURE__ */ jsx60(
    NotificationItemActionRoot,
    {
      label: children,
      size: "small",
      variant: "outlined",
      onClick: handleClick,
      sx
    }
  );
};
NotificationItemAction.displayName = "NotificationItemAction";

// src/components/NotificationPanel/NotificationEmptyState.tsx
import Box25 from "@mui/material/Box";
import Typography14 from "@mui/material/Typography";
import { styled as styled49 } from "@mui/material/styles";
import { BellOff } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx61, jsxs as jsxs21 } from "react/jsx-runtime";
var NotificationEmptyStateRoot = styled49(Box25, {
  name: "MuiNotificationPanel",
  slot: "EmptyState"
})(({ theme }) => ({
  display: "flex",
  flexDirection: "column",
  alignItems: "center",
  justifyContent: "center",
  paddingTop: theme.spacing(8),
  paddingBottom: theme.spacing(8),
  paddingLeft: theme.spacing(3),
  paddingRight: theme.spacing(3),
  textAlign: "center"
}));
var NotificationEmptyStateMessage = styled49(Typography14, {
  name: "MuiNotificationPanel",
  slot: "EmptyStateMessage"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.secondary
}));
var NotificationEmptyState = ({
  message = "No notifications",
  icon,
  sx
}) => {
  return /* @__PURE__ */ jsxs21(NotificationEmptyStateRoot, { sx, children: [
    icon || /* @__PURE__ */ jsx61(BellOff, { size: 48, style: { opacity: 0.3, marginBottom: 16 } }),
    /* @__PURE__ */ jsx61(NotificationEmptyStateMessage, { variant: "body2", children: message })
  ] });
};

// src/components/NotificationPanel/NotificationPanel.tsx
import { jsx as jsx62 } from "react/jsx-runtime";
var NotificationPanelRoot = styled50(Drawer, {
  name: "MuiNotificationPanel",
  slot: "Root",
  shouldForwardProp: (prop) => prop !== "ownerState"
})(({ theme, ownerState }) => ({
  "& .MuiDrawer-paper": {
    width: "100%",
    [theme.breakpoints.up("sm")]: {
      width: ownerState.width
    },
    maxWidth: "100%",
    display: "flex",
    flexDirection: "column"
  }
}));
var NotificationPanel = ({
  open,
  onClose,
  children,
  width = 380,
  anchor = "right",
  variant = "temporary",
  sx
}) => {
  const contextValue = React20.useMemo(() => ({ onClose }), [onClose]);
  const ownerState = { width };
  return /* @__PURE__ */ jsx62(NotificationPanelContext.Provider, { value: contextValue, children: /* @__PURE__ */ jsx62(
    NotificationPanelRoot,
    {
      anchor,
      open,
      onClose,
      ownerState,
      sx,
      variant,
      children
    }
  ) });
};
NotificationPanel.Header = NotificationHeader;
NotificationPanel.HeaderIcon = NotificationHeaderIcon;
NotificationPanel.HeaderTitle = NotificationHeaderTitle;
NotificationPanel.HeaderBadge = NotificationHeaderBadge;
NotificationPanel.HeaderClose = NotificationHeaderClose;
NotificationPanel.Tabs = NotificationTabs;
NotificationPanel.Actions = NotificationActions;
NotificationPanel.List = NotificationList;
NotificationPanel.Item = NotificationItem;
NotificationPanel.ItemAvatar = NotificationItemAvatar;
NotificationPanel.ItemTitle = NotificationItemTitle;
NotificationPanel.ItemMessage = NotificationItemMessage;
NotificationPanel.ItemTimestamp = NotificationItemTimestamp;
NotificationPanel.ItemAction = NotificationItemAction;
NotificationPanel.EmptyState = NotificationEmptyState;
NotificationPanel.displayName = "NotificationPanel";

// src/components/Footer/Footer.tsx
import * as React21 from "react";
import Box26 from "@mui/material/Box";
import { styled as styled54 } from "@mui/material/styles";

// src/components/Footer/FooterCopyright.tsx
import Typography15 from "@mui/material/Typography";
import { styled as styled51 } from "@mui/material/styles";
import { jsx as jsx63 } from "react/jsx-runtime";
var StyledCopyrightText = styled51(Typography15, {
  name: "MuiFooter",
  slot: "CopyrightText"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.secondary
}));
var FooterCopyright = ({ children }) => {
  return /* @__PURE__ */ jsx63(StyledCopyrightText, { variant: "caption", children });
};
FooterCopyright.displayName = "Footer.Copyright";

// src/components/Footer/FooterVersion.tsx
import Typography16 from "@mui/material/Typography";
import { styled as styled52 } from "@mui/material/styles";
import { jsx as jsx64 } from "react/jsx-runtime";
var StyledVersion = styled52(Typography16, {
  name: "MuiFooter",
  slot: "Version"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.disabled,
  fontFamily: "monospace",
  fontSize: 11
}));
var FooterVersion = ({ children }) => {
  return /* @__PURE__ */ jsx64(StyledVersion, { variant: "caption", children });
};
FooterVersion.displayName = "Footer.Version";

// src/components/Footer/FooterLink.tsx
import Link2 from "@mui/material/Link";
import { styled as styled53 } from "@mui/material/styles";
import { jsx as jsx65 } from "react/jsx-runtime";
var StyledLink = styled53(Link2, {
  name: "MuiFooter",
  slot: "Link"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.secondary,
  fontSize: 12,
  "&:hover": {
    color: (theme.vars || theme).palette.text.primary
  }
}));
var FooterLink = (_a) => {
  var _b = _a, { children, href } = _b, props = __objRest(_b, ["children", "href"]);
  return /* @__PURE__ */ jsx65(StyledLink, __spreadProps(__spreadValues({ href, underline: "hover" }, props), { children }));
};
FooterLink.displayName = "Footer.Link";

// src/components/Footer/FooterDivider.tsx
import Divider7 from "@mui/material/Divider";
import { jsx as jsx66 } from "react/jsx-runtime";
var FooterDivider = () => {
  return /* @__PURE__ */ jsx66(
    Divider7,
    {
      orientation: "vertical",
      flexItem: true,
      sx: { display: { xs: "none", sm: "block" } }
    }
  );
};
FooterDivider.displayName = "Footer.Divider";

// src/components/Footer/Footer.tsx
import { jsx as jsx67, jsxs as jsxs22 } from "react/jsx-runtime";
var FooterRoot = styled54(Box26, {
  name: "MuiFooter",
  slot: "Root"
})(({ theme }) => ({
  width: "100%",
  paddingTop: theme.spacing(2),
  paddingBottom: theme.spacing(2),
  paddingLeft: theme.spacing(3),
  paddingRight: theme.spacing(3),
  marginTop: "auto",
  borderTop: `1px solid ${(theme.vars || theme).palette.divider}`,
  backgroundColor: (theme.vars || theme).palette.background.paper
}));
var FooterContent = styled54(Box26, {
  name: "MuiFooter",
  slot: "Content"
})(({ theme }) => ({
  display: "flex",
  flexDirection: "column",
  alignItems: "flex-start",
  justifyContent: "space-between",
  gap: theme.spacing(2),
  [theme.breakpoints.up("sm")]: {
    flexDirection: "row",
    alignItems: "center"
  }
}));
var FooterLeft = styled54(Box26, {
  name: "MuiFooter",
  slot: "Left"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(2),
  flexWrap: "wrap"
}));
var FooterRight = styled54(Box26, {
  name: "MuiFooter",
  slot: "Right"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(2),
  flexWrap: "wrap"
}));
var Footer = ({ children, sx }) => {
  const childrenArray = React21.Children.toArray(children);
  const leftChildren = [];
  const rightChildren = [];
  childrenArray.forEach((child) => {
    if (React21.isValidElement(child)) {
      if (child.type === FooterCopyright || child.type === FooterVersion || child.type === FooterDivider) {
        leftChildren.push(child);
      } else if (child.type === FooterLink) {
        rightChildren.push(child);
      }
    }
  });
  return /* @__PURE__ */ jsx67(FooterRoot, { component: "footer", sx, children: /* @__PURE__ */ jsxs22(FooterContent, { children: [
    leftChildren.length > 0 && /* @__PURE__ */ jsx67(FooterLeft, { children: leftChildren }),
    rightChildren.length > 0 && /* @__PURE__ */ jsx67(FooterRight, { children: rightChildren })
  ] }) });
};
Footer.Copyright = FooterCopyright;
Footer.Version = FooterVersion;
Footer.Link = FooterLink;
Footer.Divider = FooterDivider;
Footer.displayName = "Footer";

// src/components/UserMenu/UserMenu.tsx
import * as React23 from "react";
import Menu2 from "@mui/material/Menu";

// src/components/UserMenu/UserMenuContext.tsx
import * as React22 from "react";
var UserMenuContext = React22.createContext(void 0);
var useUserMenu = () => {
  const context = React22.useContext(UserMenuContext);
  if (!context) {
    throw new Error("UserMenu sub-components must be used within a UserMenu component");
  }
  return context;
};
UserMenuContext.displayName = "UserMenu.Context";

// src/components/UserMenu/UserMenuTrigger.tsx
import Tooltip5 from "@mui/material/Tooltip";
import IconButton6 from "@mui/material/IconButton";
import Avatar4 from "@mui/material/Avatar";
import { styled as styled55 } from "@mui/material/styles";
import { jsx as jsx68, jsxs as jsxs23 } from "react/jsx-runtime";
var StyledTrigger = styled55(IconButton6, {
  name: "MuiUserMenu",
  slot: "Trigger"
})(({ theme, showName }) => __spreadValues({
  padding: theme.spacing(0.5),
  gap: theme.spacing(1)
}, showName && {
  paddingRight: theme.spacing(1.5),
  borderRadius: typeof theme.shape.borderRadius === "number" ? theme.shape.borderRadius * 2 : theme.spacing(2)
}));
var StyledAvatar2 = styled55(Avatar4, {
  name: "MuiUserMenu",
  slot: "Avatar"
})(({ theme }) => ({
  width: 32,
  height: 32,
  backgroundColor: (theme.vars || theme).palette.primary.main,
  color: (theme.vars || theme).palette.primary.contrastText,
  fontSize: 14,
  fontWeight: 600
}));
var StyledNameText = styled55("span", {
  name: "MuiUserMenu",
  slot: "NameText"
})(({ theme }) => ({
  fontSize: 14,
  fontWeight: 500,
  color: (theme.vars || theme).palette.text.primary,
  display: "none",
  [theme.breakpoints.up("sm")]: {
    display: "block"
  }
}));
var UserMenuTrigger = ({ name, avatar, showName = false }) => {
  const { open, handleOpen } = useUserMenu();
  return /* @__PURE__ */ jsx68(Tooltip5, { title: "Account", children: /* @__PURE__ */ jsxs23(
    StyledTrigger,
    {
      onClick: handleOpen,
      size: "small",
      showName,
      "aria-label": showName ? name : "Account",
      "aria-controls": open ? "user-menu" : void 0,
      "aria-haspopup": "true",
      "aria-expanded": open ? "true" : void 0,
      children: [
        /* @__PURE__ */ jsx68(StyledAvatar2, { src: avatar || void 0, alt: name, children: !avatar && name.charAt(0) }),
        showName && /* @__PURE__ */ jsx68(StyledNameText, { children: name })
      ]
    }
  ) });
};
UserMenuTrigger.displayName = "UserMenu.Trigger";

// src/components/UserMenu/UserMenuHeader.tsx
import Divider8 from "@mui/material/Divider";
import Box27 from "@mui/material/Box";
import Avatar5 from "@mui/material/Avatar";
import Typography17 from "@mui/material/Typography";
import Chip5 from "@mui/material/Chip";
import { styled as styled56 } from "@mui/material/styles";
import { Fragment as Fragment9, jsx as jsx69, jsxs as jsxs24 } from "react/jsx-runtime";
var StyledHeaderBox = styled56(Box27, {
  name: "MuiUserMenu",
  slot: "HeaderBox"
})(({ theme }) => ({
  padding: theme.spacing(2),
  paddingBottom: theme.spacing(1.5)
}));
var StyledHeaderContent = styled56(Box27, {
  name: "MuiUserMenu",
  slot: "HeaderContent"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(2)
}));
var StyledHeaderAvatar = styled56(Avatar5, {
  name: "MuiUserMenu",
  slot: "HeaderAvatar"
})(({ theme }) => ({
  width: 40,
  height: 40,
  backgroundColor: (theme.vars || theme).palette.primary.main,
  color: (theme.vars || theme).palette.primary.contrastText,
  fontSize: 16,
  fontWeight: 600
}));
var StyledUserInfo = styled56(Box27, {
  name: "MuiUserMenu",
  slot: "UserInfo"
})(({ theme }) => ({
  display: "flex",
  flexDirection: "column",
  gap: theme.spacing(0.25),
  flex: 1,
  minWidth: 0
}));
var StyledNameRow = styled56(Box27, {
  name: "MuiUserMenu",
  slot: "NameRow"
})(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  gap: theme.spacing(1)
}));
var StyledName = styled56(Typography17, {
  name: "MuiUserMenu",
  slot: "Name"
})({
  fontWeight: 600,
  whiteSpace: "nowrap",
  overflow: "hidden",
  textOverflow: "ellipsis"
});
var StyledRoleChip = styled56(Chip5, {
  name: "MuiUserMenu",
  slot: "RoleChip"
})({
  height: 20,
  fontSize: 11,
  fontWeight: 600
});
var StyledEmail = styled56(Typography17, {
  name: "MuiUserMenu",
  slot: "Email"
})(({ theme }) => ({
  color: (theme.vars || theme).palette.text.secondary,
  display: "block",
  whiteSpace: "nowrap",
  overflow: "hidden",
  textOverflow: "ellipsis"
}));
var UserMenuHeader = ({ name, email, avatar, role }) => {
  return /* @__PURE__ */ jsxs24(Fragment9, { children: [
    /* @__PURE__ */ jsx69(StyledHeaderBox, { children: /* @__PURE__ */ jsxs24(StyledHeaderContent, { children: [
      /* @__PURE__ */ jsx69(StyledHeaderAvatar, { src: avatar || void 0, alt: name, children: !avatar && name.charAt(0) }),
      /* @__PURE__ */ jsxs24(StyledUserInfo, { children: [
        /* @__PURE__ */ jsxs24(StyledNameRow, { children: [
          /* @__PURE__ */ jsx69(StyledName, { variant: "subtitle2", children: name }),
          role && /* @__PURE__ */ jsx69(
            StyledRoleChip,
            {
              label: role,
              size: "small",
              color: "primary"
            }
          )
        ] }),
        /* @__PURE__ */ jsx69(StyledEmail, { variant: "caption", children: email })
      ] })
    ] }) }),
    /* @__PURE__ */ jsx69(Divider8, {})
  ] });
};
UserMenuHeader.displayName = "UserMenu.Header";

// src/components/UserMenu/UserMenuItem.tsx
import MenuItem3 from "@mui/material/MenuItem";
import ListItemIcon3 from "@mui/material/ListItemIcon";
import ListItemText4 from "@mui/material/ListItemText";
import Box28 from "@mui/material/Box";
import Chip6 from "@mui/material/Chip";
import { styled as styled57 } from "@mui/material/styles";
import { ChevronRight } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx70, jsxs as jsxs25 } from "react/jsx-runtime";
var StyledMenuItem2 = styled57(MenuItem3, {
  name: "MuiUserMenu",
  slot: "MenuItem"
})(({ theme }) => ({
  paddingTop: theme.spacing(1.5),
  paddingBottom: theme.spacing(1.5)
}));
var StyledBillingChip = styled57(Chip6, {
  name: "MuiUserMenu",
  slot: "BillingChip"
})(() => ({
  height: 20,
  fontSize: 11,
  fontWeight: 600
}));
var UserMenuItem = ({
  icon,
  label,
  badge,
  onClick
}) => {
  const { handleClose } = useUserMenu();
  const handleClick = () => {
    handleClose();
    onClick == null ? void 0 : onClick();
  };
  return /* @__PURE__ */ jsxs25(StyledMenuItem2, { onClick: handleClick, children: [
    icon && /* @__PURE__ */ jsx70(ListItemIcon3, { children: icon }),
    /* @__PURE__ */ jsx70(ListItemText4, { primary: label }),
    /* @__PURE__ */ jsxs25(Box28, { sx: { display: "flex", alignItems: "center", gap: 0.5 }, children: [
      badge && /* @__PURE__ */ jsx70(
        StyledBillingChip,
        {
          label: badge,
          size: "small",
          variant: "outlined"
        }
      ),
      /* @__PURE__ */ jsx70(ChevronRight, { size: 16, style: { color: "var(--mui-palette-text-secondary)" } })
    ] })
  ] });
};
UserMenuItem.displayName = "UserMenu.Item";

// src/components/UserMenu/UserMenuLogout.tsx
import MenuItem4 from "@mui/material/MenuItem";
import ListItemIcon4 from "@mui/material/ListItemIcon";
import ListItemText5 from "@mui/material/ListItemText";
import { styled as styled58 } from "@mui/material/styles";
import { jsx as jsx71, jsxs as jsxs26 } from "react/jsx-runtime";
var StyledLogoutItem = styled58(MenuItem4, {
  name: "MuiUserMenu",
  slot: "LogoutItem"
})(({ theme }) => ({
  paddingTop: theme.spacing(1.5),
  paddingBottom: theme.spacing(1.5),
  color: (theme.vars || theme).palette.error.main,
  "&:hover": {
    backgroundColor: (theme.vars || theme).palette.error.main,
    color: (theme.vars || theme).palette.error.contrastText,
    "& .MuiListItemIcon-root": {
      color: (theme.vars || theme).palette.error.contrastText
    }
  }
}));
var UserMenuLogout = ({
  icon,
  label = "Log out",
  onClick
}) => {
  const { handleClose } = useUserMenu();
  const handleClick = () => {
    handleClose();
    onClick == null ? void 0 : onClick();
  };
  return /* @__PURE__ */ jsxs26(StyledLogoutItem, { onClick: handleClick, children: [
    icon && /* @__PURE__ */ jsx71(ListItemIcon4, { children: icon }),
    /* @__PURE__ */ jsx71(ListItemText5, { primary: label })
  ] });
};
UserMenuLogout.displayName = "UserMenu.Logout";

// src/components/UserMenu/UserMenuDivider.tsx
import Divider9 from "@mui/material/Divider";
import { jsx as jsx72 } from "react/jsx-runtime";
var UserMenuDivider = () => /* @__PURE__ */ jsx72(Divider9, {});
UserMenuDivider.displayName = "UserMenu.Divider";

// src/components/UserMenu/UserMenu.tsx
import { jsx as jsx73, jsxs as jsxs27 } from "react/jsx-runtime";
var UserMenu = ({ children }) => {
  const [anchorEl, setAnchorEl] = React23.useState(null);
  const open = Boolean(anchorEl);
  const handleOpen = (event) => {
    setAnchorEl(event.currentTarget);
  };
  const handleClose = () => {
    setAnchorEl(null);
  };
  const childrenArray = React23.Children.toArray(children);
  const triggerChild = childrenArray.find(
    (child) => React23.isValidElement(child) && child.type === UserMenuTrigger
  );
  const menuChildren = childrenArray.filter(
    (child) => React23.isValidElement(child) && child.type !== UserMenuTrigger
  );
  return /* @__PURE__ */ jsxs27(UserMenuContext.Provider, { value: { open, anchorEl, handleOpen, handleClose }, children: [
    triggerChild,
    /* @__PURE__ */ jsx73(
      Menu2,
      {
        id: "user-menu",
        anchorEl,
        open,
        onClose: handleClose,
        anchorOrigin: { vertical: "bottom", horizontal: "right" },
        transformOrigin: { vertical: "top", horizontal: "right" },
        slotProps: {
          paper: {
            sx: {
              minWidth: 240,
              mt: 1
            }
          }
        },
        children: menuChildren
      }
    )
  ] });
};
UserMenu.Trigger = UserMenuTrigger;
UserMenu.Header = UserMenuHeader;
UserMenu.Item = UserMenuItem;
UserMenu.Logout = UserMenuLogout;
UserMenu.Divider = UserMenuDivider;
UserMenu.displayName = "UserMenu";

// src/components/NotificationBanner/NotificationBanner.tsx
import * as React24 from "react";
import Alert from "@mui/material/Alert";
import AlertTitle from "@mui/material/AlertTitle";
import Collapse2 from "@mui/material/Collapse";
import Button4 from "@mui/material/Button";
import Box29 from "@mui/material/Box";
import { styled as styled59 } from "@mui/material/styles";
import { jsx as jsx74, jsxs as jsxs28 } from "react/jsx-runtime";
var NotificationBannerAlert = styled59(Alert, {
  name: "MuiNotificationBanner",
  slot: "Root"
})({
  borderRadius: 0,
  "& .MuiAlert-message": {
    width: "100%",
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    flexWrap: "wrap",
    gap: 8
  },
  "& .MuiAlert-action": {
    paddingTop: 0,
    alignItems: "center"
  }
});
var NotificationBannerContent = styled59(Box29, {
  name: "MuiNotificationBanner",
  slot: "Content"
})({
  display: "flex",
  flexDirection: "column",
  gap: 2,
  flex: 1
});
var NotificationBannerAction = styled59(Button4, {
  name: "MuiNotificationBanner",
  slot: "Action"
})({
  fontWeight: 600,
  textDecoration: "underline",
  "&:hover": {
    textDecoration: "underline",
    backgroundColor: "transparent"
  }
});
var NotificationBanner = ({
  visible = true,
  severity = "info",
  title,
  message,
  actionLabel,
  onAction,
  onDismiss,
  sx
}) => {
  const [isVisible, setIsVisible] = React24.useState(visible);
  React24.useEffect(() => {
    setIsVisible(visible);
  }, [visible]);
  const handleDismiss = () => {
    setIsVisible(false);
    onDismiss == null ? void 0 : onDismiss();
  };
  return /* @__PURE__ */ jsx74(Collapse2, { in: isVisible, children: /* @__PURE__ */ jsx74(
    NotificationBannerAlert,
    {
      severity,
      variant: "filled",
      onClose: handleDismiss,
      sx,
      action: actionLabel && onAction ? /* @__PURE__ */ jsx74(
        NotificationBannerAction,
        {
          color: "inherit",
          size: "small",
          onClick: onAction,
          children: actionLabel
        }
      ) : void 0,
      children: /* @__PURE__ */ jsxs28(NotificationBannerContent, { children: [
        title && /* @__PURE__ */ jsx74(AlertTitle, { sx: { mb: 0 }, children: title }),
        /* @__PURE__ */ jsx74(Box29, { component: "span", children: message })
      ] })
    }
  ) });
};

// src/components/Form/CardButton.tsx
import { Card as Card2, ButtonBase, Box as Box30, styled as styled60 } from "@mui/material";
import { CardHeader, CardContent as CardContent2, CardActions, CardMedia } from "@mui/material";
import { jsx as jsx75 } from "react/jsx-runtime";
var StyledCardButton = styled60(Card2, {
  shouldForwardProp: (prop) => prop !== "selected" && prop !== "disabled"
})(({ theme, selected, disabled, alignItems = "flex-start" }) => ({
  flexDirection: "column",
  alignItems,
  display: "flex",
  textAlign: "left",
  transition: "all 0.3s ease",
  cursor: "pointer",
  "&.MuiCard-root": {
    backgroundColor: selected ? "background.paper" : "background.default",
    borderColor: selected && theme.palette.primary.main
  },
  ".creation-flow-card-actions": {
    opacity: 0,
    transition: "opacity 0.3s ease"
  },
  "&:hover": __spreadValues({
    ".creation-flow-card-actions": {
      opacity: disabled ? 0 : 1
    }
  }, !disabled && {
    borderColor: theme.palette.primary.main,
    boxShadow: theme.shadows[1]
  })
}));
var CardButton = (props) => {
  const _a = props, { disabled, selected, alignItems = "flex-start" } = _a, rest = __objRest(_a, ["disabled", "selected", "alignItems"]);
  return /* @__PURE__ */ jsx75(
    StyledCardButton,
    __spreadValues({
      alignItems,
      component: ButtonBase,
      selected,
      disabled
    }, rest)
  );
};
var DisappearingCardButtonContent = (props) => /* @__PURE__ */ jsx75(Box30, __spreadValues({ className: "creation-flow-card-actions" }, props));

// src/components/Form/Stack.tsx
import { Stack as OxygenStack } from "@mui/material";
import { jsx as jsx76 } from "react/jsx-runtime";
var Stack2 = (props) => {
  return /* @__PURE__ */ jsx76(OxygenStack, __spreadValues({ spacing: 2 }, props));
};

// src/components/Form/Typography.tsx
import { Typography as Typography18 } from "@mui/material";
import { jsx as jsx77 } from "react/jsx-runtime";
var Header2 = (props) => {
  const _a = props, { children } = _a, rest = __objRest(_a, ["children"]);
  return /* @__PURE__ */ jsx77(Typography18, __spreadProps(__spreadValues({}, rest), { variant: "h4", children }));
};
var Subheader = (props) => {
  const _a = props, { children } = _a, rest = __objRest(_a, ["children"]);
  return /* @__PURE__ */ jsx77(Typography18, __spreadProps(__spreadValues({}, rest), { variant: "h5", children }));
};
var Body = (props) => {
  const _a = props, { children } = _a, rest = __objRest(_a, ["children"]);
  return /* @__PURE__ */ jsx77(Typography18, __spreadProps(__spreadValues({}, rest), { variant: "body2", children }));
};

// src/components/Form/Section.tsx
import { Card as Card3, CardContent as CardContent3, Stack as Stack3 } from "@mui/material";
import { jsx as jsx78 } from "react/jsx-runtime";
var Section = ({ children }) => {
  return /* @__PURE__ */ jsx78(Card3, { children: /* @__PURE__ */ jsx78(CardContent3, { children: /* @__PURE__ */ jsx78(Stack3, { direction: "column", spacing: 2, children }) }) });
};

// src/components/Form/Wizard.tsx
import {
  Card as Card4,
  CardContent as CardContent4,
  StepLabel,
  Step,
  Stepper,
  Divider as Divider10
} from "@mui/material";
import { styled as styled61 } from "@mui/material/styles";
import { useMemo as useMemo5 } from "react";
import { jsx as jsx79, jsxs as jsxs29 } from "react/jsx-runtime";
var StyledWizardActionsStack = styled61(Stack2)(() => ({
  justifyContent: "space-between",
  flexGrow: 1
}));
function Wizard({ steps, activeStep, actions }) {
  const step = useMemo5(() => activeStep < steps.length ? steps[activeStep] : null, [steps, activeStep]);
  return /* @__PURE__ */ jsx79(Card4, { children: /* @__PURE__ */ jsx79(CardContent4, { children: /* @__PURE__ */ jsxs29(Stack2, { children: [
    /* @__PURE__ */ jsx79(Stepper, { activeStep, children: steps.map((step2) => /* @__PURE__ */ jsx79(Step, { children: /* @__PURE__ */ jsx79(StepLabel, { children: step2.label }) }, step2.label)) }),
    /* @__PURE__ */ jsx79(Divider10, {}),
    /* @__PURE__ */ jsx79(Stack2, { direction: "row", children: step == null ? void 0 : step.component }),
    /* @__PURE__ */ jsx79(StyledWizardActionsStack, { direction: "row", children: actions })
  ] }) }) });
}

// src/components/Form/ElementWrapper.tsx
import { FormControl as FormControl3, FormLabel } from "@mui/material";
import { jsx as jsx80, jsxs as jsxs30 } from "react/jsx-runtime";
var ElementWrapper = (props) => {
  const { label, name, children } = props;
  return /* @__PURE__ */ jsxs30(FormControl3, { fullWidth: true, children: [
    /* @__PURE__ */ jsx80(FormLabel, { htmlFor: name, children: label }),
    children
  ] });
};

// src/components/Form/index.ts
var Form = {
  CardButton,
  DisappearingCardButtonContent,
  CardHeader,
  CardContent: CardContent2,
  CardActions,
  CardMedia,
  Stack: Stack2,
  Header: Header2,
  Subheader,
  Body,
  Section,
  Wizard,
  // Form Element Wrapper
  ElementWrapper
};

// src/components/AppShell/AppShell.tsx
import * as React25 from "react";
import Box35 from "@mui/material/Box";

// src/layouts/Layout.tsx
import Box31 from "@mui/material/Box";
import { jsx as jsx81 } from "react/jsx-runtime";
function Layout(_a) {
  var _b = _a, { children = null, sx } = _b, rest = __objRest(_b, ["children", "sx"]);
  return /* @__PURE__ */ jsx81(
    Box31,
    __spreadProps(__spreadValues({
      sx: __spreadValues({
        display: "flex",
        flexDirection: "row"
      }, sx)
    }, rest), {
      children
    })
  );
}
Layout.displayName = "Layout";
var Layout_default = Layout;

// src/layouts/LayoutSidebar.tsx
import Box32 from "@mui/material/Box";
import { jsx as jsx82 } from "react/jsx-runtime";
function LayoutSidebar(_a) {
  var _b = _a, { children = null, sx } = _b, rest = __objRest(_b, ["children", "sx"]);
  return /* @__PURE__ */ jsx82(
    Box32,
    __spreadProps(__spreadValues({
      sx: __spreadValues({
        display: "flex",
        flexDirection: "column"
      }, sx)
    }, rest), {
      children
    })
  );
}
LayoutSidebar.displayName = "Layout.Sidebar";
var LayoutSidebar_default = LayoutSidebar;

// src/layouts/LayoutNavbar.tsx
function LayoutNavbar({ children = null }) {
  return children;
}
LayoutNavbar.displayName = "Layout.Navbar";
var LayoutNavbar_default = LayoutNavbar;

// src/layouts/LayoutContent.tsx
import Box33 from "@mui/material/Box";
import { jsx as jsx83 } from "react/jsx-runtime";
function LayoutContent(_a) {
  var _b = _a, { children = null } = _b, rest = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx83(
    Box33,
    __spreadProps(__spreadValues({
      component: "main",
      sx: {
        display: "flex",
        flexDirection: "column",
        flexGrow: 1,
        overflow: "auto"
      }
    }, rest), {
      children
    })
  );
}
LayoutContent.displayName = "Layout.Content";
var LayoutContent_default = LayoutContent;

// src/layouts/LayoutHeader.tsx
import Box34 from "@mui/material/Box";
import { jsx as jsx84 } from "react/jsx-runtime";
function LayoutHeader(_a) {
  var _b = _a, { children = null, sx } = _b, rest = __objRest(_b, ["children", "sx"]);
  return /* @__PURE__ */ jsx84(
    Box34,
    __spreadProps(__spreadValues({
      sx: __spreadValues({
        display: "flex"
      }, sx)
    }, rest), {
      children
    })
  );
}
LayoutHeader.displayName = "Layout.Header";
var LayoutHeader_default = LayoutHeader;

// src/layouts/index.ts
var Layout2 = Object.assign(Layout_default, {
  Sidebar: LayoutSidebar_default,
  Navbar: LayoutNavbar_default,
  Content: LayoutContent_default,
  Header: LayoutHeader_default
});
var layouts_default = Layout2;

// src/components/AppShell/AppShellNavbar.tsx
import { Fragment as Fragment10, jsx as jsx85 } from "react/jsx-runtime";
var AppShellNavbar = ({ children }) => {
  return /* @__PURE__ */ jsx85(Fragment10, { children });
};
AppShellNavbar.displayName = "AppShellNavbar";

// src/components/AppShell/AppShellSidebar.tsx
import { Fragment as Fragment11, jsx as jsx86 } from "react/jsx-runtime";
var AppShellSidebar = ({ children }) => {
  return /* @__PURE__ */ jsx86(Fragment11, { children });
};
AppShellSidebar.displayName = "AppShellSidebar";

// src/components/AppShell/AppShellMain.tsx
import { Fragment as Fragment12, jsx as jsx87 } from "react/jsx-runtime";
var AppShellMain = ({ children }) => {
  return /* @__PURE__ */ jsx87(Fragment12, { children });
};
AppShellMain.displayName = "AppShellMain";

// src/components/AppShell/AppShellFooter.tsx
import { Fragment as Fragment13, jsx as jsx88 } from "react/jsx-runtime";
var AppShellFooter = ({ children }) => {
  return /* @__PURE__ */ jsx88(Fragment13, { children });
};
AppShellFooter.displayName = "AppShellFooter";

// src/components/AppShell/AppShellNotificationPanel.tsx
import { Fragment as Fragment14, jsx as jsx89 } from "react/jsx-runtime";
var AppShellNotificationPanel = ({ children }) => {
  return /* @__PURE__ */ jsx89(Fragment14, { children });
};
AppShellNotificationPanel.displayName = "AppShellNotificationPanel";

// src/components/AppShell/AppShell.tsx
import { jsx as jsx90, jsxs as jsxs31 } from "react/jsx-runtime";
var SLOT_DISPLAY_NAMES = {
  Navbar: "AppShellNavbar",
  Sidebar: "AppShellSidebar",
  Main: "AppShellMain",
  Footer: "AppShellFooter",
  NotificationPanel: "AppShellNotificationPanel"
};
var extractSlots = (children) => {
  const slots = {
    navbar: null,
    sidebar: null,
    main: null,
    footer: null,
    notificationPanel: null
  };
  React25.Children.forEach(children, (child) => {
    var _a;
    if (React25.isValidElement(child)) {
      const displayName = (_a = child.type) == null ? void 0 : _a.displayName;
      const childProps = child.props;
      switch (displayName) {
        case SLOT_DISPLAY_NAMES.Navbar:
          slots.navbar = childProps.children;
          break;
        case SLOT_DISPLAY_NAMES.Sidebar:
          slots.sidebar = childProps.children;
          break;
        case SLOT_DISPLAY_NAMES.Main:
          slots.main = childProps.children;
          break;
        case SLOT_DISPLAY_NAMES.Footer:
          slots.footer = childProps.children;
          break;
        case SLOT_DISPLAY_NAMES.NotificationPanel:
          slots.notificationPanel = childProps.children;
          break;
        default:
          if (!slots.main) {
            slots.main = child;
          }
      }
    }
  });
  return slots;
};
var AppShell = ({ children, sx }) => {
  const slots = extractSlots(children);
  return /* @__PURE__ */ jsxs31(layouts_default, { sx: __spreadValues({ height: "100vh", flexDirection: "column" }, sx), children: [
    slots.navbar && /* @__PURE__ */ jsx90(layouts_default.Navbar, { children: slots.navbar }),
    /* @__PURE__ */ jsxs31(layouts_default, { sx: { flex: 1, overflow: "hidden" }, children: [
      slots.sidebar && /* @__PURE__ */ jsx90(layouts_default.Sidebar, { children: slots.sidebar }),
      /* @__PURE__ */ jsxs31(layouts_default.Content, { sx: { flex: 1, display: "flex", flexDirection: "column" }, children: [
        /* @__PURE__ */ jsx90(Box35, { sx: { flex: 1, overflow: "auto", display: "flex" }, children: slots.main }),
        slots.footer
      ] })
    ] }),
    slots.notificationPanel
  ] });
};
AppShell.Navbar = AppShellNavbar;
AppShell.Sidebar = AppShellSidebar;
AppShell.Main = AppShellMain;
AppShell.Footer = AppShellFooter;
AppShell.NotificationPanel = AppShellNotificationPanel;
AppShell.displayName = "AppShell";

// src/components/ListingTable/ListingTable/ListingTable.tsx
import MuiTable from "@mui/material/Table";
import { styled as styled66 } from "@mui/material/styles";

// src/components/ListingTable/shared/types.ts
var densityStyles = {
  compact: { padding: "6px 16px" },
  standard: { padding: "16px" },
  comfortable: { padding: "24px 16px" }
};

// src/components/ListingTable/ListingTable/ListingTableContainer.tsx
import MuiTableContainer from "@mui/material/TableContainer";
import Paper from "@mui/material/Paper";
import { jsx as jsx91 } from "react/jsx-runtime";
function ListingTableContainer(_a) {
  var _b = _a, {
    elevation = 1,
    disablePaper = false,
    children
  } = _b, props = __objRest(_b, [
    "elevation",
    "disablePaper",
    "children"
  ]);
  if (disablePaper) {
    return /* @__PURE__ */ jsx91(MuiTableContainer, __spreadProps(__spreadValues({ sx: __spreadValues({ width: "100%" }, props.sx) }, props), { children }));
  }
  return /* @__PURE__ */ jsx91(MuiTableContainer, __spreadProps(__spreadValues({ component: Paper, elevation, sx: __spreadValues({ width: "100%" }, props.sx) }, props), { children }));
}
var ListingTableContainer_default = ListingTableContainer;

// src/components/ListingTable/ListingTable/ListingTableHead.tsx
import MuiTableHead from "@mui/material/TableHead";
import { styled as styled62 } from "@mui/material/styles";
import { jsx as jsx92 } from "react/jsx-runtime";
var StyledTableHead = styled62(MuiTableHead)(({ theme }) => ({
  "& .MuiTableCell-head": __spreadValues(__spreadValues({
    fontWeight: 600
  }, theme.applyStyles("light", {
    backgroundColor: theme.palette.grey[50]
  })), theme.applyStyles("dark", {
    backgroundColor: "rgba(255, 255, 255, 0.04)"
  }))
}));
function ListingTableHead(_a) {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx92(StyledTableHead, __spreadProps(__spreadValues({}, props), { children }));
}
var ListingTableHead_default = ListingTableHead;

// src/components/ListingTable/ListingTable/ListingTableBody.tsx
import MuiTableBody from "@mui/material/TableBody";
import { jsx as jsx93 } from "react/jsx-runtime";
function ListingTableBody(_a) {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx93(MuiTableBody, __spreadProps(__spreadValues({}, props), { children }));
}
var ListingTableBody_default = ListingTableBody;

// src/components/ListingTable/ListingTable/ListingTableFooter.tsx
import MuiTableFooter from "@mui/material/TableFooter";
import { jsx as jsx94 } from "react/jsx-runtime";
function ListingTableFooter(_a) {
  var _b = _a, { children } = _b, props = __objRest(_b, ["children"]);
  return /* @__PURE__ */ jsx94(MuiTableFooter, __spreadProps(__spreadValues({}, props), { children }));
}
var ListingTableFooter_default = ListingTableFooter;

// src/components/ListingTable/ListingTable/ListingTableRow.tsx
import MuiTableRow from "@mui/material/TableRow";
import { styled as styled63, alpha } from "@mui/material/styles";
import { jsx as jsx95 } from "react/jsx-runtime";
var StyledTableRow = styled63(MuiTableRow, {
  shouldForwardProp: (prop) => !["clickable", "variant"].includes(prop)
})(({ theme: muiTheme, clickable, variant = "table" }) => {
  const theme = muiTheme;
  const borderRadius = 8;
  const isDarkMode = theme.palette.mode === "dark";
  return __spreadValues(__spreadValues(__spreadValues({}, clickable && {
    cursor: "pointer"
  }), variant === "card" && (() => {
    var _a, _b, _c, _d;
    const borderColor = alpha(
      isDarkMode ? theme.palette.common.white : theme.palette.common.black,
      isDarkMode ? 0.3 : 0.12
    );
    return {
      // Use acrylic background for glassmorphism effect, consistent with MuiCard/MuiPaper
      backgroundColor: (_b = (_a = theme.vars) == null ? void 0 : _a.palette.background.acrylic) != null ? _b : (theme.vars || theme).palette.background.paper,
      backdropFilter: (_c = theme.blur) == null ? void 0 : _c.light,
      WebkitBackdropFilter: (_d = theme.blur) == null ? void 0 : _d.light,
      borderRadius,
      "& .MuiTableCell-root": {
        backgroundColor: "transparent",
        // Apply top and bottom borders to all cells
        borderTop: `1px solid ${borderColor}`,
        borderBottom: `1px solid ${borderColor}`,
        "&:first-of-type": {
          borderTopLeftRadius: borderRadius,
          borderBottomLeftRadius: borderRadius,
          // Left border on first cell
          borderLeft: `1px solid ${borderColor}`
        },
        "&:last-of-type": {
          borderTopRightRadius: borderRadius,
          borderBottomRightRadius: borderRadius,
          // Right border on last cell
          borderRight: `1px solid ${borderColor}`
        }
      },
      "&:hover": {
        backgroundColor: (theme.vars || theme).palette.action.hover
      }
    };
  })()), variant === "table" && {
    "&:last-child td, &:last-child th": {
      border: 0
    }
  });
});
function ListingTableRow(_a) {
  var _b = _a, {
    clickable = false,
    variant = "table",
    children
  } = _b, props = __objRest(_b, [
    "clickable",
    "variant",
    "children"
  ]);
  return /* @__PURE__ */ jsx95(StyledTableRow, __spreadProps(__spreadValues({ clickable, variant }, props), { children }));
}
var ListingTableRow_default = ListingTableRow;

// src/components/ListingTable/ListingTable/ListingTableCell.tsx
import MuiTableCell from "@mui/material/TableCell";
import { styled as styled64 } from "@mui/material/styles";
import { jsx as jsx96 } from "react/jsx-runtime";
var StyledTableCell = styled64(MuiTableCell, {
  shouldForwardProp: (prop) => {
    const excludedProps = ["truncate", "maxWidth"];
    return !excludedProps.some((excluded) => excluded === prop);
  }
})(({ truncate, maxWidth }) => __spreadValues(__spreadValues({}, truncate && {
  overflow: "hidden",
  textOverflow: "ellipsis",
  whiteSpace: "nowrap"
}), maxWidth && {
  maxWidth: typeof maxWidth === "number" ? `${maxWidth}px` : maxWidth
}));
function ListingTableCell(_a) {
  var _b = _a, {
    truncate = false,
    maxWidth,
    children
  } = _b, props = __objRest(_b, [
    "truncate",
    "maxWidth",
    "children"
  ]);
  return /* @__PURE__ */ jsx96(StyledTableCell, __spreadProps(__spreadValues({ truncate, maxWidth }, props), { children }));
}
var ListingTableCell_default = ListingTableCell;

// src/components/ListingTable/shared/ListingTableToolbar.tsx
import { useState as useState6, useCallback } from "react";
import Box36 from "@mui/material/Box";
import TextField3 from "@mui/material/TextField";
import InputAdornment3 from "@mui/material/InputAdornment";
import IconButton7 from "@mui/material/IconButton";
import { Search as Search3, X as X4 } from "@wso2/oxygen-ui-icons-react";

// src/components/ListingTable/context.ts
import * as React26 from "react";
var ListingTableContext = React26.createContext(null);
ListingTableContext.displayName = "ListingTableContext";
var useListingTable = () => {
  return React26.useContext(ListingTableContext);
};
var useListingTableRequired = () => {
  const context = React26.useContext(ListingTableContext);
  if (!context) {
    throw new Error(
      "useListingTableRequired must be used within a ListingTable.Provider. Use useListingTable() for optional context access."
    );
  }
  return context;
};

// src/components/ListingTable/shared/ListingTableToolbar.tsx
import { Fragment as Fragment15, jsx as jsx97, jsxs as jsxs32 } from "react/jsx-runtime";
function ListingTableToolbar({
  showSearch = false,
  searchSlot,
  searchValue: controlledSearchValue,
  onSearchChange,
  searchPlaceholder = "Search...",
  searchMinWidth = 200,
  searchMaxWidth = 300,
  actions,
  children,
  sx
}) {
  const tableContext = useListingTable();
  const [internalSearchValue, setInternalSearchValue] = useState6("");
  const isControlledByProp = controlledSearchValue !== void 0;
  const isControlledByContext = !isControlledByProp && (tableContext == null ? void 0 : tableContext.searchValue) !== void 0;
  const isControlled = isControlledByProp || isControlledByContext;
  const searchValue = isControlledByProp ? controlledSearchValue : isControlledByContext ? tableContext.searchValue : internalSearchValue;
  const effectiveOnSearchChange = onSearchChange != null ? onSearchChange : tableContext == null ? void 0 : tableContext.onSearchChange;
  const handleSearchChange = useCallback(
    (event) => {
      const newValue = event.target.value;
      if (!isControlled) {
        setInternalSearchValue(newValue);
      }
      effectiveOnSearchChange == null ? void 0 : effectiveOnSearchChange(newValue);
    },
    [isControlled, effectiveOnSearchChange]
  );
  const handleClearSearch = useCallback(() => {
    if (!isControlled) {
      setInternalSearchValue("");
    }
    effectiveOnSearchChange == null ? void 0 : effectiveOnSearchChange("");
  }, [isControlled, effectiveOnSearchChange]);
  const renderSearch = () => {
    if (searchSlot) {
      return searchSlot;
    }
    if (showSearch) {
      return /* @__PURE__ */ jsx97(
        TextField3,
        {
          size: "small",
          placeholder: searchPlaceholder,
          value: searchValue,
          onChange: handleSearchChange,
          sx: {
            minWidth: searchMinWidth,
            maxWidth: searchMaxWidth
          },
          InputProps: {
            startAdornment: /* @__PURE__ */ jsx97(InputAdornment3, { position: "start", children: /* @__PURE__ */ jsx97(Search3, { size: 18 }) }),
            endAdornment: searchValue ? /* @__PURE__ */ jsx97(InputAdornment3, { position: "end", children: /* @__PURE__ */ jsx97(IconButton7, { size: "small", onClick: handleClearSearch, edge: "end", children: /* @__PURE__ */ jsx97(X4, { size: 16 }) }) }) : null
          }
        }
      );
    }
    return null;
  };
  return /* @__PURE__ */ jsxs32(
    Box36,
    {
      sx: __spreadValues({
        display: "flex",
        alignItems: "center",
        gap: 2,
        padding: 2,
        borderBottom: 1,
        borderColor: "divider"
      }, sx),
      children: [
        renderSearch(),
        children,
        actions && /* @__PURE__ */ jsxs32(Fragment15, { children: [
          /* @__PURE__ */ jsx97(Box36, { sx: { flex: 1 } }),
          /* @__PURE__ */ jsx97(Box36, { sx: { display: "flex", alignItems: "center", gap: 1 }, children: actions })
        ] })
      ]
    }
  );
}
var ListingTableToolbar_default = ListingTableToolbar;

// src/components/ListingTable/shared/ListingTableEmptyState.tsx
import Box37 from "@mui/material/Box";
import Typography19 from "@mui/material/Typography";
import { jsx as jsx98, jsxs as jsxs33 } from "react/jsx-runtime";
function ListingTableEmptyState({
  title = "No data available",
  description,
  illustration,
  action,
  minHeight = 300,
  sx
}) {
  return /* @__PURE__ */ jsxs33(
    Box37,
    {
      sx: __spreadValues({
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        justifyContent: "center",
        minHeight: typeof minHeight === "number" ? `${minHeight}px` : minHeight,
        p: 4,
        textAlign: "center"
      }, sx),
      children: [
        illustration && /* @__PURE__ */ jsx98(
          Box37,
          {
            sx: {
              mb: 2,
              color: "text.secondary",
              "& svg": {
                width: 64,
                height: 64
              }
            },
            children: illustration
          }
        ),
        /* @__PURE__ */ jsx98(Typography19, { variant: "h6", component: "div", sx: { mb: description ? 1 : 0, fontWeight: 500 }, children: title }),
        description && /* @__PURE__ */ jsx98(Typography19, { variant: "body2", color: "text.secondary", sx: { mb: action ? 2 : 0, maxWidth: 400 }, children: description }),
        action && /* @__PURE__ */ jsx98(Box37, { children: action })
      ]
    }
  );
}
var ListingTableEmptyState_default = ListingTableEmptyState;

// src/components/ListingTable/shared/ListingTableDensityControl.tsx
import { useCallback as useCallback2 } from "react";
import ToggleButton from "@mui/material/ToggleButton";
import ToggleButtonGroup from "@mui/material/ToggleButtonGroup";
import Tooltip6 from "@mui/material/Tooltip";
import { AlignJustify, Menu as Menu3, List as List4 } from "@wso2/oxygen-ui-icons-react";
import { jsx as jsx99 } from "react/jsx-runtime";
var densityOptions = [
  { value: "compact", label: "Compact", icon: /* @__PURE__ */ jsx99(AlignJustify, { size: 18 }) },
  { value: "standard", label: "Standard", icon: /* @__PURE__ */ jsx99(Menu3, { size: 18 }) },
  { value: "comfortable", label: "Comfortable", icon: /* @__PURE__ */ jsx99(List4, { size: 18 }) }
];
function ListingTableDensityControl({
  value: valueProp,
  onChange: onChangeProp,
  size = "small",
  sx
}) {
  var _a;
  const tableContext = useListingTable();
  const value = (_a = valueProp != null ? valueProp : tableContext == null ? void 0 : tableContext.density) != null ? _a : "standard";
  const effectiveOnChange = onChangeProp != null ? onChangeProp : tableContext == null ? void 0 : tableContext.onDensityChange;
  const handleChange = useCallback2(
    (_event, newDensity) => {
      if (newDensity !== null) {
        effectiveOnChange == null ? void 0 : effectiveOnChange(newDensity);
      }
    },
    [effectiveOnChange]
  );
  return /* @__PURE__ */ jsx99(ToggleButtonGroup, { value, exclusive: true, onChange: handleChange, size, sx, children: densityOptions.map((option) => /* @__PURE__ */ jsx99(ToggleButton, { value: option.value, children: /* @__PURE__ */ jsx99(Tooltip6, { title: option.label, children: /* @__PURE__ */ jsx99("span", { style: { display: "flex", alignItems: "center" }, children: option.icon }) }) }, option.value)) });
}
var ListingTableDensityControl_default = ListingTableDensityControl;

// src/components/ListingTable/shared/ListingTableSortLabel.tsx
import { useCallback as useCallback3 } from "react";
import MuiTableSortLabel from "@mui/material/TableSortLabel";
import { styled as styled65 } from "@mui/material/styles";
import { jsx as jsx100 } from "react/jsx-runtime";
var StyledTableSortLabel = styled65(MuiTableSortLabel)(({ theme }) => ({
  color: "inherit !important",
  "&.Mui-active": {
    color: "inherit !important",
    "& .MuiTableSortLabel-icon": {
      color: `${theme.palette.primary.main} !important`
    }
  }
}));
function ListingTableSortLabel(_a) {
  var _b = _a, {
    field,
    active: activeProp,
    direction: directionProp,
    onClick: onClickProp,
    children,
    hideSortIcon = false
  } = _b, props = __objRest(_b, [
    "field",
    "active",
    "direction",
    "onClick",
    "children",
    "hideSortIcon"
  ]);
  var _a2;
  const tableContext = useListingTable();
  const isActiveFromContext = field !== void 0 && (tableContext == null ? void 0 : tableContext.sortField) === field;
  const active = activeProp != null ? activeProp : isActiveFromContext;
  const direction = (_a2 = directionProp != null ? directionProp : isActiveFromContext ? tableContext == null ? void 0 : tableContext.sortDirection : void 0) != null ? _a2 : "asc";
  const handleClick = useCallback3(
    (event) => {
      if (onClickProp) {
        onClickProp(event);
        return;
      }
      if (field && (tableContext == null ? void 0 : tableContext.onSortChange)) {
        const newDirection = active && direction === "asc" ? "desc" : "asc";
        tableContext.onSortChange(field, newDirection);
      }
    },
    [onClickProp, field, tableContext, active, direction]
  );
  return /* @__PURE__ */ jsx100(StyledTableSortLabel, __spreadProps(__spreadValues({ active, direction, onClick: handleClick, hideSortIcon }, props), { children }));
}
var ListingTableSortLabel_default = ListingTableSortLabel;

// src/components/ListingTable/shared/ListingTableRowActions.tsx
import Box38 from "@mui/material/Box";
import { jsx as jsx101 } from "react/jsx-runtime";
var alignmentMap = {
  left: "flex-start",
  center: "center",
  right: "flex-end"
};
function ListingTableRowActions({
  children,
  visibility = "always",
  align = "right",
  sx
}) {
  return /* @__PURE__ */ jsx101(
    Box38,
    {
      className: "table-row-actions",
      sx: __spreadValues(__spreadValues({
        display: "flex",
        alignItems: "center",
        justifyContent: alignmentMap[align],
        gap: 0.5
      }, visibility === "hover" && {
        opacity: 0,
        transition: "opacity 0.2s ease-in-out",
        // Support both TableView rows and ListView items
        ".MuiTableRow-root:hover &, .list-view-item:hover &": {
          opacity: 1
        }
      }), sx),
      children
    }
  );
}
var ListingTableRowActions_default = ListingTableRowActions;

// src/components/ListingTable/shared/ListingTableCellIcon.tsx
import Box39 from "@mui/material/Box";
import Typography20 from "@mui/material/Typography";
import { Fragment as Fragment16, jsx as jsx102, jsxs as jsxs34 } from "react/jsx-runtime";
function ListingTableCellIcon({
  icon,
  primary,
  secondary,
  iconPosition = "left",
  iconSx,
  sx
}) {
  const iconElement = /* @__PURE__ */ jsx102(
    Box39,
    {
      sx: __spreadValues({
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        flexShrink: 0,
        color: "text.secondary"
      }, iconSx),
      children: icon
    }
  );
  const textContent = /* @__PURE__ */ jsxs34(Box39, { sx: { minWidth: 0, flex: 1 }, children: [
    /* @__PURE__ */ jsx102(
      Typography20,
      {
        variant: "body2",
        component: "div",
        sx: {
          fontWeight: 500,
          overflow: "hidden",
          textOverflow: "ellipsis",
          whiteSpace: "nowrap"
        },
        children: primary
      }
    ),
    secondary && /* @__PURE__ */ jsx102(
      Typography20,
      {
        variant: "caption",
        component: "div",
        color: "text.secondary",
        sx: {
          overflow: "hidden",
          textOverflow: "ellipsis",
          whiteSpace: "nowrap"
        },
        children: secondary
      }
    )
  ] });
  return /* @__PURE__ */ jsx102(
    Box39,
    {
      sx: __spreadValues({
        display: "flex",
        alignItems: "center",
        gap: 1.5,
        minWidth: 0
      }, sx),
      children: iconPosition === "left" ? /* @__PURE__ */ jsxs34(Fragment16, { children: [
        iconElement,
        textContent
      ] }) : /* @__PURE__ */ jsxs34(Fragment16, { children: [
        textContent,
        iconElement
      ] })
    }
  );
}
var ListingTableCellIcon_default = ListingTableCellIcon;

// src/components/ListingTable/ListingTableProvider.tsx
import { useMemo as useMemo6 } from "react";
import { jsx as jsx103 } from "react/jsx-runtime";
function ListingTableProvider({
  children,
  // Search
  searchValue,
  onSearchChange,
  // Sort
  sortField,
  sortDirection,
  onSortChange,
  // Pagination
  page,
  rowsPerPage,
  totalCount,
  onPageChange,
  onRowsPerPageChange,
  // Selection
  selected,
  onSelectionChange,
  onSelectAll,
  onClearSelection,
  isSelected,
  // Filters
  filters,
  onFilterChange,
  onClearFilters,
  // Bulk Actions
  onBulkDelete,
  onBulkAction,
  // Extension
  customHandlers,
  // UI State
  density,
  onDensityChange,
  variant,
  // Loading
  loading
}) {
  const contextValue = useMemo6(
    () => ({
      // Search
      searchValue,
      onSearchChange,
      // Sort
      sortField,
      sortDirection,
      onSortChange,
      // Pagination
      page,
      rowsPerPage,
      totalCount,
      onPageChange,
      onRowsPerPageChange,
      // Selection
      selected,
      onSelectionChange,
      onSelectAll,
      onClearSelection,
      isSelected,
      // Filters
      filters,
      onFilterChange,
      onClearFilters,
      // Bulk Actions
      onBulkDelete,
      onBulkAction,
      // Extension
      customHandlers,
      // UI State
      density,
      onDensityChange,
      variant,
      // Loading
      loading
    }),
    [
      searchValue,
      onSearchChange,
      sortField,
      sortDirection,
      onSortChange,
      page,
      rowsPerPage,
      totalCount,
      onPageChange,
      onRowsPerPageChange,
      selected,
      onSelectionChange,
      onSelectAll,
      onClearSelection,
      isSelected,
      filters,
      onFilterChange,
      onClearFilters,
      onBulkDelete,
      onBulkAction,
      customHandlers,
      density,
      onDensityChange,
      variant,
      loading
    ]
  );
  return /* @__PURE__ */ jsx103(ListingTableContext.Provider, { value: contextValue, children });
}

// src/components/ListingTable/ListingTable/ListingTable.tsx
import { jsx as jsx104 } from "react/jsx-runtime";
var StyledTable = styled66(MuiTable, {
  shouldForwardProp: (prop) => {
    const excludedProps = ["variant", "density", "striped", "bordered"];
    return !excludedProps.some((excluded) => excluded === prop);
  }
})(({ theme, variant = "table", density = "standard", striped, bordered }) => __spreadValues(__spreadProps(__spreadValues({}, variant === "card" && {
  borderCollapse: "separate",
  borderSpacing: "0 12px",
  // Remove default table styling for card variant
  "& .MuiTableHead-root .MuiTableRow-root": {
    backgroundColor: "transparent"
  },
  "& .MuiTableHead-root .MuiTableCell-root.MuiTableCell-head": __spreadValues(__spreadValues({
    backgroundColor: "transparent",
    borderBottom: "none",
    fontWeight: 600,
    padding: "0 16px 8px 16px"
  }, theme.applyStyles("light", {
    color: theme.palette.text.secondary
  })), theme.applyStyles("dark", {
    color: theme.palette.grey[400]
  }))
}), {
  // Standard table cell padding based on density
  "& .MuiTableCell-root": __spreadValues({
    padding: densityStyles[density].padding
  }, bordered && variant === "table" && {
    borderRight: `1px solid ${theme.palette.divider}`,
    "&:last-child": {
      borderRight: "none"
    }
  })
}), striped && variant === "table" && {
  "& .MuiTableBody-root .MuiTableRow-root:nth-of-type(odd)": {
    backgroundColor: theme.palette.action.hover
  }
}));
function ListingTable(_a) {
  var _b = _a, {
    variant = "table",
    density = "standard",
    striped = false,
    bordered = false,
    children,
    sx
  } = _b, props = __objRest(_b, [
    "variant",
    "density",
    "striped",
    "bordered",
    "children",
    "sx"
  ]);
  return /* @__PURE__ */ jsx104(
    StyledTable,
    __spreadProps(__spreadValues({
      variant,
      density,
      striped,
      bordered,
      sx
    }, props), {
      children
    })
  );
}
var ListingTableCompound = Object.assign(ListingTable, {
  Provider: ListingTableProvider,
  Container: ListingTableContainer_default,
  Head: ListingTableHead_default,
  Body: ListingTableBody_default,
  Footer: ListingTableFooter_default,
  Row: ListingTableRow_default,
  Cell: ListingTableCell_default,
  CellIcon: ListingTableCellIcon_default,
  RowActions: ListingTableRowActions_default,
  EmptyState: ListingTableEmptyState_default,
  Toolbar: ListingTableToolbar_default,
  SortLabel: ListingTableSortLabel_default,
  DensityControl: ListingTableDensityControl_default
});
var ListingTable_default = ListingTableCompound;

// src/animations/ParticleBackground/ParticleBackground.tsx
import { useEffect as useEffect4, useRef } from "react";
import { useColorScheme as useColorScheme3 } from "@mui/material/styles";
import { jsx as jsx105 } from "react/jsx-runtime";
var ParticleBackground = ({
  opacity = 0.5,
  baseDensity = 0.12,
  maxSpeed = 0.3,
  radius = [2, 3.2],
  linkDist = 210,
  linkAlpha = 0.2,
  mouseInfluence = 110,
  repelStrength = 0.35,
  clickBurst = 120
}) => {
  const canvasRef = useRef(null);
  const { mode } = useColorScheme3();
  useEffect4(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d", { alpha: true });
    if (!ctx) return;
    const canvasEl = canvas;
    const context = ctx;
    const lightColors = {
      colorParticle: "#44444447",
      colorLink: "#000000"
    };
    const darkColors = {
      colorParticle: "#ffffff47",
      colorLink: "#ffffff"
    };
    let currentColors = mode === "dark" ? darkColors : lightColors;
    let DPR = Math.max(1, Math.min(2, window.devicePixelRatio || 1));
    let W = 0;
    let H = 0;
    let particles = [];
    let targetCount = 0;
    let animationFrameId;
    const rand = (min, max) => Math.random() * (max - min) + min;
    const clamp = (v, a, b) => Math.max(a, Math.min(b, v));
    class Particle {
      constructor() {
        this.x = 0;
        this.y = 0;
        this.vx = 0;
        this.vy = 0;
        this.r = 0;
        this.reset(true);
      }
      reset(randomPos = false) {
        this.x = randomPos ? rand(0, W) : Math.random() < 0.5 ? 0 : W;
        this.y = randomPos ? rand(0, H) : rand(0, H);
        const ang = rand(0, Math.PI * 2);
        const speed = rand(0.05, maxSpeed);
        this.vx = Math.cos(ang) * speed;
        this.vy = Math.sin(ang) * speed;
        this.r = rand(radius[0], radius[1]) * DPR;
      }
      step(mx, my) {
        if (mx !== null && my !== null) {
          const dx = this.x - mx;
          const dy = this.y - my;
          const d2 = dx * dx + dy * dy;
          const r = mouseInfluence * DPR;
          if (d2 < r * r) {
            const d = Math.sqrt(d2) || 1e-3;
            const ux = dx / d;
            const uy = dy / d;
            this.vx += ux * repelStrength * (1 - d / r);
            this.vy += uy * repelStrength * (1 - d / r);
          }
        }
        const sp = Math.hypot(this.vx, this.vy);
        const maxSp = maxSpeed;
        if (sp > maxSp) {
          this.vx *= maxSp / sp;
          this.vy *= maxSp / sp;
        }
        this.x += this.vx * DPR;
        this.y += this.vy * DPR;
        if (this.x < -50) this.x = W + 50;
        if (this.x > W + 50) this.x = -50;
        if (this.y < -50) this.y = H + 50;
        if (this.y > H + 50) this.y = -50;
      }
      draw() {
        context.beginPath();
        context.arc(this.x, this.y, this.r, 0, Math.PI * 2);
        context.fillStyle = currentColors.colorParticle;
        context.globalAlpha = 0.9;
        context.fill();
      }
    }
    function resize() {
      DPR = Math.max(1, Math.min(2, window.devicePixelRatio || 1));
      W = canvasEl.width = Math.floor(window.innerWidth * DPR);
      H = canvasEl.height = Math.floor(window.innerHeight * DPR);
      canvasEl.style.width = window.innerWidth + "px";
      canvasEl.style.height = window.innerHeight + "px";
      computeParticlesCount();
    }
    function computeParticlesCount() {
      const area = W * H / (DPR * DPR);
      targetCount = Math.round(baseDensity * (area / 1e4));
      targetCount = clamp(targetCount, 40, 220);
      if (particles.length < targetCount) {
        const add = targetCount - particles.length;
        for (let i = 0; i < add; i++) particles.push(new Particle());
      } else if (particles.length > targetCount) {
        particles.length = targetCount;
      }
    }
    const mouse = { x: null, y: null };
    const handleMouseMove = (e) => {
      mouse.x = e.clientX * DPR;
      mouse.y = e.clientY * DPR;
    };
    const handleMouseLeave = () => {
      mouse.x = mouse.y = null;
    };
    const handleClick = (e) => {
      const mx = e.clientX * DPR;
      const my = e.clientY * DPR;
      for (let i = 0; i < particles.length; i++) {
        const p = particles[i];
        const dx = p.x - mx;
        const dy = p.y - my;
        const d2 = dx * dx + dy * dy;
        const r = mouseInfluence * DPR;
        if (d2 < r * r) {
          const d = Math.sqrt(d2) || 1e-3;
          const ux = dx / d;
          const uy = dy / d;
          p.vx += ux * (clickBurst / 100);
          p.vy += uy * (clickBurst / 100);
        }
      }
    };
    function drawLinks() {
      context.lineWidth = 1 * DPR;
      context.strokeStyle = currentColors.colorLink;
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          const a = particles[i];
          const b = particles[j];
          const dx = a.x - b.x;
          const dy = a.y - b.y;
          const dist = Math.hypot(dx, dy);
          if (dist < linkDist * DPR) {
            const alpha2 = linkAlpha * (1 - dist / (linkDist * DPR));
            context.globalAlpha = alpha2;
            context.beginPath();
            context.moveTo(a.x, a.y);
            context.lineTo(b.x, b.y);
            context.stroke();
          }
        }
      }
      context.globalAlpha = 1;
    }
    function loop() {
      context.clearRect(0, 0, W, H);
      for (let i = 0; i < particles.length; i++) {
        particles[i].step(mouse.x, mouse.y);
      }
      drawLinks();
      for (let i = 0; i < particles.length; i++) {
        particles[i].draw();
      }
      animationFrameId = requestAnimationFrame(loop);
    }
    resize();
    for (let i = 0; i < 120; i++) particles.push(new Particle());
    computeParticlesCount();
    window.addEventListener("resize", resize, { passive: true });
    window.addEventListener("mousemove", handleMouseMove, { passive: true });
    window.addEventListener("mouseleave", handleMouseLeave);
    window.addEventListener("click", handleClick);
    loop();
    return () => {
      window.removeEventListener("resize", resize);
      window.removeEventListener("mousemove", handleMouseMove);
      window.removeEventListener("mouseleave", handleMouseLeave);
      window.removeEventListener("click", handleClick);
      cancelAnimationFrame(animationFrameId);
    };
  }, [baseDensity, maxSpeed, radius, linkDist, linkAlpha, mouseInfluence, repelStrength, clickBurst, mode]);
  return /* @__PURE__ */ jsx105(
    "canvas",
    {
      ref: canvasRef,
      id: "particle-bg",
      style: {
        position: "fixed",
        top: 0,
        left: 0,
        width: "100%",
        height: "100%",
        pointerEvents: "none",
        zIndex: -1,
        opacity
      }
    }
  );
};

// src/hooks/useThemeContent/useThemeContent.ts
function useThemeContent(contentMap, fallback) {
  var _a, _b;
  const { currentTheme } = useThemeSwitcher();
  return (_b = (_a = contentMap[currentTheme]) != null ? _a : fallback) != null ? _b : Object.values(contentMap)[0];
}
var useThemeContent_default = useThemeContent;

// src/hooks/useAppShell/useAppShell.ts
import * as React27 from "react";
var useAppShell = (options = {}) => {
  const {
    initialCollapsed = false,
    initialActiveItem = "dashboard"
  } = options;
  const [state, setState] = React27.useState({
    sidebarCollapsed: initialCollapsed,
    notificationPanelOpen: false,
    activeMenuItem: initialActiveItem,
    expandedMenus: {}
  });
  const toggleSidebar = React27.useCallback(() => {
    setState((prev) => __spreadProps(__spreadValues({}, prev), { sidebarCollapsed: !prev.sidebarCollapsed }));
  }, []);
  const toggleNotificationPanel = React27.useCallback(() => {
    setState((prev) => __spreadProps(__spreadValues({}, prev), {
      notificationPanelOpen: !prev.notificationPanelOpen
    }));
  }, []);
  const setActiveMenuItem = React27.useCallback((id) => {
    setState((prev) => __spreadProps(__spreadValues({}, prev), { activeMenuItem: id }));
  }, []);
  const toggleMenu = React27.useCallback((id) => {
    setState((prev) => __spreadProps(__spreadValues({}, prev), {
      expandedMenus: __spreadProps(__spreadValues({}, prev.expandedMenus), {
        [id]: !prev.expandedMenus[id]
      })
    }));
  }, []);
  const actions = React27.useMemo(
    () => ({
      toggleSidebar,
      toggleNotificationPanel,
      setActiveMenuItem,
      toggleMenu
    }),
    [
      toggleSidebar,
      toggleNotificationPanel,
      setActiveMenuItem,
      toggleMenu
    ]
  );
  return {
    state,
    actions
  };
};

// src/hooks/useNotifications/useNotifications.ts
import * as React28 from "react";
var generateId = () => {
  return `notif-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
};
var useNotifications = (options = {}) => {
  const { initialNotifications = [] } = options;
  const [notifications, setNotifications] = React28.useState(
    initialNotifications
  );
  const markRead = React28.useCallback((id) => {
    setNotifications(
      (prev) => prev.map((n) => n.id === id ? __spreadProps(__spreadValues({}, n), { read: true }) : n)
    );
  }, []);
  const dismiss = React28.useCallback((id) => {
    setNotifications((prev) => prev.filter((n) => n.id !== id));
  }, []);
  const markAllRead = React28.useCallback(() => {
    setNotifications((prev) => prev.map((n) => __spreadProps(__spreadValues({}, n), { read: true })));
  }, []);
  const clearAll = React28.useCallback(() => {
    setNotifications([]);
  }, []);
  const add = React28.useCallback((notification) => {
    const newNotification = __spreadProps(__spreadValues({}, notification), {
      id: generateId()
    });
    setNotifications((prev) => [newNotification, ...prev]);
  }, []);
  const unreadCount = React28.useMemo(
    () => notifications.filter((n) => !n.read).length,
    [notifications]
  );
  const unreadNotifications = React28.useMemo(
    () => notifications.filter((n) => !n.read),
    [notifications]
  );
  const actions = React28.useMemo(
    () => ({
      markRead,
      dismiss,
      markAllRead,
      clearAll,
      add
    }),
    [markRead, dismiss, markAllRead, clearAll, add]
  );
  return {
    notifications,
    actions,
    unreadCount,
    unreadNotifications
  };
};

// src/index.ts
export * from "@mui/material";
import * as colors from "@mui/material/colors";
import * as DataGrid from "@mui/x-data-grid";
import * as DatePickers from "@mui/x-date-pickers";
import { AdapterDateFns } from "@mui/x-date-pickers/AdapterDateFns";
import * as TreeView from "@mui/x-tree-view";
var version = package_default.version;
export {
  AcrylicOrangeTheme_default as AcrylicOrangeTheme,
  AcrylicPurpleTheme_default as AcrylicPurpleTheme,
  AdapterDateFns,
  AppShell,
  ChoreoTheme_default as ChoreoTheme,
  ClassicTheme_default as ClassicTheme,
  CodeBlock_default as CodeBlock,
  ColorSchemeImage,
  ColorSchemeToggle,
  ColorSchemeToggle_default as ColorSchemeToggleDefault,
  ComplexSelect_default as ComplexSelect,
  DataGrid,
  DatePickers,
  Footer,
  Form,
  Header,
  HighContrastTheme_default as HighContrastTheme,
  layouts_default as Layout,
  ListingTable_default as ListingTable,
  ListingTableContext,
  ListingTableProvider,
  NotificationBanner,
  NotificationPanel,
  AcrylicBaseTheme_default as OxygenTheme,
  OxygenUIThemeProvider,
  PageContent_default as PageContent,
  PageTitle_default as PageTitle,
  PaleBaseTheme_default as PaleBaseTheme,
  PaleGray_default as PaleGrayTheme,
  PaleIndigo_default as PaleIndigoTheme,
  ParticleBackground,
  SearchBar_default as SearchBar,
  SearchBarWithAdvancedFilter_default as SearchBarWithAdvancedFilter,
  Sidebar,
  StatCard_default as StatCard,
  ThemeSelect,
  ThemeSwitcher,
  TreeView,
  UserMenu,
  colors,
  formatRelativeTime,
  pxToRem,
  useAppShell,
  useListingTable,
  useListingTableRequired,
  useNotifications,
  useThemeContent,
  useThemeContent_default as useThemeContentDefault,
  useThemeSwitcher,
  version
};
//# sourceMappingURL=index.js.map
