...
const parseToml = (content: string): Record<string, unknown> => {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      throw new Error(`Parse error at line ${lineNumber}: ${error instanceof Error ? error.message : 'Invalid syntax'}`);
    }
  }
...
const flattenObject = (obj: unknown, prefix = ''): Record<string, TomlValue> => {
...
const convertArrayToFormFormat = (arrayValue: unknown[], keyPrefix: string): Record<string, TomlValue> => {
  ...
  arrayValue.forEach((item, index) => {
    ...
    if (item && typeof item === 'object' && !Array.isArray(item)) {
      Object.assign(result, flattenObject(item, indexedKey));
    } else if (Array.isArray(item)) {
      Object.assign(result, convertArrayToFormFormat(item, indexedKey));
    } else {
...
const convertObjectToMapFormat = (objectValue: Record<string, unknown>, keyPrefix: string): Record<string, TomlValue> => {
  ...
  Object.entries(objectValue).forEach(([mapKey, mapValue]) => {
    ...
    if (mapValue && typeof mapValue === 'object' && !Array.isArray(mapValue)) {
      Object.assign(result, flattenObject(mapValue, fullKey));
    } else if (Array.isArray(mapValue)) {
      Object.assign(result, convertArrayToFormFormat(mapValue, fullKey));
    } else {
  ...
  try {
    ...
    Object.entries(parsed).forEach(([key, value]) => {
      if (Array.isArray(value)) {
        ...
        Object.entries(arrayFormatted).forEach(([arrayKey, arrayValue]) => configMap.set(arrayKey, arrayValue));
      } else if (value && typeof value === 'object') {
        const objectFormatted = convertObjectToMapFormat(value as Record<string, unknown>, key);
        Object.entries(objectFormatted).forEach(([objKey, objValue]) => configMap.set(objKey, objValue));
      } else {
        configMap.set(key, value as TomlValue);
      }
    });
    return { success: true, data: convertTomlValuesToBaseType(configMap) };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : 'Failed to parse TOML file' };
  }
};
...
const extractBaseKey = (tomlKey: string): string => {
  ...
  if (lastDotIndex > 0) baseKey = baseKey.substring(0, lastDotIndex);
  return baseKey;
};
  ...
  if (matchedSchemaKey.includes('[*]')) {
    ...
    if (arrayIndices.length > 0) {
      ...
      arrayIndices.forEach((index) => {
        if (result.includes('.[*]')) result = result.replace('.[*]', `.${index}`);
        else result = result.replace('[*]', `.${index}`);
      });
      return result;
    }
  }
...
const matchesWithStripping = (tomlKey: string, schemaKey: string): boolean => {
  if (schemaKey === tomlKey) return true;
  ...
  if (schemaKeyParts.length > 1 && checkKeyMatch(tomlKey, schemaKeyParts.slice(1).join('.'))) return true;
  return checkKeyMatch(tomlKey, schemaKey);
};
  ...
  tomlValues.forEach((value, tomlKey) => {
    for (const validKey of validKeys) {
      if (matchesWithStripping(tomlKey, validKey)) {
        filteredMap.set(convertTomlKeyToSchemaKey(tomlKey, validKey), value);
        break;
      }
    }
  });
