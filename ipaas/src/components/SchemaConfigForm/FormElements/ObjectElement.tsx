...
import { type LinkingInfo, generateArrayJsonPath, generateMapJsonPath, generateObjectJsonPath, generatedNestedLabel, isBaseType } from '../schemaUtils';
    ...
    <Box>
      {schema.properties &&
        Object.keys(schema.properties).map((key) => {
          ...
          if (propType === 'object' && prop.additionalProperties) {
            return (
              <MapElement
                key={key}
            ...
            return (
              <Box mt={2} key={generatedJsonPath}>
                <ObjectElement
                  title={nestedLabel || key}
                  type={propType}
                  schema={prop || { type: 'object', properties: {} }}
                  valueMap={valueMap}
                  validationMap={validationMap}
                  handleValueChange={handleValueChange}
                  handleValidationChange={handleValidationChange}
          ...
          if (isBaseType(propType) || prop.enum) {
            return (
              <BaseElement
                key={generatedJsonPath}
                title={nestedLabel || key}
                type={prop.enum ? 'string' : propType || ''}
                valueMap={valueMap}
                validationMap={validationMap}
                handleValueChange={handleValueChange}
                handleValidationChange={handleValidationChange}
