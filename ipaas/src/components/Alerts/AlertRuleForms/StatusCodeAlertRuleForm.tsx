...
export default function StatusCodeAlertRuleForm(props: AlertRuleFormProps): JSX.Element {
  ...
  const [metric, setMetric] = useState<AlertTypeOption | null>(selectedAlertRule?.statusCode ? ({ label: getAlertRuleMetricNameByValue(selectedAlertRule.statusCode as AlertTypeConstants), value: selectedAlertRule.statusCode } as AlertTypeOption) : null);
  ...
  const validate = (): AlertRule | null => {
    ...
    Object.assign(alertRule, { statusCode: metric?.value, count, interval: interval.value, emails: emailList });
    return alertRule;
  };
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 2, '& .MuiFormLabel-asterisk': { color: 'error.main' } }}>
      ...
      <Alert severity="info" variant="outlined">
        {getAlertRuleExplanation(AlertTypes.STATUS_CODE, metric?.label, count, interval.label as AlertRulePeriod)}
      </Alert>

      <AlertRuleFormActions
        isEditAlertRule={isEditAlertRule}
        isDisabled={!emailList.length || !count || !interval}
