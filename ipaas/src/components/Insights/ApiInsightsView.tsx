...
function SlowestApiBars({ rows }: { rows: { name: string; latencyMs: number }[] }): JSX.Element {
  if (rows.length === 0) return <Typography variant="body2" color="text.secondary">No data in range</Typography>;
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 2 }}>
            <InsightsCard title="Usage by Application">
              {data.traffic.byApplication.length === 0 ? (
                ...
                <ChartBox>
                  <BarChart data={data.traffic.byApplication} xAxisDataKey="label" height={220} colors={[CHART.requests]} bars={[{ dataKey: 'value', name: 'Requests', fill: CHART.requests, radius: [3, 3, 0, 0] }]} tooltip={{ show: true }} grid={{ show: true }} />
                </ChartBox>
              )}
            </InsightsCard>
