...
// envId/projectId are part of the shared contract for the cloud (OpenChoreo)
// product, which keys schedules per environment; the devant Choreo v2 backend
// keys by releaseId and ignores them.
export async function fetchExecutionConfigs(componentId: string, releaseId: string, _envId = '', _projectId = ''): Promise<GqlExecutionConfigs | null> {
