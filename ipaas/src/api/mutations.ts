...
// ── Update auto-deploy setting on a deployment track ──
...
export function useUpdateAutoDeployEnabled() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (input: UpdateAutoDeployInput) =>
      gql<{ updateDeploymentTrack: { id: string; autoDeployEnabled: boolean } }>(UPDATE_DEPLOYMENT_TRACK, {
        componentId: input.componentId,
...
export function useGenerateComponentEndpoints() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (input: GenerateComponentEndpointsInput) =>
      gql<{ generateComponentEndpoints: GqlEnvEndpoint[] }>(GENERATE_COMPONENT_ENDPOINTS, {
        componentId: input.componentId,
        versionId: input.versionId,
        releaseId: input.releaseId,
        commitHash: input.commitHash,
