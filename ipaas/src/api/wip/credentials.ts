...
function providerConfig(input: CreateGitCredentialInput): string {
  if (input.bitbucketCredential) return `bitbucketCredential: { userName: "${esc(input.bitbucketCredential.userName)}", appPassword: "${esc(input.bitbucketCredential.appPassword)}" }`;
  if (input.bitbucketServerConfig) return `bitbucketServerConfig: { serverUrl: "${esc(input.bitbucketServerConfig.serverUrl.replace(/\/$/, ''))}", serverToken: "${esc(input.bitbucketServerConfig.serverToken)}" }`;
  if (input.gitLabServerConfig) return `gitLabServerConfig: { serverUrl: "${esc(input.gitLabServerConfig.serverUrl.replace(/\/$/, ''))}", pat: "${esc(input.gitLabServerConfig.pat)}" }`;
  if (input.azureDevOpsConfig) return `azureDevOpsConfig: { organizationName: "${esc(input.azureDevOpsConfig.organizationName)}", pat: "${esc(input.azureDevOpsConfig.pat)}" }`;
  return '';
}
...
export async function createGitCredential(input: CreateGitCredentialInput): Promise<GitCredential> {
  ...
  return Array.isArray(d.createCommonCredential) ? d.createCommonCredential[0] : d.createCommonCredential;
}
